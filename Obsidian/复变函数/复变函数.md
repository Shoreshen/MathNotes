...
根据[[复变函数#^MK86|分母]]以及[[复变函数#^MK69|相乘]]
...
复函数 $f(x)$ 在点 $x_0\in\mathbb{C}$ 的去心[[复变函数#^MK55|领域]] $O(x_0,\rho)$ 上有定义

则存在 $A$ 满足 $\forall\epsilon>0\rightarrow \exists H>0\big(\forall x(|x-x_0|<H\rightarrow |f(x)-A|<\epsilon)\big)$ 其中 $|x|$ 为对 $x$ 求[[复变函数#^MK56|模]]
...
若对于 $x\in D\subset\mathbb{C}$ 均有上述性质，则称其在区域 $D$ 上连续
...
根据[[复变函数#^MK59|定义]]可得 $S_k=a_k+ib_k$ 且由于 $\exists M,\forall i\in\mathbb{N}^+\Rightarrow |S_i|<M$ 根据[[复变函数#^MK56|模]]定义可得 $a_k,b_k$ 均需有界

构造 $\vec{x}_k=(a_k,b_k)$ 由于 $a_k\in\mathbb{R}\land b_k\in\mathbb{R}$ 故根据[[Euclid空间上的极限与连续#^BW|Bolzano-Weierstrass定理]]
...
根据[[离散数学概论#^MK194|函数定义]]，由于 $f(z)$ 为函数，故对于 $z=x+iy$ 必有一个 $f(z)=a+ib$ 形成映射

故对于 $u(x,y),v(x,y)$ 若 $x+iy=z$ 则必有 $u(x,y)=a,v(x,y)=b$

同理由于 $f(z)$ 为函数，故当 $a_1+ib_1=z_1=f(z)\land a_2+ib_2=z_2=f(z)$ 时必有 $z_1=z_2$

根据[[复变函数#^MK60|复数相等]]可得 $a_1=a_2,\land b_1=b_2$ 故友 $a_1=u(x,y)\land a_2=u(x,y)\rightarrow a_1=a_2$
...
复函数 $f(x)$ 在点 $x_0\in\mathbb{C}$ 的[[复变函数#^MK55|领域]] $O(x_0,\rho)$ 上有定义，则其在点 $x_0$ 的导数定义为 $$\lim_{\Delta x\to 0}\frac{f(x_0+\Delta x)-f(x_0)}{\Delta x}$$
...
1. $u(x,y),v(x,y)$ 在点 $(x_0,y_0)$ 根据[[多元函数微分#^MK28|全微分]]定义，可微
...
当可导时，根据[[复变函数#^MK64|微分]]证明可得 $f(\Delta z+z_0)-f(z_0)=f'(z_0)\Delta z+o(\Delta z)$
...
即根据[[多元函数微分#^MK28|全微分]]定义， $u(x,y)$ 在点 $x_0,y_0$ 可微

且根据[[多元函数微分#^MK6|可微与偏导]]
...
定义 $z_0=x_0+iy_0$ 由于可微，则根据[[多元函数微分#^MK6|可微与偏导]]可得
...
若复函数 $f(z)$ 在 $z_0$ 点的复领域 $O(z_0,\rho)=\left\{z\big||z-z_0|<\rho\right\}$ 上[[复变函数#^MK63|可导]]，则称其在 $z_0$ 点解析
...
设 $D\subset\mathbb{C}$ 为复平面上的区域，若存在复平面上[[曲面，曲线积分与场论#^MK52|可求长线段]] $L\subset D$

定义 $x_0,...,x_n$ 为线段上的点，将线段划分成 $n-1$ 份，其中 $x_0,x_n$ 分别为线段的起始和终止点

规定 $x_0\to x_n$ 为线段的正方向。若[[复变函数#^MK61|复函数]] $f(x)$ 在 $L$ 上有定义且有界（ $\forall z\in L\rightarrow|f(z)|<\infty$ ）

取 $\zeta_i$ 为线段 $x_i,x_{i+1}$ 上的一点，则有[[定积分#^MK23|黎曼和]] $$\sum_{i=1}^{n-1}f(\zeta_i)(x_{i+1}-x_i)$$

定义 $s_i$ 为 $x_i,x_{i+1}$ 间的弧长， $\lambda=\max(\Delta s_i)$

则有[[复变函数#^MK57|极限]] $$\lim_{\lambda\to0}\sum_{i=1}^{n-1}f(\zeta_i)(x_{i+1}-x_i)$$ 若存在 $A$ 使得该极限对于任意的划分 $x_0,...,x_n$ 任意取点 $\zeta_i$
...
即转化为两个[[曲面，曲线积分与场论#^MK57|第二类曲线积分]]，其中 $u(x,y)+iv(x,y)=f(x+iy)$ 根据[[复变函数#^MK61|定理]]所得

#### 证明

##### 整理和式 ^MK71

根据[[复变函数#^MK61|定理]]可得 $f(x+iy)=u(x,y)+iv(x,y)$

根据[[复变函数#^MK59|复数]]定义 $x_i=a_i+ib_i$ 故有 $x_{i+1}-x_i=(a_{i+1}-a_i)+i(b_{i+1}-b_i)=\Delta a_i+i\Delta b_i$
...
根据[[曲面，曲线积分与场论#^MK59|推论]] $\lim_{\lambda\to0}\left\{\sum_{i=1}^{n-1}(u(\alpha_i,\beta_i),-v(\alpha_i,\beta_i))\cdot(\Delta a_i,\Delta b_i)\right\}$ 为微分形式的黎曼和
...
3. 由于曲线段光滑，故可求长。故对于 $t\in[0,1]$ 选择点 $z$ 满足起始点到 $z$ 的弧长度为 $\frac{t}{l}$ 其中 $l$ 为曲线段 $L$ 的长度 由于起始点到 $z$ 前后的点必然小于或大于 $\frac{t}{l}$ 故 $t$ 只对应 $z$ 一点，反之亦然，则一一对应。

4. 函数 $f(z)=f(x+iy)$ 在 $(x,y)\in L$ 上[[复变函数#^MK62|连续]]
...
若 $f(z)$ [[复变函数#^MK62|连续]]，则 $f(z)$ 在有界闭区间内可积

#### 证明

根据[[复变函数#^MK71|之前证明]]可得
...
根据[[曲面，曲线积分与场论#^MK58|连续函数可积]]可得 $\vec{F}(x,y)$ 可积
...
由于[[复变函数#^MK67|可积]]故实部虚部均可积。根据[[曲面，曲线积分与场论#^MK9|区间可加]]可得实部虚部均满足区间可加
...
根据[[复变函数#^MK68|三角不等式]]，[[复变函数#^MK69|相乘等价]]可得 $\left|\sum_{i=1}^nf(\xi_i)\Delta z_i\right|\leq\sum_{i=1}^n\left|f(\xi_i)\right||\Delta z_i|\leq M\sum_{i=1}^n|\Delta z_i|$

根据[[曲面，曲线积分与场论#^MK52|可求长线段]]定义 $\lim_{\lambda\to0}\sum_{i=1}^n|\Delta z_i|=l$ 又由于弧长大于等于弦长，故有 $\sum_{i=1}^n|\Delta z_i|\leq l$
...
根据[[复变函数#^MK67|积分定义]]以及极限四则运算可得 $$\int_Lzdz=\lim_{\lambda\to0}\sum_{i=1}^n\xi_i\Delta z_i=\lim_{\lambda\to0}\sum_{i=1}^n\xi_i(z_{i+1}-z_i)$$
...
根据[[复变函数#^MK61|定理]]函数可有等价映射 $u(x,y)+iv(x,y)$ 根据[[复变函数#^MK62|连续]]定义可得 $u(x,y),v(x,y)$ 亦连续

定义 $L$ 长度为 $l$ ，由于 $L$ 为有界闭集，故根据[[Euclid空间上的极限与连续#^MK40|康托定理]]可得 $u(x,y),v(x,y)$ 一致连续。

则可证 $f(z)$ [[复变函数#^MK66|一致连续]]，即 $\forall\epsilon>0,\exists\delta'>0,\forall|z'-z''|<\delta\Rightarrow|f(z')-f(z'')|<\frac{\epsilon}{2l}$

根据[[复变函数#^MK67|积分]]定义 $\forall\epsilon>0,\exists\delta''>0,\forall\lambda<\delta\Rightarrow\left|\sum_{i=1}^{n-1}f(\zeta_i)\Delta z_i-\int_L f(z)dz\right|<\frac{\epsilon}{2}$ 其中 $\lambda=\max(|\Delta z_i|)$
...
定义 $L$ 的参数方程为 $z(t)=\alpha+r\left(\cos(t)+i\sin(t)\right)$ 根据[[微分方程#^MK9|欧拉公式]]可得 $z(t)=\alpha+re^{it}$ 其中 $t\in[0,2\pi]$
...
若 $D\subset\mathbb{C}$ 是[[曲面，曲线积分与场论#^MK40|平面单连通区域]]，函数 $f(z)$ 在 $z\in D$ 上[[复变函数#^MK65|解析]]

若有[[复变函数#^MK81|简单]]闭合曲线 $L\subset D$ ，其包含的区域亦为单连通，则有 $$\int_Lf(z)dz=0$$
...
则根据[[复变函数#^MK74|区间可加]]有 $$\int_Lf(z)dz=\sum_{i=1}^4\int_{L_i}f(z)dz$$

根据[[复变函数#^MK68|三角不等式]]可得 $$\left|\int_Lf(z)dz\right|\leq\sum_{i=1}^4\left|\int_{L_i}f(z)dz\right|$$ 定义 $$M=\left|\int_Lf(z)dz\right|$$
...
利用以上（1）（2）条件，根据[[Euclid空间上的极限与连续#^KTDL|康托闭区域套定理]]可得

复平面上必存在一个点 $z_0\in D$ 满足 $z_0\in\cap_{i=1}^\infty\Delta_i$

###### 利用解析性质

由于[[复变函数#^MK65|解析]]，则必有在 $z_0$ 点[[复变函数#^MK63|可导]]，则有 $\forall\epsilon>0,\exists\delta>0,\forall|z-z_0|<\delta\Rightarrow\left|\frac{f(z)-f(z_0)}{z-z_0}-f'(z_0)\right|<\epsilon$

根据[[复变函数#^MK69|相乘等价]]两边同乘 $|z-z_0|$ 整理后得 $\left|f(z)-f(z_0)-f'(z_0)z+f'(z_0)z_0\right|<\epsilon|z-z_0|$

取 $n$ 足够大使得 $\forall z\in\Delta_n\Rightarrow |z-z_0|<\delta$ 由于 $|z-z_0|$ 显然小于等于三角形周长

故有 $\left|f(z)-f(z_0)-f'(z_0)z+f'(z_0)z_0\right|<\epsilon\frac{l}{2^n}$ 。则根据[[复变函数#^MK76|引理1]]以及[[复变函数#^MK77|线性]]可得：
...
根据[[复变函数#^MK70|长大不等式]]可得 $$\left|\int_{L^{(n)}}f(z)\right|\leq\frac{l}{2^n}\epsilon\frac{l}{2^n}$$ 则有 $$\frac{M}{4^n}\leq\left|\int_{L^{(n)}}f(z)\right|\leq\frac{l}{2^n}\epsilon\frac{l}{2^n}$$

最终 $M\leq \epsilon l^2$ 由于对于任意小的 $\epsilon>0$ 该式成立，故有 $$\left|\int_Lf(z)dz\right|=M=0$$ 则 $$\int_Lf(z)dz=0$$

##### 其他区域

对于其他区域，利用[[复变函数#^MK78|引理2]]有折线 $P$ 满足：
...
由于 $P$ 为有限边折线，则进一步根据[[复变函数#^MK79|多边形三角化]]可分割成有限个三角形。

故根据三角形区域证明及[[复变函数#^MK74|区间可加]]可得 $$\int_Pf(z)dz=\sum_{i=1}^n\int_{\Delta_i}f(z)dz=0$$

又有条件（2）得 $$\int_Lf(z)dz=0$$

### 柯西积分公式 ^MK88

若 $D\subset\mathbb{C}$ 是有界的[[曲面，曲线积分与场论#^MK40|平面单连通区域]]，函数 $f(z)$ 在 $D$ 上[[复变函数#^MK65|解析]]

若有[[复变函数#^MK81|简单]]闭合曲线 $L\subset D$ 且点 $z_0\not\in L$ 在 $L$ 包含的区域内，则有 $$f(z_0)=\frac{1}{2\pi i}\int_L\frac{f(z)}{z-z_0}dz$$
...
定义 $D^o_0$ 的半径为 $r'$ 。由于 $f(z)$ 解析，故根据[[复变函数#^MK83|导数与解析]]可得连续，

则根据[[复变函数#^MK62|函数连续]]与[[复变函数#^MK57|函数极限]]可得 $\forall\epsilon>0,\exists\rho>0,\forall|z-z_0|<\rho\Rightarrow\left|f(z)-f(z_0)\right|<\epsilon$
...
故根据[[复变函数#^MK84|柯西积分定理]]可得 $$\int_{L_1}\frac{f(z)}{z-z_0}dz=0\land\int_{L_2}\frac{f(z)}{z-z_0}dz=0$$

若规定顺时针为正方向，则 $L_1,L_2$ 在 $l$ 处重合且方向相反， $L$ 为顺时针 $L^o$ 为逆时针

根据[[复变函数#^MK74|区间可加]]可得 $$\int_{L_1}\frac{f(z)}{z-z_0}dz+\int_{L_2}\frac{f(z)}{z-z_0}dz=\int_{L}\frac{f(z)}{z-z_0}dz-\int_{L^o}\frac{f(z)}{z-z_0}dz=0$$

固有 $$\int_{L}\frac{f(z)}{z-z_0}dz=\int_{L^o}\frac{f(z)}{z-z_0}dz$$

##### 证明结论

根据[[复变函数#^MK85|引理3]]可得 $$\int_{L^o}\frac{1}{(z-z_0)^n}dz=2\pi i$$ 固有 $$f(z_0)\frac{1}{2\pi i}\int_{L^o}\frac{1}{(z-z_0)^n}dz=f(z_0)$$

即 $$f(z_0)=\frac{1}{2\pi i}\int_{L^o}\frac{f(z_0)}{(z-z_0)^n}dz$$ 另根据[[复变函数#^MK69|相乘等价]]以及[[复变函数#^MK86|分母]]可得
...
则根据[[复变函数#^MK69|相乘等价]]、[[复变函数#^MK77|线性]]以及[[复变函数#^MK70|长大不等式]]
...
则取 $\xi\in O(z_0,\delta)$ 时根据[[复变函数#^MK87|分式]]有 $\left|\frac{\xi-z_0}{z-z_0}\right|=\frac{|\xi-z_0|}{|z-z_0|}\leq\frac{|\xi-z_0|}{\rho}\leq\frac{\min\left(\frac{\rho}{2},1\right)}{\rho}\leq\frac{1}{2}$
...
根据[[复变函数#^MK88|柯西积分公式]]、[[复变函数#^MK77|线性]]有
...
由于 $L$ 为有界闭集且 $f(z)$ 解析，故 $f(z)$ 连续，根据[[Euclid空间上的极限与连续#^MK4|有界性定理]]可得有界

定义 $M\geq\left|f(z)\right|,z\in L$ 则根据[[复变函数#^MK87|分式]]、[[复变函数#^MK69|相乘]]、[[复变函数#^MK68|三角不等式]]以及[[复变函数#^MK89|之前证明]]有：
...
则定义 $L$ 长度 $l$ 根据[[复变函数#^MK70|长大不等式]]有
...
沿用[[复变函数#^MK89|之前证明]]中的定义，并定义 $$U=\sum_{i=2}^\infty\left(\frac{\xi-z_0}{z-z_0}\right)^i$$ 来简化表达式，则根据二项式定理有：
...
根据[[复变函数#^MK91|模的性质]]，以及[[复变函数#^MK90|之前证明]]有 $$U\leq|\xi-z_0|^2\frac{2}{\rho^2}$$ 且收敛，故作如下定义及推导
...
根据[[复变函数#^MK90|之前证明]]可推导 $$\frac{f^{(k)}(\xi)-f^{(k)}(z_0)}{\xi-z_0}=&\frac{k!(k+1)}{2\pi i}\int_L\frac{f(z)}{(z-z_0)^{k+2}}+\frac{k!}{2\pi i(\xi-z_0)}\int_L\frac{f(z)V}{(z-z_0)^{k+1}}dz$$

同样根据[[复变函数#^MK90|之前定义]] $M\geq|f(z)|$ 有界，故根据[[复变函数#^MK70|长大不等式]]结合上一步证明有 $$\left|\int_L\frac{f(z)V}{z-z_0}dz\right|\leq\frac{|\xi-z_0|^2CMl}{|\rho|^{k+1}}$$
...
若函数 $f(z)$ 在复平面 $z\in\mathbb{C}$ 上[[复变函数#^MK65|解析]]且有界，则 $f(z)$ 必定为常数函数，即 $f(z)=K$ 其中 $K\in\mathbb{C}$ 为常数

#### 证明

由于解析，故对于任意圆心为 $z_0$ 任意半径为 $R$ 的在复平面上的圆周曲线 $O$ 满足[[复变函数#^MK92|柯西积分推广公式]]

故有 $$f'(z_0)=\frac{1}{2\pi i}\int_O\frac{f(z)}{(z-z_0)^{2}}dz$$

同样由于解析，故 $f(z)$ 有界，定义 $\forall z\in\mathbb{C}\left(|f(z)|\leq M\right)$

故根据[[复变函数#^MK87|分式]]有 $$\left|\frac{f(z)}{(z-z_0)^{2}}\right|=\frac{|f(z)|}{|z-z_0|^{2}}\leq\frac{M}{R^2}$$

则根据[[复变函数#^MK70|长大不等式]]有 $$\left|\frac{1}{2\pi i}\int_O\frac{f(z)}{(z-z_0)^{2}}dz\right|\leq\frac{2\pi RM}{2\pi R^2}=\frac{M}{R}$$
