...
由于 $D_{n-1}$ 有界闭集，故根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]] $D_{n-1}$ 为紧集

根据[[Euclid空间上的极限与连续#^MK3|紧集的连续映射（连续函数）也是紧集]]得出 $\frac{\partial f_i(\vec{t})}{\partial t_j}$ 亦为紧集，同理为有界闭集，则存在 $m_{ij}\leq\frac{\partial f_i(\vec{t})}{\partial t_j}\leq M_{ij}$

定义 $M=\max\left\{|M_{ij}|,|m_{ij}|\big|1\leq i\leq n, 1\leq j\leq n-1\right\}$ ，定义 $\vec{t}_1,\vec{t}_2\in D_{n-1}$ 为区域内任意两点

若 $\vec{t}_1,\vec{t}_2$ 之间的连线 $l_{12}\in D_{n-1}$ 则根据[[多元函数微分#^MK10|中值定理]]有
...
由于 $D_{n-1}$ 为连通集，根据[[Euclid空间上的极限与连续#^MK32|连通集定义]]不存在 $\vec{t}_1,\vec{t}_2$ 没有通路的情况，所以
...
由于有界闭集，则根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]]得 $D$ 是紧集，又由于 $f(x,y)$ 连续，根据[[Euclid空间上的极限与连续#^MK40|康托定理]]得函数一直连续，
...
故的 $\forall \epsilon>0,\exists \delta,\forall \lambda<\delta\Rightarrow \sum_{i=1}^n\omega_i\sigma_i<\epsilon$ 即 $\lim_{\lambda\to0}\sum_{i=1}^n\omega_i\sigma_i=0$ 故根据[[定积分#^MK1|定积分可积充要条件2:]]得黎曼积分可积
...
若 $D$ 是有界闭集，且 $f(\vec{x})$ 在 $D$ 上连续，则可积。证明与[[重积分#^MK1|之前证明]]同理
...
根据[[Euclid空间上的极限与连续#^MK21|柯西收敛原理]] $\lim_{n\to\infty}\vec{f}_n(t)$ 收敛，定义 $\vec{f}(t)=\lim_{n\to\infty}\vec{f}_n(t)$

故对于任意给定 $t\in[0,1]$ 存在映射 $\vec{f}(t)$

### 证明面积

定义 $S=\left\{\vec{f}(t)\big|t\in[0,1]\right\}$ 由于 $[0,1]$ 有界闭集，根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]]得出是紧集。

又根据据[[Euclid空间上的极限与连续#^MK3|紧集的连续映射（连续函数）也是紧集]]得出 $S$ 也为紧集，同理 $S$ 有界闭集。

则根据[[Euclid空间上的极限与连续#^MK5|闭集定义]]故有 $S$ 包含聚点。
...
故对于 $\vec{a}$ 的任意领域均包含 $S$ 的点，故根据[[Euclid空间上的极限与连续#^MK32|定义]] $\vec{a}$ 为 $S$ 的聚点。又由于 $S$ 闭集，故 $\vec{a}\in S$
...
其中 $L$ 为达布大和， $l$ 为达布小和，证明同[[定积分#^MK20|定积分可积充要条件1]]
...
2. $D_i$ 包含 $P'$ 划分的曲面网，并被曲面网进一步划分 定义此类子点集为 $D_{i^*},i^*=1,2,...,k$ 定义 $\lambda_*=\max(\phi_{i^*})$ 定义内部被划分为 $n_{i^*}$ 个，每个为 $D_{i^*j},j=1,2,...,n_{i^*}$
...
2. $D'_i$ 被进一步分割 定义此类子点集为 $D'_{i^*},i^*=1,2,...,k$ 定义内部被划分为 $n_{i^*}$ 个，每个为 $D'_{i^*j},j=1,2,...,n_{i^*}$
...
根据[[定积分#^MK1|定积分可积充要条件2:]]得 $\forall P:\lim_{\lambda \to 0} \sum_{i=1}^n \omega_i v_i = 0$ 即 $\forall\epsilon>0,\exists \delta, \forall \lambda<\delta\Rightarrow \forall P: \sum_{i=1}^n \omega_iv_i<\epsilon$
...
根据[[定积分#^MK1|定积分可积充要条件2:]]得 $\lim_{\lambda\to0}\sum_{i=1}^n v_i \sigma_i=0$ 对于任意划分，其中 $\omega_i$ 为区域 $D_i$ 上函数的振幅
...
即有 $\forall \epsilon >0,\exists P\Rightarrow \sum_{i=1}^n \omega_i v^1_i<\epsilon$ 则根据[[重积分#^MK2|重积分可积充要条件3]]得 $\forall P:\lim_{\lambda\to0} \sum_{i=1}^n \omega_iv^1_i=0$ 同理可得 $\forall P:\lim_{\lambda\to0} \sum_{i=1}^n \omega_iv^2_i=0$

又根据[[定积分#^MK1|定积分可积充要条件2:]]得 $f(\vec{x})$ 在 $D_1,D_2$ 上分别可积
...
且根据[[定积分#^MK1|定积分可积充要条件2:]]得 $\lim_{\lambda\to0}\sum_{i=1}^n \omega_i v^1_i=0,\lim_{\lambda\to0}\sum_{i=1}^n \omega_i v^2_i=0$
...
则有 $\forall \epsilon>0,\exists P\Rightarrow \sum_{i=1}^n \omega_i v_i<\epsilon$ 故根据[[重积分#^MK2|重积分可积充要条件3]]得 $\forall P:\lim_{\lambda\to0} \sum_{i=1}^n \omega_iv_i=0$ 即 $f(\vec{x})$ 在 $D$ 上可积
...
由于 $f(\vec{x})-g(\vec{x})\geq0$ 根据重积分[[重积分#^MK3|线性]]性质 $\int_D f(\vec{x})dv-\int_D g(\vec{x})dv=\int_D\left\{f(\vec{x})-g(\vec{x})\right\}dv>0$
...
根据[[重积分#^MK3|线性]]有 $\int_D Mdv=M\int_D dv$ 根据[[重积分#^MK5|该性质]]有 $\int_D Mdv=M\int_D dv=MV$ 同理可证 $\int_D mdv=mv$

由于 $m\leq f(x)\leq M$ 根据[[重积分#^MK4|保序性]] $mV\leq\int_D f(\vec{x})dv\leq MV$
...
由于可积，根据[[重积分#^MK31|可积充要条件2]]可得 $\lim_{\lambda \to 0} \sum_{i=1}^n \omega^f_i v_i = 0$ 其中 $\omega^f_i$ 为函数 $f(\vec{x})$ 在区域 $\sigma_i$ 中的振幅
...
根据夹逼定理 $\lim_{\lambda \to 0} \sum_{i=1}^n \omega_i^g \Delta x_i=0$ 根据[[重积分#^MK1|定积分可积充要条件2]] $g(\vec{x})=|f(\vec{x})|$ 可积

##### 证明不等式

由于 $-|f(\vec{x})|\leq f(\vec{x})\leq|f(\vec{x})|$ 故根据[[重积分#^MK4|保序性]]可得 $\int_D-|f(\vec{x})|dv\leq \int_D f(\vec{x})dv\leq\int_D|f(\vec{x})|dv$

根据[[重积分#^MK3|线性]] $-\int_D|f(\vec{x})|dv\leq \int_D f(\vec{x})dv\leq\int_D|f(\vec{x})|dv\Rightarrow \left|\int_D f(\vec{x})dx\right|\leq\int_D|f(\vec{x})|dv$
...
根据[[重积分#^MK4|保序性]]有 $\int_D mg(\vec{x})dv\leq \int_D f(\vec{x})g(\vec{x})dv\leq \int_DMg(\vec{x})dv$

根据[[重积分#^MK3|线性]]有 $m\int_D g(\vec{x})dv\leq \int_D f(\vec{x})g(\vec{x})dv\leq M\int_Dg(\vec{x})dv$
...
故根[[Euclid空间上的极限与连续#^KTDL|康托闭区域套定理]]，必有唯一一点 $\vec{\xi}\in \cap_{n=1}^\infty D^n_i$ ，且有 $\eta\in[m^k_i,m^k_j],\forall k\in N^+$
...
根据定义 $m_{\vec{v}_j} \leq f(\vec{x})\leq M_{\vec{v}_j},\vec{x}\in D_{\vec{v}_j}$ 根据[[重积分#^MK4|保序性]]有不等式
...
根据[[重积分#^MK3|线性]]及[[重积分#^MK5|重积分性质]]又可对以上不等式做以下推论
...
又因为 $\int_D f(\vec{x})dv$ 可积，则根据[[定积分#^MK20|定积分可积充要条件1]]得
...
由于 $\overline{f}(\vec{x})$ 在 $D,\overline{D}_c$ 上均可积，根据[[重积分#^MK6|区域可加性]]则有 $\int_{\overline{D}}\overline{f}(\vec{x})dv$ 可积，且 $\int_{\overline{D}}\overline{f}(\vec{x})dv=\int_{\overline{D}_c}\overline{f}(\vec{x})dv+\int_{D}\overline{f}(\vec{x})dv=0+\int_{D}f(\vec{x})dv$

##### 证明 $\int_D f(\vec{x})dv=\int_{a_1}^{b_1}\left\{\int_{D^*}f(\vec{x})dv\right\}dx_1$

根据[[重积分#^MK7|矩形区域重积分]]可得 $\int_{\overline{D}}\overline{f}(\vec{x})dv=\int_a^b\left\{\int_{\overline{D}^*}\overline{f}(\vec{x})dv\right\}dx_1$ 当给定 $x_1$ 时
...
故根据[[重积分#^MK6|区域可加性]]有 $\int_{\overline{D}^*}\overline{f}(\vec{x})dv=\int_{D^*(x_1)}\overline{f}(\vec{x})dv+\int_{\overline{D}^*_c(x_1)}\ovelrline{f}(\vec{x})dv=\int_{D^*(x_1)}f(\vec{x})dv$
...
根据[[重积分#^MK7|矩形区域重积分]]可得 $\int_{\overline{D}}\overline{f}(\vec{x})dv=\int_{\overline{D}^*}\left\{\int_a^b\overline{f}(\vec{x})dx_1\right\}dv$

又根据定义 $\overline{D}^*=D^*\cup\overline{D}^*_c$ 故根据[[重积分#^MK6|区域可加性]] $\int_{\overline{D}^*}\left\{\int_{a}^{b}f(\vec{x})dx_1\right\}dv=\int_{D^*}\left\{\int_{a}^{b}f(\vec{x})dx_1\right\}dv+\int_{\overline{D}^*_c}\left\{\int_{a}^{b}f(\vec{x})dx_1\right\}dv$

两者均可积，且因为 $\forall x_1,\vec{x}^*\in \overline{D}^*_c\Rightarrow (x_1,\vec{x}^*)\in \overline{D}_c\Rightarrow f(x_1,\vec{x}^*)=0$ 故根据[[重积分#^MK3|线性]] $\int_{\overline{D}^*_c}\left\{\int_{a}^{b}f(\vec{x})dx_1\right\}dv=0$
...
故根据[[重积分#^MK6|区域可加性]]有 $\int_{D^*}\left\{\int_a^b\overline{f}(\vec{x})dx_1\right\}dv=\int_{D^*}\left\{\int_{a}^{a(\vec{x}^*)}\overline{f}(\vec{x})dx_1+\int_{a(\vec{x}^*)}^{b(\vec{x}^*)}\overline{f}(\vec{x})dx_1+\int_{b(\vec{x}^*)}^{b}\overline{f}(\vec{x})dx_1\right\}dv=\int_{D^*}\left\{\int_{a(\vec{x}^*)}^{b(\vec{x}^*)}f(\vec{x})dx_1\right\}dv$
...
根据重积分[[重积分#^MK3|线性]]性质 $\int_DR(x,y)d\sigma$ 亦可积且 $\int_DR(x,y)d\sigma=\int_DR_1(x,y)d\sigma+\int_DR_2(x,y)d\sigma=0$ 证毕
...
4. $D$ 为连通的有界闭集（不连通可根据[[重积分#^MK6|区域可加性]]化为 $n<\infty$ 个连通有界闭集）
...
### 引理1：本源映射长方体的体积有 $\left|\det\left(\frac{\partial \vec{T}(\vec{\xi})}{\partial\vec{x}}\right)\right|V(A)=V(T(A)),\vec{\xi}\in A$ ^MK13
...
由于雅克比行列式为 $\vec{T}(\vec{x})$ 偏导乘加所得，故偏导连续得出 $\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial \vec{x}}\right)$ 连续，又因为 $\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial \vec{x}}\right)\ne0$ 故 $\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial \vec{x}}\right)$ 不变号 ^MK10
...
由于 $1$ 为连续函数，故对于任意降维零边界积分均存在，则根据[[重积分#^MK8|重积分化累次积分]]有
...
定义 $\vec{x}_k\in A^k=[a_1,b_1]\times[a_2,b_2]\times...\times[a_{k-1},b_{k-1}]\times[a_{k+1},b_{k+1}]\times...\times[a_n,b_n]$ 故同理根据[[重积分#^MK8|重积分化累次积分]]的逆运算
...
2. 由于 $T_k(\vec{x})$ 有连续偏导，故根据[[多元函数微分#^MK5|可微条件]]得出可微，根据[[多元函数微分#^MK9|可微与连续]]推出 $T_k(\vec{x})$ 连续 ^MK12

3. $[T_k(b_k,\vec{x}_k)-T_k(a_k,\vec{x}_k)]$ 根据结论（2）也连续

4. 由于连续映射，且 $A^k$ 为有界集闭集， 根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]]及[[Euclid空间上的极限与连续#^MK3|紧集的连续映射（连续函数）也是紧集]]得出 $[T_k(\vec{x}_u)-T_k(\vec{x}_l)]$ 有界

定义  则根据[[重积分#^MK9|积分中值定理]]有
...
又因为结论（2）可得 $T_k(u,\vec{\xi})$ 对于 $u$ 在 $[a,b]$ 连续，根据条件（1）得出 $u$ 在 $(a,b)$ 可导，故根据[[微分方程#^MK5|拉格朗日中值定理]]可得
...
### 引理2：本源映射，结论成立，即 $\int_D f\left(\vec{T}(\vec{x})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial\vec{x}}\right)\right|dv=\int_{T(D)} f(\vec{t})dv$ ^MK19

#### 两个积分均可积

根据条件（3） $f(\vec{t})$ 连续，根据[[重积分#^MK11|重积分推论]]可得 $\int_{T(D)} f(\vec{t})dv$ 可积

根据[[重积分#^MK12|之前证明]] $\vec{T}(\vec{x})$ 连续，结合条件（3）得 $f\left(\vec{T}(\vec{x})\right)$ 连续，根据[[重积分#^MK10|之前证明]] $\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial \vec{x}}\right)$ 连续且不变号，故得 $f\left(\vec{T}(\vec{x})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial\vec{x}}\right)\right|$ 连续

同样根据[[重积分#^MK11|重积分推论]] $\int_D f\left(\vec{T}(\vec{x})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial\vec{x}}\right)\right|dv$ 可积
...
根据[[重积分#^MK13|引理1]]对于 $D_i\subset D$ 有 $\exists\xi_i\in D_i\Rightarrow \left|\det\left(\frac{\partial \vec{T}(\vec{\xi_i})}{\partial\vec{x}}\right)\right|V(D_i)=V(T(D_i))$ 则当 $\vec{\eta_i}=\vec{T}(\vec{\xi}_i)$ 由于映射一一对应，且 $\vec{\xi}_i\in D_i$ 故得 $\vec{T}(\vec{\xi}_i)\in T(D_i)$
...
若 $D_i$ 中包含 $D$ 的边界点，则根据[[Euclid空间上的极限与连续#^MK32|边界点]]的定义 $D_i$ 中包含不属于 $D$ 的点
...
2. $D_i$ 包含 $D$ 的边界点，则 $T(D_i)$ 包含 $T(D)$ 的边界点 ^MK14
...
由于[[重积分#^MK14|之前证明]] 当且仅当 $D_i$ 包含 $D$ 的边界点时 $T(D_i)$ 包含 $T(D)$ 的边界点

亦有 $\forall\epsilon>0,\exists\delta,\forall \lambda>\delta\Rightarrow\forall P:\sum_{i'=1}^{n'}V(T(D'_{i'}))<\epsilon$

#### 证明结论

根据条件（4）有 $D$ 为有界闭集，根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]] $D$ 紧集

根据[[Euclid空间上的极限与连续#^MK3|紧集的连续映射（连续函数）也是紧集]]得 $T(D)$ 紧集，同理 $T(D)$ 有界闭集

故 $f(\vec{t})$ 为有界闭集上的连续函数，根据[[Euclid空间上的极限与连续#^MK39|最大值定理]]存在 $m'\leq f(\vec{t})\leq M',\vec{t}\in T(D)$ 同理存在 $m''\leq f\left(\vec{T}(\vec{x})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial\vec{x}}\right)\right|\leq M''$

定义 $M=\max(M',M''),m=\min(m',m'')$ ，定义包含边界点的区域为 $D'_{i'}$ 不包含边界点的矩形为 $D''_{i''}$

根据[[重积分#^MK13|引理1]]，在 $(P^*,\xi)$ 下给定 $\vec{\eta_i}=\vec{T}(\vec{\xi}_i)$ 有 $f\left(\vec{T}(\vec{\xi_{i''}})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{\xi_{i''}})}{\partial\vec{x}}\right)\right|V(D_{i''})=f(\vec{\eta}_{i''})V(T(D_{i''}))$ 即重积分的[[重积分#^MK4|保序性]]得
...
### 引理3：复合映射的雅克比行列式等于两个映射雅克比行列式乘积 ^MK17

定义 $\vec{F}(\vec{x}_0)=\vec{T}\left(\vec{G}(\vec{x}_0)\right)\in R^n, \vec{x}_0\in R^n$ 满足[[多元函数微分#^MK7|链式法则]]的两个条件，即
...
### 引理4：对于任意给定 $\vec{x}_0\in D$ 的某领域内 $O(\vec{x}_0,\delta)$ 映射 $\vec{T}(\vec{x})$ 可以表示为 $n$ 个本源映射的复合 ^MK18
...
故根据[[多元函数微分#^MK15|逆映射定理]]在 $O(\vec{t}^1_0,\rho_1)$ 内有
...
由于[[重积分#^MK12|之前证明]] $\vec{t}^1=\vec{T}^1(\vec{x})$ 连续映射，故 $\lim_{\vec{x}\to\vec{x}_0}\vec{T}^1(\vec{x})=\vec{T}^1(\vec{x}_0)=\vec{t}^1_0$ 根据有 $\exists\delta_1,\forall\vec{x}\in O(\vec{x}_0,\delta_1)\Rightarrow |\vec{T}^1(\vec{x})-\vec{t}^1_0|<\rho_1$
...
同理根据[[多元函数微分#^MK15|逆映射定理]]在 $O(\vec{t}^2_0,\rho_2)$ 内
...
由于[[重积分#^MK12|之前证明]] $\vec{t}^2=\vec{T}^1(\vec{x})$ 为连续映射，故复合映射 $\vec{t}^2=\vec{T}^2\left(\vec{T}^1(\vec{x})}\right)$ 亦连续，
...
同理根据[[多元函数微分#^MK15|逆映射定理]]在 $O(\vec{t}^3_0,\rho_3)$ 内
...
定义 $D$ 的无限开覆盖 $\left\{U\big|O\left(\vec{x},\frac{\delta}{2}\right),\vec{x}\in D\right\}$ 其中 $\delta$ 是根据[[重积分#^MK18|引理4]]导出的在 $\vec{x}$ 点周围可将映射分解为 $n$ 个本源映射的领域半径

根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]] $D$ 为紧集，

根据[[Euclid空间上的极限与连续#^MK5|紧集]]给出的定义，任意无限开覆盖必有有限子覆盖。
...
故若有复合映射 $\vec{T}(\vec{t}^{i-2})=\vec{T}^i\left(\vec{T}^{i-1}(\vec{t}^{i-2})\right)$ 其中 $\vec{t}^{i-1}=\vec{T}^{i-1}(\vec{t}^{i-2})$ 则根据[[重积分#^MK17|引理3]]有 $G_i\left(\vec{T}^{i-1}(\vec{t}^{i-2})\right)G_{i-1}(\vec{t}^{i-2})=\det\left(\frac{\partial\vec{T}(\vec{t}^{i-2})}{\partial\vec{t}^{i-2}}\right)$

由于 $\vec{T}(\vec{x})=\vec{T}^n\circ\vec{T}^{n-1}\circ...\circ\vec{T}^1(\vec{x})$ 则根据[[重积分#^MK18|引理4]]中的复合映射规则 $\Pi_{i=1}^n G_i\left(\vec{T}^{i-1}\circ\vec{T}^{i-2}\circ...\circ\vec{T}^1(\vec{x})\right)=\det\left(\frac{\partial\vec{T}(\vec{x})}{\partial\vec{x}}\right)$

#### 定义划分下 $D_i$ 的重积分

则根据[[重积分#^MK19|引理2]]有如下推导
...
运用[[重积分#^MK24|球面坐标变换]]  $\begin{cases}x=r\sin(\phi)\cos(\theta)\\y=r\sin(\phi)\sin(\theta)\\z=r\cos(\phi)\end{cases}$ 其中 $\left\{T(D)\big| r\in(0,a),\phi\in(0,\pi),\theta\in(0,2\pi)\right\}$
...
由于 $f(x)>0,x\in[a,b]$ 且根据[[定积分#^L1|定理1:]]得 $\sum_{i=1}^n f(m_i)\Delta x_i\leq\underline{\int_a^b}f(x)dx\right$
...
则根据[[定积分#^MK7|区间可加]]，在某一给定划分满足条件的划分 $P: a=x_0<x_1<x_2<...<x_{n-1}<x_n=b,\lambda<\delta$ 成立
...
又因为 $f(x)$ 在 $[a,b]$ 上可积，根据[[定积分#^MK20|定积分可积充要条件1]] $\underline{\int_a^b}f(x)dx=\int_a^bf(x)dx$
...
则根据[[重积分#^MK25|引理1]]有 $\forall\epsilon>0,\forall n\in N^+,\exists g_n(x)>0\Rightarrow g_n(x)\leq f_n(x),\int_a^b f_n(x)<g_n(x)+\frac{\epsilon}{2^n}$ 其中 $g_n(x)$ 为连续梯形函数
...
故根据[[函数项级数#^MK9|Dini定理：]]函数 $h_n(x)$ 在定义域 $x\in[a,b]$ 上一致收敛

又根据[[函数项级数#^MK13|可积性：]] $\lim_{n\to\infty}\left\{\int_a^b h_n(x)dx\right\}=\int_a^b \left\{\lim_{n\to\infty}h_n(x)\right\}dx=0$
...
则根据[[定积分#^MK8|保序性]]及[[定积分#^MK5|线性]]有：
...
故根据之前推导和[[定积分#^MK8|保序性]]及[[定积分#^MK5|线性]]有：
...
1. $0\leq\phi_n(x)\leq M,\forall n\in N^+,\forall x\in[a,b]$ 即有界； ^MK27

2. $\lim_{n\to\infty}\phi_n(x)=0,\forall x\in[a,b]$

3. $f_n(x),f(x)$ 在 $[a,b]$ 上均可积，故根据[[定积分#^MK5|线性]] $\int_a^bf_n(x)-f(x)dx$ 也可积， 又根据[[定积分#^MK27|绝对可积]] $\int_a^b|f_n(x)-f(x)|dx$ 也可积
...
则根据[[重积分#^MK26|引理2]]有 $\lim_{n\to\infty}\int_a^b\psi_n(x)dx=0$

且由于 $\psi_n(x)\geq\phi_n(x)$ 和之前条件[[重积分#^MK27|（1）]]有 $0\leq\phi_n(x)\leq\psi_n(x)$
...
则根据[[定积分#^MK8|保序]]可得 $\int_a^b-|f_n(x)-f(x)|dx\leq \int_a^b f_n(x)-f(x)dx\leq\int_a^b|f_n(x)-f(x)|dx$
...
再根据[[定积分#^MK5|线性]]和极限四则运算有 $\lim_{n\to\infty}\int_a^b f_n(x)-f(x)dx=0=\lim_{n\to\infty}\int_a^b f_n(x)dx-\lim_{n\to\infty}\int_a^b f(x)dx$
...
3. 定义 $d(\Gamma)=inf\left\{|\vec{x}|\big|\vec{x}\in\Gamma\right\}$ 其中 $|\vec{x}|$ 为向量 $\vec{x}$ 到原点的距离，根据[[Euclid空间上的极限与连续#^MK25|定义距离]]计算
...
由于 $\int_D g(\vec{x})d\vec{x}=I$ 收敛，则根据[[重积分#^MK28|引理1]]存在一组满足相应条件的 $\{\Gamma_n\}$ 使得 $\lim_{n\to\infty}\left\{\int_{D_n}g(\vec{x})d\vec{x}\right\}=I$

由于 $0\leq f(\vec{x})\leq g(\vec{x})$ 根据[[重积分#^MK4|保序性]]有 $\int_{D_n}f(\vec{x})d\vec{x}\leq\int_{D_n}g(\vec{x})d\vec{x}$ 且 $I\geq\int_{D_n}g(\vec{x})d\vec{x}\geq\int_{D_n}f(\vec{x})d\vec{x}$

故 $\int_{D_n}f(\vec{x})d\vec{x}$ 有界且单调递增，根据极限定理 $\lim_{n\to\infty}\left\{\int_{D_n}f(\vec{x})d\vec{x}\right\}$ 存在

故根据[[重积分#^MK28|引理1]] $\int_D f(\vec{x})d\vec{x}$ 收敛。
...
由于 $0\leq f(\vec{x})\leq g(\vec{x})$ 根据[[重积分#^MK4|保序性]] $\int_{D_\Gamma}g(\vec{x})d\vec{x}>G>\int_{D_\Gamma}f(\vec{x})d\vec{x}>G$
...
2. 由于 $f(\vec{x})>0,x\in D$ 根据[[重积分#^MK4|保序性]]有 $\int_{D_\Gamma}f(\vec{x})dx>\int_{D_\Gamma}0dx=0$
...
根据[[重积分#^MK28|引理1]]，由于 $\int_D f(\vec{x})d\vec{x}$ 发散，则有 $\lim_{n\to\infty}\left\{\int_{D_n}f(\vec{x})d\vec{x}\right\}$ 发散

又由于 $0\leq\int_{D_n} f(\vec{x})d\vec{x}\leq M$ 故数列 $\left\{\int_{D_n}f(\vec{x})d\vec{x}\right\}$ 有界，根据[[Euclid空间上的极限与连续#^MK29|Bolzano-Weierstrass定理]]，有界数列必有收敛子列

故得一列 $\left\{\int_{D_k}f(\vec{x})d\vec{x}\right\}$ 当 $k\to\infty$ 时，数列收敛。同样根据[[重积分#^MK28|引理1]]得反常积分收敛，故推出矛盾
...
由于 $f(\vec{x}),g(\vec{x})$ 可积，故根据[[重积分#^MK31|可积充要条件]] $\forall P\Rightarrow \lim_{\lambda\to0}\sum_{i=1}^n \omega^f_i v_i=\lim_{\lambda\to0}\sum_{i=1}^n \omega^g_i v_i=0$

故根据极限夹逼定理可得 $\forall P\Rightarrow \lim_{\lambda\to0}\sum_{i=1}^n \omega_i v_i=0$ 同理根据[[重积分#^MK31|可积充要条件]] $H(\vec{x})$ 在 $D$ 上可积，证毕。
...
故根据[[重积分#^MK29|比较判别法]]得 $\int_Df^+(\vec{x})d\vec{x},\int_Df^-(\vec{x})d\vec{x}$ 均可积

故根据[[重积分#^MK3|线性]]性质有 $\int_Df(\vec{x})d\vec{x}=\int_Df^+(\vec{x})-f^-(\vec{x})d\vec{x}=\int_Df^+(\vec{x})d\vec{x}-\int_Df^-(\vec{x})d\vec{x}$
...
- **基础条件**
  ...
  由于 $f^+(\vec{x})\geq0$ ，则根据[[重积分#^MK30|引理2]]可得发散的定义 $\forall K>0,\exists\delta,\forall \left\{\Gamma\big|d(\Gamma)>\delta\right\}\Rightarrow \int_{D_\Gamma}f^+(\vec{x})d\vec{x}>K$

  由于反常积分 $\int_D f(\vec{x})d\vec{x}$ 可积，则对于有界区域 $D_i\subset D$ 来说 $\int_{D_i} f(\vec{x})d\vec{x}$ 可积是前提条件

  又由于 $f^+(\vec{x})=\max(f(\vec{x}),0)$ 故根据[[重积分#^MK32|引理3]] $\int_{D_i} f^+(\vec{x})d\vec{x}$ 也可积

  且根据[[重积分#^MK6|区域可加]]有 $\int_{D_i-D_j}f^+(\vec{x})d\vec{x},\ D_j\subset D_i$ 亦可积

- **下面开始取 $\{\Gamma_n\}$ ：**

  1. 取 $\Gamma_1$ 为任一满足条件的曲面

  2. 取 $\Gamma_{n+1}$ 使得 $\left\{\int_{D_{n+1}-D_n}f^+(\vec{x})d\vec{x}\right\}>n$ 由于 $\int_{D_i}f^+(\vec{x})d\vec{x}$ 可积，故利用反常积分不可积定义 对于 $K=\int_{D_i}f^+(\vec{x})d\vec{x}+n$ 存在一个 $\Gamma_{n+1}$ 使得 $\int_{D_\Gamma}f^+(\vec{x})d\vec{x}>K$ 且围成的体积 $D_n\subset D_{n+1}$
...
- **下面开始取 $\{\Gamma_n'\}$**
  ...
  1. 区域 $D_{n+1}-D_{n}$ 由零边界曲面划分为小区域 $\sigma_i$ ，定义 $m_i=\min\{f^+(\vec{x})\},\vec{x}\in \sigma_i$ 由于可积， 故[[重积分#^MK33|可积定理1]]可得 $\lim_{\lambda\to0}\sum_{i=0}^n m_iv_i=\int_{D_{n+1}-D_n}f^+(\vec{x})d\vec{x}$ 故只要划分足够细，则有 $\sum_{i=0}^n m_iv_i>\int_{D_{n+1}-D_n}f^+(\vec{x})d\vec{x}-1>n-1$ 取 $P_n=\cup \sigma_k,m_k>0$
...
根据[[重积分#^MK35|推论]]，当 $p>n$ 时 $\int_D\frac{1}{(\sum_{i=1}^n x_i^2)^{\frac{p}{2}}}d\vec{x}$ 收敛，则 $\int_D\frac{M}{(\sum_{i=1}^n x_i^2)^{\frac{p}{2}}}d\vec{x}$ 收敛

又由于 $|f(\vec{x})|\geq 0$ 根据[[重积分#^MK29|比较判别法]] $\int_D|f(\vec{x})|d\vec{x}$ 收敛，又根据[[重积分#^MK36|引理4]]得 $\int_Df(\vec{x})d\vec{x}$ 收敛。
...
根据有界区域重积分[[重积分#^MK3|线性]] $\int_{D_\Gamma} \left\{Af(\vec{x})+Bg(\vec{x})\right\}d\vec{x}=A\int_{D_\Gamma}f(\vec{x})d\vec{x}+B\int_{D_\Gamma}g(\vec{x})d\vec{x}$
...
定义 $K>a_i,F(x_i)=\int_{a_{i+1}}^\infty...\int_{a_n}^\infty f(\vec{x})dx_n...dx_{i+1},G(x_i)=\int_{a_{i+1}}^\infty...\int_{a_n}^\infty g(\vec{x})dx_n...dx_{i+1}$ ，根据[[定积分#^MK5|线性]]及给定条件
...
- **证明**
  ...
  - **利用积分极限等价定理**
    ...
    1. $\forall u\in N^+$ 定积分 $\int_{a_i}^{A_i^u} \psi_n(x_i)dx_i$ 可积（根据[[反常积分#^MK2|边界无穷积分]]及反常累次积分可积条件）
    ...
    故根据[[重积分#^MK37|积分极限等价定理推论]]有 $\lim_{u\to\infty}\int_{a_i}^{A^k_i}\psi_u(x_i)dx_i=\int_{a_i}^{A^k_i}\lim_{u\to\infty}\psi_u(x_i)dx_i=\int_{a_i}^{A^k_i}\psi(x_i)dx_i$
...
故根据[[重积分#^MK28|引理1]]若 $\lim_{k\to\infty}\int_{D_k}f(\vec{x})d\vec{x}$ 收敛，则有 $\lim_{k\to\infty}\int_{D_k}f(\vec{x})d\vec{x}=\int_Df(\vec{x})d\vec{x}$

根据[[重积分#^MK8|重积分计算]]有 $\int_{a_1}^{B^k_1}\int_{a_2}^{B^k_2}...\int_{a_n}^{B^k_n} f(\vec{x})dx_ndx_{n-1}...dx_1=\int_{D_k}f(\vec{x})d\vec{x}$
...
故根据[[重积分#^MK39|之前证明]]可得 $\int_D f^+(\vec{x})d\vec{x}=\int_{a_1}^\infty\int_{a_2}^\infty...\int_{a_n}^\infty f^+(\vec{x})dx_ndx_{n-1}...dx_1=\frac{I_1+I_2}{2}$
...
2. 累次积分 $\int_0^\infty\int_0^\infty e^{-(x^2+y^2)}dydx=\int_0^\infty e^{-x^2}dx\int_0^\infty e^{-y^2}dy=\left(\int_0^\infty e^{-x^2}dx\right)^2$ 由于 $0<e^{-x^2}<e^{-x},x\in[1,\infty]$ 且根据[[定积分#^MK28|牛顿莱布尼茨公式]] $\int_1^\infty e^{-x} dx=-\frac{1}{e^\infty}-\left(-\frac{1}{e}\right)<\infty$ 故根据保序性 $\int_1^\infty e^{-x^2}dx<\int_1^\infty e^{-x}dx<\frac{1}{e}$ 有界，且函数 $f(A)=\int_0^A e^{-x^2}dx,A>0$ 为单调递增函数 故 $\int_1^\infty e^{-x^2}dx$ 收敛，又由于 $\int_0^1 e^{-x^2}dx$ 收敛，故根据[[定积分#^MK7|区间可加]]  $\int_0^\infty e^{-x^2}dx$ 收敛

则根据[[重积分#^MK43|重积分计算]]得 $\int_D e^{-(x^2+y^2)}d\sigma=\int_0^\infty\int_0^\infty e^{-(x^2+y^2)}dydx=\int_0^\infty e^{-x^2}dx\int_0^\infty e^{-y^2}dy=\left(\int_0^\infty e^{-x^2}dx\right)^2$
...
显然其敛散性取决于 $\int_a^R\frac{1}{r^{p-n+1}}dr$ 根据[[定积分#^MK28|牛顿莱布尼茨公式]]
...
根据[[重积分#^MK45|定义]] $\int_{D}f(\vec{x})dx=\lim_{\rho(\Gamma)\to0}\left\{\int_{D_\Gamma}f(\vec{x})d\vec{x}\right\}$
