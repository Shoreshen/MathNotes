...
根据[[定积分#^MK26|乘积可积]]可得 $f(x)^2,g(x)^2,f(x)g(x)$ 均在 $[a,b]$ 上可积

根据[[定积分#^MK25|定积分定义]]，对于任意可积函数在任意划分与取点下，只要满足 $\lambda\to0$ 黎曼和均收敛于定积分，故有： $\int_a^bf(x)^2dx=\lim_{n\to\infty}\sum_{i=1}^nf(\xi_i)^2\frac{b-a}{n},\xi\in[x_i,x_{i+1}]$ $\int_a^bg(x)^2dx=\lim_{n\to\infty}\sum_{i=1}^ng(\xi_i)^2\frac{b-a}{n},\xi\in[x_i,x_{i+1}]$ $\int_a^bf(x)g(x)dx=\lim_{n\to\infty}\sum_{i=1}^nf(\xi_i)g(\xi_i)\frac{b-a}{n},\xi\in[x_i,x_{i+1}]$

由于根据[[傅里叶级数#^MK7|形式1]]有 $\left(\sum_{i=1}^n f(\xi_i)g(\xi_i)\right)^2\leq \sum_{i=1}^nf(\xi_i)^2\sum_{i=1}^ng(\xi_i)^2$
...
则根据[[定积分#^MK24|三角函数正交性:]]及[[函数项级数#^MK13|可积性：]]有
...
根据积分[[定积分#^MK7|区间可加]] $\int_0^\infty\frac{\sin(x)}{x}dx=\int_0^{\frac{\pi}{2}}\frac{\sin(x)}{x}dx+\int_{\frac{\pi}{2}}^\infty\frac{\sin(x)}{x}dx$

分别为区间无限及无界反常积分由于 $\int_a^b\sin(x)dx$ 有界，对于任意 $0\leq a<b<\infty$ 且 $\lim_{x\to\infty}\frac{1}{x}=0$ 且 $\frac{1}{x}$ 单调

故根据[[反常积分#^MK11|Dirichlet]] $\int_{\frac{\pi}{2}}^\infty\frac{\sin(x)}{x}dx$ 收敛

又因为 $\frac{\sin(x)}{x}<1$ 且有 $\lim_{a\to0}\int_a^\frac{\pi}{2}1dx=\frac{\pi}{2}$ 收敛，故根据[[反常积分#^MK10|非负反常积分判别]] $\int_0^{\frac{\pi}{2}}\frac{\sin(x)}{x}dx$ 亦收敛
...
且有 $\sin(x)=-\sin(x+\pi)$ 故 $\frac{\sin(x)}{x}+\frac{\sin(x+\pi)}{x+\pi}>0$ 根据[[定积分#^MK8|保序]]可得 $\int_{2i\pi}^{(2i+1)\pi}\left(\frac{\sin(x)}{x}+\frac{\sin(x+\pi)}{x+\pi}\right)dx\geq0$
...
由于可积，则根据[[定积分#^MK1|定积分可积充要条件2:]]有 $\forall\epsilon>0,\exists\delta,\forall\lambda<\delta,\forall P\Rightarrow\left|\sum_{i=1}^\infty\omega_i\Delta x_i\right|<\frac{\epsilon}{2}$

且根据[[定积分#^MK26|乘积可积]]得 $\phi(x)\sin(px)$ 亦可积
...
则根据[[定积分#^MK5|线性]]，[[定积分#^MK27|绝对可积]]，[[定积分#^MK26|乘积可积]]以及[[定积分#^MK7|区间可加]]可得
...
定义 $b$ 为奇点，则根据条件有 $\int_a^b|\phi(x)|dx$ 存在，根据[[反常积分#^MK8|定义：]] $\forall\epsilon>0,\exists\delta>0,\forall\eta<\delta\Rightarrow\int_{b-\eta}^b|\phi(x)|dx<\frac{\epsilon}{2}$

且根据[[傅里叶级数#^MK1|可积条件下证明]]有 $\lim_{p\to\infty}\int_a^{b-\eta}\phi(x)\sin(px)dx=0$ 故 $\forall\epsilon>0,\eixsts P,\forall p>P\Rightarrow\left|\int_a^{b-\eta}\phi(x)\sin(px)dx\right|<\frac{\epsilon}{2}$

故根据[[定积分#^MK27|绝对可积]]以及根据[[反常积分#^MK9|反常积分绝对收敛:]]得
...
故根据[[定积分#^MK3|推论1]]得出 $g(u)$ 在 $[0,\pi]$ 上可积

故根据[[定积分#^MK26|乘积可积]]有 $g(u)\phi(u)$ 在 $u\in[0,\delta]$ 上亦可积或绝对可积 则有
...
根据[[傅里叶级数#^MK2|黎曼引理]]得 $\lim_{m\to\infty}\left\{\int_{0}^{\delta}\left\{\phi(u)g(u)\sin\left(\frac{2m+1}{2}u\right)\right\}du\right\}=0$ 证毕
...
根据[[傅里叶级数#^MK1|引理1]]存在一个常数 $K$ 有对于任意 $0\leq a<b\leq\infty$ 均有 $\left|\int_a^b\frac{\sin(x)}{x}dx\right|<K$ 其中 $K$ 为常数
...
由于 $\phi(u)$ 单调，则 $\phi(u)-\phi(0^+)$ 亦单调，根据[[定积分#^MK21|推论2]]得出可积

由于 $\frac{1}{u}$ 在 $[\eta,\delta]$ 上亦可积，则根据[[定积分#^MK26|乘积可积]] $\frac{\phi(u)-\phi(0^+)}{u}$ 可积

故根据[[傅里叶级数#^MK2|黎曼引理]]得 $\lim_{p\to\infty}\int_\eta^\delta\frac{\phi(u)-\phi(0^+)}{u}\sin(pu)du=0$ 即 $\forall\epsilon>0\exists P,\forall p>P\Rightarrow\left|\int_\eta^\delta\frac{\phi(u)-\phi(0^+)}{u}\sin(pu)du\right|<\frac{\epsilon}{K+1}$
...
故根据[[定积分#^MK30|推论]]有在 $(0,\eta]$ 上的反常积分等于在 $[0,\eta]$ 上的常义积分
...
由于 $\phi(u)-\phi(0^+)$ 单调 $\frac{\sin(pu)}{u}$ 在 $[0,\eta]$ 上连续，则根据[[定积分#^MK3|推论1]]得必可积

故根据[[反常积分#^MK1|第二积分中值定理：]]得
...
根据[[傅里叶级数#^MK1|引理1]]存在一个常数 $K$ 有对于任意 $0\leq a<b\leq\infty$ （ $a=0$ 或 $b=\infty$ 时为反常积分 ）
...
故根据[[定积分#^MK30|推论]]有在 $(0,\pi]$ 上的反常积分等于在 $[0,\pi]$ 上的常义积分
...
定义 $\sigma(x)$ 为 $S_m(x)$ 的收敛值，则有 $\lim_{m\to\infty}\{S_m(x)-\sigma(x)\}=0$ 根据[[定积分#^MK5|线性]]有
...
则有当 $u\in[0,\delta]$ 时 $\phi(u),\phi'(u)$ 单调，故根据[[定积分#^MK7|区间可加]]和极限四则运算拆分积分
...
同理 $\frac{1}{\sin\left(\frac{u}{2}\right)}$ 可积，故根据[[定积分#^MK26|乘积可积]] $\frac{\phi(u,x)}{\sin\left(\frac{u}{2}\right)}$ 在 $u\in[a,\pi],0<a<\pi$ 可积或绝对可积

故满足[[傅里叶级数#^MK2|黎曼引理]]条件，则有 $\lim_{m\to\infty}\int_{\delta}^{\pi}\left\{\frac{\phi(u,x)}{2\sin\left(\frac{u}{2}\right)}\sin\left(\frac{2m+1}{2}u\right)\right\}du=0$

根据[[傅里叶级数#^MK4|黎曼引理推论]]有 $\lim_{m\to\infty}\int_{0}^{\delta}\left\{\frac{f(x+u)-f(x^+)}{2\sin\left(\frac{u}{2}\right)}\sin\left(\frac{2m+1}{2}u\right)\right\}du=\lim_{m\to\infty}\int_{0}^{\delta}\left\{\frac{f(x+u)-f(x^+)}{u}\sin\left(\frac{2m+1}{2}u\right)\right\}du$
...
且根据条件 $\psi(u)$ 在 $u\in[0,\delta]$ 上单调，故满足[[傅里叶级数#^MK5|Dirichlet引理]]，
...
由于 $\left|\frac{f(x+u)-f(x^{+})}{u}\right|>0$ 故根据[[反常积分#^MK10|非负反常积分判别]] $\left|\frac{f(x+u)-f(x^{ +})}{u}\right|$ 亦可积

故有 $\frac{f(x+u)-f(x^{+})}{u}$ 在 $u\in(0,\delta]$ 上反常积分绝对可积，满足[[傅里叶级数#^MK2|黎曼引理]]条件
...
由于 $f(x)$ 可积或绝对可积，根据[[傅里叶级数#^MK2|黎曼引理]] $\lim_{i\to\infty}a_n=\lim_{i\to\infty}\frac{1}{\pi}\int_{-\pi}^\pi f(x)\cos(ix)dx=0$
...
故 $F(x)$ 为周期函数，周期为 $2\pi$ 。且则根据[[定积分#^L1|定理1:]]可得 $F(x)$ 连续
...
即左右导数均存在，且等于 $f(x)$ 在 $x$ 点的左右极限，故根据[[傅里叶级数#^MK6|Dini-Lipschitz判别]]，结合连续得 $F(x)$ 的傅里叶级数收敛于 $F(x)$
...
由于 $F(c)=0$ 故 $\frac{A_0}{2}=-\sum_{i=1}^\infty\left(-\frac{b_i}{i}\cos(ic)+\frac{a_i}{i}\sin(ic)\right)$ 故根据[[定积分#^MK5|线性]]有
...
假设 $S$ 为定义了內积运算 $(\cdot,\cdot)$ 的线性空间，且內积性质有： ^MK8
...
定义 $\vec{y}=\sum_{i=1}^m d_i\vec{\phi}_i$ 且 $\vec{x}-\vec{y}\perp T$ 则有 $(\vec{x}-\vec{y},\vec{\phi}_i)=0$ 根据[[傅里叶级数#^MK8|內积性质]]
...
根据[[傅里叶级数#^MK9|之前证明]]得当 $d_i=\frac{(\vec{\phi}_i,\vec{x})}{(\vec{\phi}_i,\vec{\phi}_i)}$ 时为最佳平方逼近

###### 必要性

定义 $\vec{y}=\sum_{i=1}^m d_i\vec{\phi}_i$ 为最佳平方逼近，则根据[[傅里叶级数#^MK9|之前证明]]得 $d_i=\frac{(\vec{\phi}_i,\vec{x})}{(\vec{\phi}_i,\vec{\phi}_i)}$ 则有
