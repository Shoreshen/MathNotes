...
对于数列 $\{x_n\}$ 若有极限点 $\xi$ 则存在[[Euclid空间上的极限与连续#^MK16|子列]] $\{y_n\}$ 满足 $\lim_{n\to\infty}y_n=\xi$

## 上下极限 ^MK2

定义 $E=\{\xi\}$ 为所有 $\{x_n\}$ [[数项级数#^MK1|极限点]]的集合，则定义：
...
根据[[Euclid空间上的极限与连续#^MK29|Bolzano-Weierstrass定理]]必有收敛子列，假设子列 $\{S_{k_1},S_{k_2},...\}$ 收敛与 $A$
...
#### 推论1：$\sum_{n=1}^\infty |x_n|$ 收敛，则 $\sum_{n=1}^\infty x_n$ 也收敛，根据三角不等式证明如下： ^MK14
...
根据[[数项级数#^MK3|柯西收敛原理]]可得 $\forall\frac{\epsilon}{2}>0,\exists N,\forall m>n>N\Rightarrow \left|\sum_{i=n}^m x_i\right|<\frac{\epsilon}{2}$
...
根据[[数项级数#^MK3|柯西收敛原理]]可得 $\forall \epsilon>0,\exists N,\forall m>n>N\Rightarrow \left|\sum_{i=n}^m x_i\right|<\epsilon$
...
### 推论1：若 $p_n=1+a_n$ 且 $a_n$ 不变号，则 $\prod_{i=1}^\infty p_i$ 与 $\sum_{i=N'+1}^\infty a_i$ 同时敛散，证明： ^MARK1
...
\end{cases}
$$
\ \&\ g(0)=0,g\left(\frac{\pi}{2}\right)=0\\ &\therefore \sin(x) \geq \frac{2}{\pi}x>0,\ \ x\in\left[0,\frac{\pi}{2}\right]\\ &\therefore \sin^2\left(\frac{k\pi}{2n+1}\right) \geq \left(\frac{k\pi}{2n+1}\right)^2\\ &\therefore 1 \geq \prod_{k=m+1}^n\left( 1 - \frac{\sin^2(\frac{x}{2n+1})}{\sin^2(\frac{k\pi}{2n+1})}} \right) =\frac{\sin(x)}{(2n+1)\sin(\frac{x}{2n+1}) \prod_{k=1}^m\left( 1 - \frac{\sin^2(\frac{x}{2n+1})}{\sin^2(\frac{k\pi}{2n+1})}} \right)} \geq \prod_{k=m+1}^n\left( 1 - \frac{x^2}{k^2\pi^2} \right) \geq \prod_{k=m+1}^\infty \left( 1 - \frac{x^2}{k^2\pi^2} \right)\\ \end{aligned}
