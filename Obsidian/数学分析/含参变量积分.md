...
# Cauchy-Bunyakovsky-Schwarz Inequality/Definite Integrals ^MK27

$\left(\int_a^bf(t)g(t)dt\right)^2\leq\int_a^b(f(t))^2dt\int_a^b(g(t))^2dt$

## 证明

定义 $x\in R$ 则有 $0\leq \left(xf(t)+g(t)\right)^2$ 故根据[[定积分#^MK8|保序]]有 $0=\int_a^b0dt\leq\int_a^b\left(xf(t)+g(t)\right)^2dt$
...
由于[[含参变量积分#^MK1|含参变量常义积分定义]]，被积函数 $f(\vec{x},\vec{y})$ 在区域 $D$ 上连续，又由于 $D$ 为有界闭区域，

故根据[[Euclid空间上的极限与连续#^MK38|Heine-Borel定理]]得 $D$ 为紧集

又根据[[Euclid空间上的极限与连续#^MK40|康托定理]]得 $f(\vec{x},\vec{y})$ 在区域 $D$ 上一致连续，定义积分区域 $S$ 的面积为 $V$

则根据[[Euclid空间上的极限与连续#^MK32|补充多元函数一致连续定义]]
...
根据中[[重积分#^MK3|线性]]，[[重积分#^MK5|当 $f(\vec{x})=1$ 时]]，[[重积分#^MK4|保序性]]以及[[重积分#^MK49|绝对可积性]]得
...
由于[[含参变量积分#^MK1|含参变量常义积分定义]]，被积函数 $f(\vec{x},\vec{y})$ 在区域 $D$ 上连续，故根据[[重积分#^MK11|推论]]任意维度下子重积分均可积

故根据[[重积分#^MK8|不规则区域上重积分]]重积分可化为累次积分。
...
定义 $\vec{y}=(\vec{y}',y_k)$ ，故根据导数定义，[[重积分#^MK3|线性]]以及[[含参变量积分#^MK2|连续性定义推论]]
...
根据[[含参变量积分#^MK3|积分函数求导]] $\frac{\partial I(u,v,w)}{\partial w}=\int_u^vf_w(x,w)dx$ 由于 $f_y(x,y)$ 连续，又根据[[含参变量积分#^MK4|连续性定理]]可得 $\int_u^vf_w(x,w)dx$ 连续

定义 $\frac{\partial G(x,y)}{\partial x}=f(x,y)$ 即 $G(x,y)$ 是 $f(x,y)$ 关于 $x$ 的一个原函数，且根据条件 $f(x,y)$ 连续。

故根据[[定积分#^MK28|牛顿莱布尼茨公式]] $I(u,v,w)=\int_u^vf(x,w)dx=G(u,w)-G(v,w)$
...
由于 $I(u,v,w)$ 的偏导数均连续，故根据[[多元函数微分#^MK5|可微条件]]函数在定义域上可微
...
故根据[[多元函数微分#^MK7|链式法则]] $\frac{\partial I(u,v,w)}{\partial y}=\frac{\partial I(u,v,w)}{\partial u}\frac{\partial u}{\partial y}+\frac{\partial I(u,v,w)}{\partial v}\frac{\partial v}{\partial y}+\frac{\partial I(u,v,w)}{\partial w}\frac{\partial w}{\partial y}$
...
根据[[反常积分#^MK7|必要]]可得在以上条件下 $\forall A>A_0,\forall y\in[c,d]\Rightarrow\left|\int_a^Af(x,y)dx-K(y)\right|<2\epsilon$

则有 $\forall\epsilon>0,\exists A_0>a,\forall A>A_0,\forall y\in[c,d]\Rightarrow \left|\int_a^Af(x,y)dx-K(y)\right|<\epsilon$ 根据[[反常积分#^MK5|定义]]可得 $K(y)=\int_a^\infty f(x,y)dx=I(y)$
...
由于 $\int_a^\infty F(x)dx$ 收敛，根据[[反常积分#^MK6|柯西收敛原理：]]可得 $\forall\epsilon>0,\exists A_0,\forall A''>A'>A_0\Rightarrow\left|\int_{A'}^{A''}F(x)dx\right|<\epsilon$

由于 $0\leq|f(x,y)|\leq F(x)$ 故有 $\forall\epsilon>0,\exists A_0,\forall A''>A'>A_0\Rightarrow\int_{A'}^{A''}F(x)dx<\epsilon$

根据[[定积分#^MK8|保序]]及[[定积分#^MK27|绝对可积]]可得 $\left|\int_{A'}^{A''}f(x,y)dx\right|\leq\int_{A'}^{A''}|f(x,y)|dx\leq\int_{A'}^{A''}F(x)dx$
...
根据[[含参变量积分#^MK5|一致收敛定义]]，条件（1）： $\forall\epsilon>0,\exists A_0,\forall A''>A'_A_0,\forall y\in[c,d]\Rightarrow \left|\int_{A'}^{A''}f(x,y)dx\right|<\frac{\epsilon}{2L}$

根据[[反常积分#^MK1|第二积分中值定理：]]及条件（2）可得 $\forall y\in[c,d]$ 均有：
...
根据[[反常积分#^MK1|第二积分中值定理：]]及条件（1）可得 $\forall y\in[c,d]$ 均有：
...
根据[[含参变量积分#^MK4|连续性定理]]可得 $u(y)=\int_{a_n}^{a_{n+1}}f(x,y)dx$ 连续，根据[[含参变量积分#^MK6|引理]]及条件（2）可得级数 $S(y)$ 一致连续

根据[[函数项级数#^MK12|连续性：]]可得 $S(y)$ 连续，即 $I(y)$ 连续
...
根据[[函数项级数#^MK12|连续性：]]可得 $\int_c^d S(y)dy = \sum_{i=1}^\infty \int_c^d dy\int_{a_i}^{a_{i+1}}f(x,y)dx$

根据[[含参变量积分#^MK7|常义积分次序交换]]可得 $\int_c^d dy\int_{a_i}^{a_{i+1}}f(x,y)dx = \int_{a_i}^{a_{i+1}}\int_c^df(x,y)dydx$
...
定义 $I_y(y) = \int_a^\infty f_y(x,y)dx$ 根据条件（2），（3），[[含参变量积分#^MK8|可积性]]及[[定积分#^MK28|牛顿莱布尼茨公式]]
...
故根据[[含参变量积分#^MK9|Dirichlet判别]] $I(\alpha)$ 关于 $\alpha\in[0,1]$ 一致收敛
...
故根据[[含参变量积分#^MK9|Dirichlet判别]]可推出 $f_y(x,\alpha)$ 关于 $\alpha\in[\alpha_0,\infty)$ 一致收敛，其中 $\alpha_0$ 为 $(0,1]$ 间任意值。

故根据[[含参变量积分#^MK10|可导性]]能推出
...
根据[[反常积分#^MK7|必要]]可得在以上条件下 $\forall \eta<\eta_0,\forall y\in[c,d]\Rightarrow\left|\int_a^{b-\eta} f(x,y)dx-K(y)\right|<2\epsilon$

则有 $\forall\epsilon>0,\exists \eta_0>0,\forall \eta<\eta_0,\forall y\in[c,d]\Rightarrow \left|\int_a^{b-\eta}f(x,y)dx-K(y)\right|<\epsilon$ 根据[[反常积分#^MK8|定义：]]可得 $K(y)=\int_a^b f(x,y)dx=I(y)$
...
由于 $\int_a^b F(x)dx$ 收敛，根据[[反常积分#^MK6|柯西收敛原理：]]可得 $\forall\epsilon>0,\exists \eta_0,\forall \eta''<\eta'<\eta_0\Rightarrow\left|\int_{b-\eta'}^{b-\eta''}F(x)dx\right|<\epsilon$

由于 $0\leq|f(x,y)|\leq F(x)$ 故有 $\forall\epsilon>0,\exists \eta_0,\forall\eta''<\eta'<\eta_0\Rightarrow\int_{b-\eta'}^{b-\eta''}F(x)dx<\epsilon$

根据[[定积分#^MK8|保序]]及[[定积分#^MK27|绝对可积]]可得 $\left|\int_{b-\eta'}^{b-\eta''}f(x,y)dx\right|\leq\int_{b-\eta'}^{b-\eta''}|f(x,y)|dx\leq\int_{b-\eta'}^{b-\eta''}F(x)dx$
...
则根据[[反常积分#^MK12|比较判别法]]当且仅当 $p>0$ 时 $\int_0^{\frac{1}{2}}x^{p-1}(1-x)^{q-1}dx$ 收敛
...
由于[[含参变量积分#^MK11|敛散性证明]]中已证 $\int_0^1x^{p_0-1}(1-x)^{q_0-1}dx$ 在 $p_0>0,q_0>0$ 上收敛

故根据[[含参变量积分#^MK12|Weierstrass判别]] $\beta(p,q)=\int_0^1x^{p-1}(1-x)^{q-1}dx$ 在 $[p_0,\infty)\times[q_0,\infty)$ 上一致收敛
...
故根据[[定积分#^MK7|区间可加]]分割积分区域 $\Gamma(s)=\int_0^1 x^{s-1}e^{-x}dx+\int_1^\infty x^{s-1}e^{-x}dx$

##### 第一部分

由于当 $x\in(0,1]$ 时 $x^{s-1}e^{-x}>0,x^{s-1}>0$ 且有 $\left|x^{s-1}e^{-x}\right|\leq x^{s-1}$

根据[[含参变量积分#^MK11|之前证明]]可得积分 $\int_0^{1}x^{p-1}dx$ 当 $0<p<1$ 时为反常积分，积分收敛。当 $p\geq 1$ 时为常义积分，可积。

故根据[[反常积分#^MK12|比较判别法]]当 $s>0$ 时 $\int_0^1 x^{s-1}e^{-x}dx$ 存在
...
由于当 $x\in[1,s+2]$ 时 $x^{s-1}e^{-x}$ 连续，故根据[[定积分#^MK3|推论1]]若 $s>1$ 则 $\int_1^{s+2} x^{s-1}e^{-x}dx$ 必可积
...
则根据[[反常积分#^MK11|Dirichlet]] $\int_s^\infty \frac{1}{x}\frac{x^s}{e^x}dx$ 对于任意 $s$ 收敛

根据[[定积分#^MK7|区间可加]] $\int_1^\infty \frac{1}{x}\frac{x^s}{e^x}dx$ 对于任意 $s$ 收敛
...
定义 $[a,b]$ 其中 $a>0,b<\infty$ 同样根据[[定积分#^MK7|区间可加]]分割积分区域

$\int_0^\infty x^{s-1}e^{-x}dx=\int_0^1 x^{s-1}e^{-x}dx+\int_1^\infty x^{s-1}e^{-x}dx$

###### 第一部分

由于 $\left|x^{s-1}e^{-x}\right|\leq x^{a-1}$ 由于 $a>0$ 根据[[含参变量积分#^MK11|之前证明]] $\int_0^1 x^{a-1}dx$ 收敛

故根据[[含参变量积分#^MK12|Weierstrass判别]] $\int_0^1 x^{s-1}e^{-x}dx$ 对于 $s\in[a,b]$ 一致收敛

###### 第二部分

由于 $\left|x^{s-1}e^{-x}\right|\leq x^{b-1}e^{-x}$ 对于 $\int_1^\infty x^{b-1}e^{-x}dx$ 根据[[含参变量积分#^MK13|之前证明]]可得反常积分收敛

故根据[[含参变量积分#^MK14|Weierstrass判别]]有 $\int_1^\infty x^{s-1}e^{-x}dx$ 关于 $s\in[a,b]$ 一致收敛
...
- **证明 $\lim_{x\to0}\frac{(\ln(x))^{n}}{x^{-0.5a}}=0$** ^MK15
  ...
  - **当 $n=1$ 时**
    ...
    故根据[[微分方程#^MK22|洛必达法则]]有 $\lim_{x\to0^+}\frac{f(x)}{g(x)}=\lim_{x\to0^+}\frac{f'(x)}{g'(x)}$ 即 $\lim_{x\to0^+}\frac{(\ln(x))^n}{x^{-0.5a}}=\lim_{x\to0^+}\frac{x^{0.5a}}{-0.5a}=0$
  ...
  - **当 $n>1$ 时**
    ...
    则根据[[微分方程#^MK22|洛必达法则]]有 $\lim_{x\to0^+}\frac{f(x)}{g(x)}=\lim_{x\to0^+}\frac{f'(x)}{g'(x)}=0$

  - **结论**

    综上说书，对于任意 $n\in N^+,N<\infty$ 均有 $\lim_{x\to0}\frac{(\ln(x))^{n}}{x^{-s}}=0$

- **证明 $\int_0^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 收敛** ^MK16
  ...
  则根据[[定积分#^MK7|区间可加]]分割积分 $\int_0^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx=\int_0^\delta x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx+\int_\delta^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 由于 $x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 在 $[\delta,1]$ 上连续，

  故根据根据[[定积分#^MK3|推论1]] $\int_\delta^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 可积

  另一方面由于 $\int_0^\delta x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 满足

  1. 根据[[含参变量积分#^MK11|之前证明]]可得 $F(A)=\int_A^1x^{0.5a-1}dx,A\in(0,1],a>0$ 有界

  2. $\lim_{x\to0}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|=0$ 且 $\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|$ 在 $x\in(0,\delta]$ 上单调

  则根据[[反常积分#^MK11|Dirichlet]] $\int_0^\delta x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 收敛
...
- **结论**
  ...
  且 $\int_0^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 收敛，故根据[[含参变量积分#^MK12|Weierstrass判别]] $\int_0^1 x^{s-1}e^{-x}(\ln(x))^ndx$ 关于 $s\in[a,b]$ 一致收敛
...
根据[[含参变量积分#^MK15|之前证明]]可同理得 $\lim_{x\to\infty}g(x)=0$

由于 $g'(x)=\frac{x^b(\ln(x))^{n-1}[\ln(x)(b+1-x)-n]}{e^x}$ 根据[[含参变量积分#^MK16|之前证明]]同理可得存在 $\delta$ 使得 $g(x)$ 在 $x>\delta$ 上单调

又有 $\forall A>s+2,\forall s>0\Rightarrow\int_{s+2}^A\frac{1}{x^2}dx=-\frac{1}{x}\big|_{s+2}^A=\frac{1}{s+2}-\frac{1}{A}$ 有界

故根据[[反常积分#^MK11|Dirichlet]]得 $\int_1^\infty x^{b-1}e^{-x}(\ln(x))^ndx$ 收敛

故根据[[含参变量积分#^MK14|Weierstrass判别]]有 $\int_1^\infty x^{s-1}e^{-x}(\ln(x))^2dx$ 关于 $s\in[a,b]$ 一致收敛
...
故根据[[含参变量积分#^MK17|连续性]] $\Gamma(s)$ 在 $(0,\infty)$ 上连续
...
故根据[[含参变量积分#^MK10|可导性]]得 $\Gamma^{(n)}(s)=\int_0^\infty x^{s-1}e^{-x}(\ln(x))^nxdx$ 对于 $s\in(0,\infty)$ 成立

### 性质

#### $\ln(\Gamma(s))$ 为凸函数 ^MK29

根据[[含参变量积分#^MK28|之前证明]] $\Gamma(s)$ 在定义域上连续，故 $\ln(\Gamma(s))$ 亦为定义域上连续函数

根据凸函数定义，对于任意 $x,y$ 在定义域内有 $f\left(\frac{x+y}{2}\right)\leq \frac{1}{2}f(x)+\frac{1}{2}f(y)$

##### 证明

根据[[含参变量积分#^MK27|不等式]]若定义 $0<\delta<\Delta$ 则有
...
根据[[含参变量积分#^MK18|之前证明]]可得 $\Gamma(p)=2\int_0^\infty t^{2p-1}e^{-t^2}dt,\Gamma(q)=2\int_0^\infty t^{2q-1}e^{-t^2}dt$
...
故根据[[重积分#^MK43|区域无限反常重积分计算]]以及[[重积分#^MK50|无界反常重积分计算]]
...
则根据[[重积分#^MK52|重积分变量代还]]有 $\int_{D'} s^{2p-1}t^{2q-1}e^{-s^2-t^2}dv=\int_{T(D')} r^{2(p+1)-1}e^{-r^2}(\cos\theta)^{2p-1}(\sin\theta)^{2q-1}dv$
...
根据[[重积分#^MK28|引理1]]和[[重积分#^MK53|引理1]]
...
其中根据[[含参变量积分#^MK18|Gamma函数其他表示]] $\int_0^\infty r^{2(p+1)-1}e^{-r^2}dr=\frac{1}{2}\Gamma(p+q)$

根据[[含参变量积分#^MK19|Beta函数其他表示]] $\int_{0}^{\frac{\pi}{2}}(\cos\theta)^{2p-1}(\sin\theta)^{2q-1}d\theta=\frac{1}{2}\beta(p,q)$
...
根据[[含参变量积分#^MK20|函数关系]] $\beta(s,s)=\frac{\Gamma(s)\Gamma(s)}{\Gamma(2s)}$ ，根据[[含参变量积分#^MK21|定义]] $\beta(s,s)=\int_0^1x^{s-1}(1-x)^{s-1}dx$
...
根据条件（3）及[[反常积分#^MK10|非负反常积分判别]]得 $\int_a^b u(x)dx$ 也收敛

故 $\forall\epsilon>0$

根据条件（3），（4）及[[反常积分#^MK8|定义：]]有 $\exists\delta,\forall 0<\eta<\delta\Rightarrow\int_{b-\eta}^b\phi(x)dx<\frac{\epsilon}{3}$
...
- **证明**
  ...
  切分 $(a,b)$ 为 $(a,c],[c,b)$ 利用[[含参变量积分#^MK24|引理1]]可得 $\lim_{n\to\infty}\int_a^c u_n(x)dx=\int_a^c u(x)dx,\lim_{n\to\infty}\int_c^b u_n(x)dx=\int_c^b u(x)dx$
  ...
  根据[[定积分#^MK7|区间可加]] $\int_a^c u(x)dx+\int_c^b u(x)dx=\int_a^b u(x)dx,\int_a^c u_n(x)dx+\int_c^b u_n(x)dx=\int_a^b u_n(x)dx$
...
- **对数转换**
  ...
  根据[[数项级数#^MK11|正弦函数无穷乘积展开]]得 $\sin(x)=x\prod_{i=1}^\infty\left(1-\frac{x^2}{i^2\pi^2}\right)$
...
- **内闭一致连续**
  ...
  根据[[数项级数#^MK9|P级数：]]得 $\sum_{i=1}^\infty \frac{2b}{i^2\pi^2}$ 收敛，且 $l=\lim_{n\to\infty}\frac{\frac{2b}{i^2\pi^2-b^2}}{\frac{2b}{i^2\pi^2}}=\frac{1}{1-\frac{b^2}{i^2\pi^2}}=1$ 故 $0<l<\infty$

  又由于 $b<\pi\Rightarrow\frac{2b}{i^2\pi^2-b^2}>0$ 根据[[数项级数#^MK7|比较判别法：]]得 $\sum_{i=1}^\infty\frac{2b}{i^2\pi^2-b^2}$ 亦收敛

  又根据[[函数项级数#^MK6|Weierstrass判别]]得 $u_n'(x)$ 在 $x\in(0,b]$ 上一致收敛
...
- **求导**
  ...
  故当 $x\in(0,b]$ 时根据[[函数项级数#^MK14|可导性：]]可得 $u_n'(x)=(\ln(\sin(x))-\ln(x))'$
...
根据[[含参变量积分#^MK20|欧拉函数关系]]得 $\beta(s,1-s)=\frac{\Gamma(s)\Gamma(1-s)}{\Gamma(1)}=\Gamma(s)\Gamma(1-s)$

根据[[含参变量积分#^MK22|Beta函数分式表述]]及[[含参变量积分#^MK23|对称性]]得 $\beta(s,1-s)=\beta(1-s,s)=\int_0^1\frac{t^{s-1}}{1+t}dt+\int_0^1\frac{t^{-s}}{1+t}dt$

根据[[函数项级数#^MK10|经典例题]]得 $(1+t)^{-1}=\sum_{i=0}^\infty\left(\binom {-1} n t^i\right)$
...
5. $\forall\eta>0$ 定义 $a_n(t)=t^{s+i-1},b_n(t)=(-1)^n$ 则当 $t\in[\eta,1-\eta]$ 时有 a) $0<a_n(t)<(1-\eta)^{s+n-1}$ 且 $\lim_{n\to\infty}(1-\eta)^{s+n-1}=0$ ， 则根据[[函数项级数#^MK6|Weierstrass判别]]及夹逼定理得 $a_n(t)$ 一致收敛到 $0$ b) $\left|\sum_{i=0}^n b_i(t)\right|\leq 1<1.5$ 故根据[[函数项级数#^MK11|Dirichlet:]]得函数项级数 $\left\{\sum_{i=0}^n(-1)^it^{s+i-1}\right\}$ 在 $[\eta,1-\eta],1>\eta>0$ 上一致收敛

则根据[[含参变量积分#^MK25|推论]]有 $\lim_{n\to\infty}\int_0^1\left\{\sum_{i=0}^n(-1)^it^{s+i-1}\right\}dx=\int_0^1\frac{t^{s-1}}{1+t}dt$

同理第二部分有 $\lim_{n\to\infty}\int_0^1\left\{\sum_{i=0}^n(-1)^it^{-s+i}\right\}dx=\int_0^1\frac{t^{-s}}{1+t}dt$

###### 推导结论

故综合之前推导，极限四则运算和[[含参变量积分#^MK26|引理2]]，当 $s\in(0,1)$ 时有
...
定义 $\alpha+\beta=1,\alpha>0,\beta>0$ 由于[[含参变量积分#^MK29|之前证明]]可得 $\ln(\Gamma(s))$ 为凸函数
...
定义 $s=n+y+1$ 其中 $n\in N^+,0<y<1$ 则根据[[含参变量积分#^MK30|不等式]]有 $(n+y)^y\Gamma(n+1)\leq\Gamma(s)\leq(n+1)^y\Gamma(n+1)$
...
根据[[数项级数#^MK10|Stirling公式：]]得 $\lim_{n\to\infty}\frac{n!}{\sqrt{2\pi}n^{n+\frac{1}{2}}e^{-n}}=1$ 故根据极限保序性及四则运算有：
...
根据[[函数项级数#^MK15|幂级数展开：]]在 $x_0=0$ 点有展开
...
- **收敛半径**
  ...
  则根据[[数项级数#^MK6|达朗贝尔]]以及[[数项级数#^MK8|收敛性质]]
...
##### 证明不等式 $e<\left(1+\frac{1}{s}\right)^{s+\frac{1}{2}}<e^{1+\frac{1}{12s(s+1)}}$ ^MK33

根据[[含参变量积分#^MK31|泰勒展开有]] $\ln\frac{1+x}{1-x}=\ln(1+x)-\ln(1-x)=2\left(x+\frac{1}{3}x^3+\frac{1}{5}x^5+...\right)$
...
故根据[[含参变量积分#^MK33|不等式]] $1<\frac{a(s)}{a(s+1)}<e^{\frac{1}{12s(s+1)}}=e^{\frac{1}{12}\left(\frac{1}{s}-\frac{1}{s+1}\right)}=\frac{e^{\frac{1}{12s}}}{e^{\frac{1}{12(s+1)}}}$
...
且由于 $A=\lim_{n\to\infty}a(n+y)=\lim_{s\to\infty}\frac{\Gamma(s)e^s}{s^{s+\frac{1}{2}}}$ 则根据[[含参变量积分#^MK32|Gamma函数极限]]以及极限四则运算有：
