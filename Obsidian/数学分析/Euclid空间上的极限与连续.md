...
且构建确界 $\beta=a_0.a_1a_2a_2...\\$ 根据[[实变函数#^MK33|定理2]]可得 $\beta\in\mathbb{R}$
...
任意数列 $\{s_n\}$ 均有[[Euclid空间上的极限与连续#^MK29|子列]]满足 $\lim_{i\to\infty}s_{n_i}=A\in\mathbb{R}_e$ 其中 $\mathbb{R}_e$ 为[[实变函数#^MK143|广义实数]]

#### 证明

若 $\{s_n\}$ 有界，则根据[[Euclid空间上的极限与连续#^MK29|定理]]可得
...
则由于 $\{s_{n_i}\}$ 是可数集，则 $\{s_{n_v}\},\{s_{n_u}\}$ 必有一个无限集[[离散数学概论#^MK43|无限集]]
...
亦可根据[[实变函数#^MK31|完备性等价定理]]从[[Euclid空间上的极限与连续#^MK9|确界存在定理]]证得
...
若 $V$ 是[[线性方程组解#^MK24|数域]] $K$ 上的根据[[线性空间#^MK1|线性空间]]，定义 $R$ 为实数，则根据[[离散数学概论#^MK194|函数]]有函数 $\|\cdot\|:V\rightarrow R$ 满足：
...
$\forall \vec{x}\in S\Rightarrow \vec{x}\in S_i$ 即所有 $S$ 包含的点均为 $S$ 的[[Euclid空间上的极限与连续#^MK7|内点]]

#### 满足拓扑定义 ^MK24

根据[[实变函数#^MK39|拓扑定义]]有[[Euclid空间上的极限与连续#^MK15|定理8]]可得（1）、[[Euclid空间上的极限与连续#^P2|定理3]]可得（2）以及[[Euclid空间上的极限与连续#^P3|定理4]]可得（3）

故该定义为有效拓扑定义

### 开覆盖 ^MK11

若 $S\subset R^n$ ，存在一组[[Euclid空间上的极限与连续#^MK10|开集]] $\{U_\alpha\}\Rightarrow S\subset\cup_\alpha U_\alpha$ ，则称 $\{U_\alpha\}$ 是 $S$ 的开覆盖

### 闭集定义 ^MK13

$S'\subset S$ 或 $S'=\varnothing$ 即 $S$ 包含的点均为 $S$ 的[[Euclid空间上的极限与连续#^MK8|聚点]]，或 $S$ 没有聚点

#### 满足拓扑定义

根据[[Euclid空间上的极限与连续#^P1|定理1]]可得闭集 $S$ 的补集 $S^c$ 是[[Euclid空间上的极限与连续#^MK10|开集]]，由于[[Euclid空间上的极限与连续#^MK24|满足拓扑定义]]，故亦为拓扑开集

则根据[[实变函数#^MK41|拓扑闭集定义]]可得 $(S^c)^c=S$ 亦为拓扑定义闭集

### 紧集 ^MK5

若 $S$ 的任意一个[[Euclid空间上的极限与连续#^MK11|开覆盖]]，必有有限子覆盖，即 $\exists U_{\alpha_i}\in \{U_\alpha\},0\leq i\leq p<+\infty\Rightarrow S\subset \cup_{i=0}^p U_{\alpha_i}$
...
可知 $M\subset\mathbb{R}^n=\left(\bigcup_{\lambda\in\Lambda}U_\lambda\right)\cup F^c$ 根据[[Euclid空间上的极限与连续#^P1|定理1]]以及 $F$ 是闭集的条件可得 $F^c$ 是开集

则有 $\{U_{\lambda\in\Lambda},F^c\}$ 是 $M$ 的开覆盖，由于 $M$ 是[[Euclid空间上的极限与连续#^MK5|紧集]]则必有有限子覆盖 $\{U_1,...,U_n,F^c\}$

由于 $F^c\cap F=\varnothing$ 且 $F\subset M$ 故 $\bigcup_{i=1}^n U_i$ 必定覆盖 $F$ 即 $F$ 亦有有限子覆盖

### 完备集 ^MK34

不存在[[Euclid空间上的极限与连续#^MK14|孤立点]]的[[Euclid空间上的极限与连续#^MK13|闭集]]是完备集

#### 证明

根据[[实变函数#^MK45|完备集]]，进一步根据闭集定义以及[[Euclid空间上的极限与连续#^MK8|聚点]]定义，

可得若闭集 $S$ 不存在孤立点，则对于任意 $x\in S$ 不是孤立点的定义为对于任意 $\delta\in\mathbb{R}\land\delta>0$

均有 $y\in S\rightarrow d(x,y)<\delta$ 即聚点[[Euclid空间上的极限与连续#^JDCYTJ1|充要条件1]]。故所有 $S$ 中的点均为聚点。即是完备集
...
已知 $S$ 为闭集，若点 $\vec{x}\in S^c$ 则 $\vec{x}$ 不是 $S$ 的聚点，根据[[Euclid空间上的极限与连续#^JDCYTJ1|聚点充要条件1]]可得不是聚点的充要条件为：
...
因为 $\cap_\alpha S_\alpha=\cup_\alpha S_\alpha^c$ 有根据[[Euclid空间上的极限与连续#^P1|之前证明]] $S_\alpha^c$ 为开集，则根据[[Euclid空间上的极限与连续#^P2|之前证明]]任意 $\cup_\alpha S_\alpha^c$ 为开集，则其补集为闭集
...
因为 $\cup_{i=1}^k S_i=\cap_{i=1}^k S_i^c$ 有根据[[Euclid空间上的极限与连续#^P1|之前证明]] $S_\alpha^c$ 为开集，则根据[[Euclid空间上的极限与连续#^P3|之前证明]]有限个 $\cap_{i=1}^k S_i^c$ 为开集，则其补集为闭集
...
对于任意 $a\in A$ 定义 $\delta(a)=\frac{1}{2}d(\{a\},B)$ 其中 $d(a,B)$ 是[[实变函数#^MK89|子集距离]]
...
则根据[[实变函数#^MK38|度量空间]]三角性可得 $d(a',x)+d(x,b')\geq d(a',b')$
...
根据[[Euclid空间上的极限与连续#^MK10|开集定义]]表达式 $\forall \vec{x}\in S\rightarrow \vec{x}\in S_i$ 为真是 $S$ 为开集的等价条件，由于对于空集 $\vec{x}\in S'$ 永假，

故根据真值表 $\forall \vec{x}\in S\rightarrow \vec{x}\in S_i$ 为真，则根据等价性空集是开集

###### 闭集

对于全集 $\mathbb{R}^n$ 显然任意点均为内点，根据[[Euclid空间上的极限与连续#^MK10|开集定义]] $\mathbb{R}^n$ 是开集

根据[[Euclid空间上的极限与连续#^P1|定理1]]可得其补集为闭集，由于全集的补集是空集，故空集为闭集

##### 推论 ^MK26

根据本定理以及[[Euclid空间上的极限与连续#^P1|定理1]]可得全集 $\mathbb{R}^n$ 既是开集也是闭集

#### 定理9 ^MK35

若[[Euclid空间上的极限与连续#^MK13|闭集]] $S$ 存在上确界 $\sup(S)$ 则必有 $x\in S$ 满足 $x=\sup(S)$
...
综上所述 $\lim_{i\to\infty}x_i=\sup(S)$ 故根据[[Euclid空间上的极限与连续#^JDCYTJ2|充要条件]]可得 $\sup(S)$ 是[[Euclid空间上的极限与连续#^MK8|聚点]]

则根据[[Euclid空间上的极限与连续#^MK13|闭集定义]] $\sup(S)\in S$
...
故根据[[Euclid空间上的极限与连续#^JDCYTJ2|聚点充要条件2]]得出， $\vec{x}$ 为聚点。
...
故当 $j\to \infty$ 时，$diamI_j^{\alpha_j}\cap S\to 0$ 且 $I_j^{\alpha_j}\cap S\subset I_{j-1}^{\alpha_{j-1}}\cap S$ 故根据[[Euclid空间上的极限与连续#^P4|任意闭集交集为闭集]]以及[[Euclid空间上的极限与连续#^KTDL|康托闭区域套定理]] $\exists \vec{x}\in \cap_{j=1}^\infty I_j^{\alpha_j} \cap S$
...
则取 $\delta=\min(\delta_{p_1},...,\delta_{p_n})$ 则有 $O(q,\delta)\cap S=\varnothing$ 即 $q$ 是 $S^c$ 的[[Euclid空间上的极限与连续#^MK7|内点]]

由于 $q$ 的任意性，故根据[[Euclid空间上的极限与连续#^MK10|定义]]可得 $S^c$ 是开集，进一步根据[[Euclid空间上的极限与连续#^P1|定理1]]可得 $S^c$ 的补集是[[Euclid空间上的极限与连续#^MK13|闭集]]，即 $S$ 是闭集。
...
根据[[Euclid空间上的极限与连续#^BW|Bolzano-Weierstrass定理]]必有收敛子列，定义为 $\exists \{\vec{x}_k\} \in U\Rightarrow \lim_{k\to\infty}\vec{x}_k=\vec{a},\vec{x}_k\ne\vec{a}$
...
则对于所有满足 $\{\vec{x}_k\}\in S,\lim_{k\to\infty}\vec{x}_k=\vec{a},\vec{x}_k\ne\vec{a}$ 来说 $\vec{a}$ 必在 $S$ 中，根据[[Euclid空间上的极限与连续#^JDCYTJ2|聚点充要条件2]]推出 $S$ 为闭集。
...
根据[[Euclid空间上的极限与连续#^MK1|多重极限与多次极限的联系]]，则有  $\lim_{x_1\to a_1}\left\{\lim_{x_2\to a_2}\left\{...\lim_{x_n\to a_n}\left\{f(\vec{x})\right\}\right\}\right\}=\lim_{\vec{x}\to\vec{a}}f(\vec{x})$
...
则根据[[Euclid空间上的极限与连续#^MK25|距离定义]]可得 $\left|\vec{f}(\vec{x})-\vec{A}\right|=\sqrt{\sum_{i=1}^m\left(f_i(\vec{x})-A_i\right)^2}$
...
又根据[[Euclid空间上的极限与连续#^MK25|距离定义]]易证 $\left|\vec{f}(\vec{x})-\vec{A}\right|<\sqrt{\sum_{i=1}^m\epsilon_i^2}$ 则对于任意 $\epsilon^*>0$ 取 $\epsilon_1=\epsilon^*$
...
## 紧集的连续映射（连续函数）也是紧集 ^MK3

若连续函数 $\vec{f}(\vec{x})=\vec{y}, \vec{x}\in K\subset R^n, S=\{\vec{y}\}\subset R^m$ 且 $K$ 是紧集，则 $S$ 也为紧集

### 证明

对于任意无限点列 $\left\{\vec{y}_k=\vec{f}(\vec{x}_k)\big|\vec{y}_i\ne\vec{y}_i,i\ne j \right\} \subset S$ 因为 $\{\vec{x}_k\}\in K$ 为紧集，根据[[Euclid空间上的极限与连续#^MK2|Heine-Borel定理推论]] $K$ 为有界闭集，故 $\{\vec{x}_k\}$ 有界，则存在 $\{\vec{x}_{k_i}\}\Rightarrow\lim_{i\to\infty} \vec{x}_{k_i}=\vec{a}$
...
故对于任意无限点列 $\left\{\vec{y}_k=\vec{f}(\vec{x}_k)\big|\vec{y}_i\ne\vec{y}_i,i\ne j \right\} \subset S$ 均存在聚点 $\vec{f}(\vec{a})\in S$ 根据[[Euclid空间上的极限与连续#^MK2|Heine-Borel定理推论]]可得 $S$ 为紧集
...
根据[[Euclid空间上的极限与连续#^MK3|紧集映射]]可得 $S$ 亦为紧集，根据[[Euclid空间上的极限与连续#^MK2|Heine-Borel定理推论]]可得 $S$ 为有界闭集，故 $\vec{y}\in S$ 有界
...
根据[[Euclid空间上的极限与连续#^MK4|多元函数有界性定理]]可得映射 $S_i$ 有界闭集，故 $f_i(\vec{x})$ 有界
...
故根据[[Euclid空间上的极限与连续#^JDCYTJ1|聚点充要条件]]可得 $\sup\{f_i(\vec{x})\}$ 为 $S_i$ 聚点
...
则 $\vec{f}(\vec{x})$ 一致连续，此处添加一元函数[[定积分#^K3|康托定理]]的链接
...
取开覆盖 $\left\{O\left(\vec{a},\frac{\delta_{\vec{a}}}{2}\right)\big|\vec{a}\in K\right\}$ ，则根据[[Euclid空间上的极限与连续#^MK5|紧集定义]]，必有有限子覆盖  $O_i\left(\vec{a}_i,\frac{\delta_{\vec{a}_i}}{2}\right),i=1,2,...,P$
...
根据[[Euclid空间上的极限与连续#^MK6|向量值函数复合推论]]复合函数 $\vec{f}(\Gamma(u))=\vec{f}\cdot\Gamma(u)$ 亦为连续函数，且 $\vec{f}\cdot\Gamma(0)=\vec{y}',\vec{f}\cdot\Gamma(1)=\vec{y}''$ 故 $S$ 为连通集
...
其中 $\vec{A}\in\mathbb{R}_e^m$ 为[[实变函数#^MK143|广义实数]] ， $|\vec{h}_n|$ 是点 $\vec{h}_n$ 到原点的[[Euclid空间上的极限与连续#^MK25|距离]]

## 证明

根据 $\lim_{n\to\infty}\vec{h}_n=0$ 可得 $\forall \epsilon>0,\exists N,\forall n>N\rightarrow |\vec{h}_n|<\epsilon$

根据[[Euclid空间上的极限与连续#^MK33|推论]]可得其充要条件为对于每一个分量函数 $f_i(\vec{x})$ 均有 $\lim_{\vec{x}\to\vec{x}_0}f_i(\vec{x})=A_i$
