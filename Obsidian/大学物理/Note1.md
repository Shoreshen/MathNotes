...
根据[[Note1#^P1|刚体定轴转动定律]] $M_i=r_i^2 m_i \beta$ 其中 $\beta$ 为角加速度 $\frac{d\omega}{dt}$ ，带入[[Note1#^P2|力矩功]]的公式中:
...
不可压[[Note1#^P4|缩静止流]]体中任一点受外力产生压力增值后，此压力增值瞬时间传至静止流体各点。
...
[[Note1#^P5|定常流动]]的不可压缩流体，在同一时间通过任意流管段的流体质量相等： $S_1v_1=S_2v_2$

## 伯努利方程：

### 描述：

[[Note1#^P5|定常流动]]的[[Note1#^P6|理想流体]]在同一[[Note1#^P3|流管]]中不同界面处有以下有： $P+\frac{1}{2}\rho v^2+\rho gh=C$ ，其中：

$P$ 为外力压强； $\rho$ 为流体密度； $g$ 为重力加速度常量； $h$ 为重力势能高度； $C$ 为恒定常数。

### 推导：

取：

$S_1,S_2$ ：流管中横截面； $P_1,P_2$ ：所受外力的压强； $v_1,v_2$ ：截面流体流速为； $h_1,h_2$ ：截面的平均高度为；
