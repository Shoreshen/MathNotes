...
根据[[Note1#^MK1|动量定律]]以及[[Note2#^MK1|线性恢复力]]可得 $F=ma\Rightarrow m\frac{d^2x}{dt^2}=-kx$

整理上式得 $\frac{d^2x}{dt^2}+\frac{k}{m}x=0$ 定义 $\omega^2=\frac{k}{m}$ 则有 $\frac{d^2x}{dt^2}+\omega^2x=0$ 此为 **简谐运动的动力学方程**

上式为二阶常系数齐次微分方程，根据[[微分方程#^MK13|二阶常系数齐次微分方程]]
...
故有 $x=A\left[\cos(\omega t)\cos(\phi)-\sin(\omega t)\sin(\phi)\right]=A\cos(\omega t+\phi)$ 该形式与简写运动[[Note2#^MK2|定义]]一致
...
故[[Note2#^MK3|动力学方程1]]可简化为 $\frac{d^2\theta(t)}{dt^2}+\frac{g}{L}\theta=0$ 近似视为简谐运动
...
带入得 $E_k=\frac{1}{2}mA^2\omega^2\sin^2(\omega t+\phi)$ 其中又根据[[Note2#^MK4|动力学方程]]有 $\omega^2=\frac{k}{m}$

带入得 $E_k=\frac{1}{2}kA^2\sin^2(\ometa t+\phi)$

### 势能

势能可看做位移返回平衡位置时[[Note2#^MK1|线性恢复力]]做的功，故当距离为 $x$ 时积分 $E_s=\int_x^0ktdt=\frac{1}{2}kx^2$ 为线性恢复力做的功
...
当 $t=0$ 时 $x=A_1\cos(\phi_1)+A_2\cos(\phi_2)$ 接下来利用[[Note2#^MK5|相图]]求距离和 $x$ 的值
...
根据上述证明同理，结果任然为简谐运动。利用[[Note2#^MK5|相图]]可得 $\vec{A}=\sum_{i=1}^n\vec{A}_i$ 即为
