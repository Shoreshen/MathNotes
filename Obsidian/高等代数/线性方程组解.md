...
设[[多项式#^MK12|交换环]] $<U,+,\cdot>$ 满足：

1. 存在乘法[[抽象代数#^MK299|幺元]]，定义为 $1$ （根据[[多项式#^MK9|定理]]存在乘法零元，等于加法幺元，记作 $0$ ）

2. $1$ 不等于加法[[抽象代数#^MK299|幺元]]（定义为 $0$ ）

3. $\forall a\left(a\in U\land a\ne0\rightarrow\exists y(y\in U\land a\cdot y=1)\right)$ 即任意不等于加法幺元的元素存在乘乘法[[抽象代数#^MK302|逆元]]
...
根据复数上的加法/乘法，以及[[多项式#^MK12|交换环]]可知
...
故根据[[线性方程组解#^MK24|域定义]]得出代数结构 $<K,+,\cdot>$ 是域
...
若有线性方程组满足[[线性方程组解#^MK3|定义]]，则其增广矩阵为 $E=\left[\begin{matrix}a_{11}&a_{12}&...&a_{1n}&b_1\\a_{21}&a_{22}&...&a_{2n}&b_2\\...&...&...&...&...\\a_{m1}&a_{m2}&...&a_{mn}&b_m\end{matrix}\right]$

### 行的主元 ^MK31

在矩阵中，第 $r$ 行的主元为该行第一个非零元素。
...
简化行阶梯型矩阵可由阶梯矩阵经过[[线性方程组解#^MK1|初等行变换]]得到。
...
该操作可视为 $&\textcircled{1}$ 式乘以系数加到 $&\textcircled{k}$ 式，故操作后方程组解相同，改类操作称为 **初等行变换** ^MK1
...
$n$ 元线性方程组解的情况只有三种，当增广矩阵经过[[线性方程组解#^MK1|初等行变换]]后称为阶梯型矩阵 $E$ 时，若：
...
根据[[线性方程组解#^MK4|增广矩阵]]定义，线性方程组对应的增广矩阵列数为 $n+1$ 行数为 $m$

由于不存在（1）所述情况，且 $r=n$ ，故有 $m\geq n$ 且最后的非零行主元所在列 $d_r<n+1$

又根据[[线性方程组解#^MK2|阶梯矩阵]]定义，得第 $r-1$ 行的主元所在列满足 $d_{r-1}<d_r$ 最大为 $d_r-1$
...
故根据[[线性方程组解#^MK5|简化行阶梯型矩阵]]定义，上述阶梯矩阵可由初等变换得到简化行阶梯型矩阵 $E_k$ 且根据定义 $E_k$ 满足

1. 当 $i>n$ 时第 $i$ 行的元素全部为零

2. 当 $i<1$ 时第 $i$ 行的第 $i$ 个元素为 $1$ 其余全为零

故根据[[线性方程组解#^MK6|矩阵消元法]]得经过初等行变换的矩阵与原线性方程组通解。

$E_k$ 对于每一个未知量 $x_i$ 均有赋值，故 $E_k$ 有唯一解，同理原线性方程组也仅有唯一解

#### 条件（3）

同理根据[[线性方程组解#^MK5|简化行阶梯型矩阵]]定义，阶梯矩阵可由初等变换得到简化行阶梯型矩阵 $E_k$
...
### 推论1：$n$ 元齐次线性方程组只有零解和无穷解两种情况 ^MK34

根据定义 $x_1=x_2=...=x_n=0$ 必为其中一解，且根据[[线性方程组解#^MK7|线性方程组解的情况]]，若有非零解，则线性方程组有无穷解。

### 推论2：$n$ 元齐次线性方程组有非零解的充分条件为方程数目小于未知数数目 ^MK36

根据[[线性方程组解#^MK7|线性方程组解的情况]]，若转化为阶梯型矩阵 $E$ 的非零行数目 $r$ 小于未知数数目 $n$ 则有有无穷解，故有非零解。
...
由于 $j_{i_1}<j_{i_2}<...<j_{i_n}$ 故 $j_{i_1}j_{i_2}...j_{i_n}$ 的逆序数为零，故为偶排列。根据[[线性方程组解#^MK9|定理1]]每次互换均改变排列奇偶性。
...
2. $\tau(j_1j_2...j_n)$ 为排列的[[线性方程组解#^MK8|逆序数]]
...
根据[[线性方程组解#^MK9|定理1]] $\tau(k_1k_2...k_n)$ 与 $\tau(j_1j_2...j_n)$ 相比奇偶性改变 $s$ 次，且 $\tau(i_1i_2...i_n)=s$
...
代数余子式记作 $A_{ij}=(-1)^{i+j}\overline{M}_{ij}$ 其中 $\overline{M}_{ij}$ 为[[线性方程组解#^MK22|余子式]]
...
综合来说，余子式是去除[[线性方程组解#^MK21|子式]]取的行列后，剩下的行列上元素组成的行列式
...
当 $k=1$ 时，其定义与[[线性方程组解#^MK17|单个元素]]相同
...
根据[[线性方程组解#^MK10|定理1]]有 $|A|=\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_1j_2...j_n)}a_{1j_1}a_{2j_2}...a_{nj_n}=\sum_{\forall i_1i_2...i_n}(-1)^{\tau(i_1i_2...i_n)}a_{i_11}a_{i_22}...a_{i_nn}=|A^T|$
...
根据[[线性方程组解#^MK11|定义]] $|B|=\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_1j_2...j_n)}a_{1j_1}...Ka_{ij_i}...a_{nj_n}=K\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_1j_2...j_n)}a_{1j_1}a_{2j_2}...a_{nj_n}=K|A|$

#### 推论

且根据[[线性方程组解#^MK12|转置相等]]性质可得，列乘系数同样 $|B|=K|A|$
...
且根据[[线性方程组解#^MK12|转置相等]]性质可得列为两组数和同样 $|C|=|A|+|B|$
...
根据[[线性方程组解#^MK11|定义]] $|A|=\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_1...j_{u-1}j_uj_{u+1}...j_{v-1}j_vj_{v+1}...j_n)}a_{1j_1}...a_{(u-1)j_{u-1}}a_{uj_u}a_{(u+1)j_{u+1}}...a_{(v-1)j_{v-1}}a_{vj_v}a_{(v+1)j_{v+1}}...a_{nj_n}$
...
根据[[线性方程组解#^MK12|转置相等]]可得 $|B|=|B^T|=-|A^T|=-|A|$
...
根据行列式[[线性方程组解#^MK11|定义]]，行列式为每行取一个元素的乘积的和，故第 $n$ 行唯一不为零的元素是 $a_{nn}$
...
且根据[[线性方程组解#^MK12|转置相等]]性质可得，下三角行列式同样 $|A^T|=\prod_{i=1}^n a_{ii}$
...
根据[[线性方程组解#^MK14|行乘系数]]可得 $|A|=K\left|\begin{matrix}a_{11}&a_{12}&...&a_{1n}\\...&...&...&...\\a_{i1}&a_{i2}&...&a_{in}\\...&...&...&...\\a_{i1}&a_{i2}&...&a_{in}\\...&...&...&...\\a_{n1}&a_{n2}&...&a_{nn}\end{matrix}\right|=K|E|$ 将 $i,j$ 两行互换后的矩阵 $B$

由于两行互换后矩行列式计算同理可提出系数，则有 $B=K|E'|$ 且提出系数后 $|E'|=|E|$

根据[[线性方程组解#^MK13|两行互换]]可得 $|B|=-|A|=-K|E|$ 结合上式得 $K|E|=-K|E|\Rightarrow |E|=0$

故有 $|A|=0$

#### 推论

且根据[[线性方程组解#^MK12|转置相等]]性质可得，一列为另一列 $K$ 倍 $|A|=0$
...
根据[[线性方程组解#^MK15|行为两组数和]]得 $|D|=|A|+\left|\begin{matrix}a_{11}&a_{12}&...&a_{1n}\\...&...&...&...\\Ka_{i1}&Ka_{i2}&...&Ka_{in}\\...&...&...&...\\a_{n1}&a_{n2}&...&a_{nn}\end{matrix}\right|$

又根据[[线性方程组解#^MK16|一行为另一行K倍]]得 $\left|\begin{matrix}a_{11}&a_{12}&...&a_{1n}\\...&...&...&...\\Ka_{i1}&Ka_{i2}&...&Ka_{in}\\...&...&...&...\\a_{n1}&a_{n2}&...&a_{nn}\end{matrix}\right|=0$

故得 $|A|=|D|$

#### 推论

且根据[[线性方程组解#^MK12|转置相等]]性质可得，初等列变换 $|A|=|D|$
...
其中 $A_{ki}$ 为矩阵 $A$ 对于元素 $a_{ki}$ 的[[线性方程组解#^MK17|代数余子式]]

#### 证明

根据[[线性方程组解#^MK11|定义]]得 $|A|=\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_1j_2...j_n)}a_{1j_1}a_{2j_2}...a_{nj_n}$

又根据[[线性方程组解#^MK10|定理1]]得 $|A|=\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_kj_1...j_{k-1}j_{k+1}...j_n)+\tau(k1..(k-1)(k+1)...n)}a_{kj_k}a_{1j_1}...a_{(k-1)j_{k-1}}a_{(k+1)j_{k+1}}..a_{nj_n}$
...
当矩阵中某一行为零时，取该行的行列式 $|A|=\sum_{i=1}^n 0A_{ki}=0$ ，且根据[[线性方程组解#^MK12|转置相等]]某一列为零时亦然

### 可逆矩阵行列式 ^MK33

若矩阵 $A$ [[矩阵运算#^MK28|可逆]]，则有 $|A^{-1}|=\frac{1}{|A|}$

#### 证明

根据[[线性方程组解#^MK18|上三角矩阵行列式]]可得[[矩阵运算#^MK4|单位矩阵]]的行列式为 $|I|=1$

根据[[线性方程组解#^MK32|矩阵乘法与行列式]]可得 $|A^{-1}||A|=|A^{-1}A|=|I|=1$ 故有 $|A^{-1}|=\frac{1}{|A|}$
...
3. 当 $s<n$ 时 $|AB|=\sum_{\forall 1\leq j_1\leq...\leq j_s\leq s}A\left(\begin{matrix}1&2&...&s\\j_1&j_2&...&j_s\end{matrix}\right)B\left(\begin{matrix}j_1&j_2&...&j_s\\1&2&...&s\end{matrix}\right)$ 其中 $A,B$ 分别代表[[线性方程组解#^MK21|子式]]
...
根据[[矩阵运算#^MK24|乘法性质]]可得 $\dim(AB)\leq \dim(A)\leq n$

由于根据[[矩阵运算#^MK5|矩阵乘法]]可得 $AB$ 为 $s$ 阶方阵，则 $AB$ 不满秩，故 $|AB|=0$

##### 证明（2）

若 $|A|=0$ 则 $A$ 不满秩，同理根据[[矩阵运算#^MK24|乘法性质]]可得 $\dim(AB)\leq \dim(A)< n$

故得 $|AB|=0$

若 $|A|\ne 0$ 则 $A$ 满秩，故根据[[矩阵运算#^MK29|可逆条件]]可得 $A$ 可逆

进一步根据[[矩阵运算#^MK37|定理7]]可得 $A=\prod_{i=1}^vP_i$ 其中 $P_i$ 为[[矩阵运算#^MK13|初等矩阵]]

顾得 $|AB|=\left|\prod P_i B\right|$ 根据[[矩阵运算#^MK48|初等矩阵性质]]以及[[矩阵运算#^MK21|乘法结合律]]可得 $|AB|=\left|\prod P_i\right||B|=|A||B|$

##### 证明（3）

###### 利用分块矩阵

构造[[矩阵运算#^MK38|分块矩阵]] $\left[\begin{matrix}A&0_{s}\\I_n&B\end{matrix}\right]$ 其中分块 $I_n$ 为 $n$ 阶[[矩阵运算#^MK4|单位矩阵]]，且 $0_s$ 为 $s$ 阶[[矩阵运算#^MK9|零矩阵]]

构造[[矩阵运算#^MK45|分块初等矩阵]] $\left[\begin{matrix}I_s&-A\\0_{n\times s}&I_n\end{matrix}\right]$ 则有：
...
根据[[线性方程组解#^MK18|上三角矩阵行列式]]可得 $\left|\begin{matrix}I_s&-A\\0_{n\times s}&I_n\end{matrix}\right|=1$ 根据[[矩阵运算#^MK29|可逆条件]]可得该矩阵可逆，根据[[矩阵运算#^MK37|定理7]]可得该矩阵可分解为[[矩阵运算#^MK13|初等矩阵]]相乘

故根据[[矩阵运算#^MK48|初等矩阵与乘法]]以及[[矩阵运算#^MK21|乘法结合律]]可得 $\left|\begin{matrix}I_s&-A\\0_{n\times s}&I_n\end{matrix}\right|\left|\begin{matrix}A&0_s\\I_n&B\end{matrix}\right|=\left|\begin{matrix}A&0_s\\I_n&B\end{matrix}\right|=\left|\begin{matrix}0_{s\times n}&-AB\\I_{n\times n}&B\end{matrix}\right|$

###### 利用拉普拉斯定理

- **等式左边矩阵行列式**

  根据[[线性方程组解#^MK26|拉普拉斯定理]]，对 $\left|\begin{matrix}A&0_s\\I_n&B\end{matrix}\right|$ 取前 $s$ 行即 $i_l=l,1\leq l\leq s$ ，对其列进行展开
  ...
  则根据[[线性空间#^MK102|性质3]]可得求子式的矩阵列向量现行相关，则根据[[线性空间#^MK57|行列式与线性相关性]]可得其行列式为零，故子式为零，则得
...
- **等式右边矩阵行列式**
  ...
  且根据[[线性方程组解#^MK14|行乘系数]] $|-AB|$ 每行有一个 $-1$ 则 $(-1)^s|AB|=|-AB|$ 故：
...
根据[[线性方程组解#^MK7|解的情况]]有 $0=d$ 的等式出现，则 $B$ 必有某行全部为零，根据[[线性方程组解#^MK18|上三角矩阵]]可得 $|B|=\prod_{i=1}^n b_{ii}=0$

又根据[[线性方程组解#^MK19|初等行变换]]可得 $|B|=|A|$

##### 有无穷解

根据[[线性方程组解#^MK7|解的情况]]非零行数目小于未知数数目，则 $B$ 必有某行全部为零，根据[[线性方程组解#^MK18|上三角矩阵]]可得 $|B|=\prod_{i=1}^n b_{ii}=0$

又根据[[线性方程组解#^MK19|初等行变换]]可得 $|B|=|A|$

##### 有唯一解

根据[[线性方程组解#^MK7|解的情况]]非零行数目等于未知数数目，则 $B$ 为上三角矩阵且对角线上的数字均不等于零，则有 $|B|=\prod_{i=1}^n b_{ii}\ne0$

又根据[[线性方程组解#^MK19|初等行变换]]可得 $|B|\ne0\Rightarrow|A|\ne0$

#### 必要性

若 $|A|\ne 0$ 根据[[线性方程组解#^MK19|初等行变换]]经过初等行变换获得阶梯矩阵 $B$ 后行列式 $|B|=|A|$
...
其中 $M\left(\begin{matrix}i_1&i_2&...&i_k\\j_1&j_2&...&j_k\end{matrix}\right),\overline{M}\left(\begin{matrix}i_1&i_2&...&i_k\\j_1&j_2&...&j_k\end{matrix}\right)$ 分别为 $A$ 的[[线性方程组解#^MK21|子式]]和[[线性方程组解#^MK22|余子式]]，
...
根据[[线性方程组解#^MK11|行列式定义]]有 $|A|=\sum_{\forall j_1j_2...j_n}(-1)^{\tau(j_1j_2...j_n)}a_{1j_1}a_{2j_2}...a_{nj_n}$

根据[[线性方程组解#^MK10|定理1]]将所有行标属于 $I$ 的移动到最前面
...
由于 $j_1j_2...j_k$ 逆序数为零，故根据[[线性方程组解#^MK23|定理2]]转换的次数的奇偶性与 $\tau(j_{i_1}...j_{i_k})$ 的奇偶性一致

且根据[[线性方程组解#^MK9|定理1]]，每转换一次逆序数的奇偶性改变一次
...
根据[[线性方程组解#^MK22|定义]] $M\left(\begin{matrix}i'_1&i'_2&...&i'_{n-k}\\j'_1&j'_2&...&j'_{n-k}\end{matrix}\right)=\sum_{\forall j'_1...j'_{n-k}}\left((-1)^{\tau(j_{i'_k}...j_{i'_{n-k}})}a_{i'_1j_{i'_1}}...a_{i'_{n-k}j_{i'_{n-k}}}\right)$

根据[[线性方程组解#^MK21|定义]] $M\left(\begin{matrix}i_1&i_2&...&i_k\\j_1&j_2&...&j_k\end{matrix}\right)=\sum_{\forall j_1...j_k}\left[(-1)^{\tau(j_{i_1}...j_{i_k})}a_{i_1j_{i_1}}...a_{i_kj_{i_k}}\right]$
...
根据[[线性方程组解#^MK12|转置相等]]性质可得任取 $k$ 列亦可
...
根据[[线性方程组解#^MK19|初等行变换推论]]做初等列变换，将所有 $k,k\ne1$ 列加到第一列得 $|A|=|A_2|=\left|\begin{matrix}x+(n-1)y&y&...&y\\x+(n-1)y&x&...&y\\\vdots&\vdots&\ddots&\vdots\\x+(n-1)y&y&...&x\end{matrix}\right|$

根据[[线性方程组解#^MK14|行乘系数推论]]对第一列提出 $x+(n-1)y$ 故得 $|A|=|A_3|=(x+(n-1)y)\left|\begin{matrix}1&y&...&y\\1&x&...&y\\\vdots&\vdots&\ddots&\vdots\\1&y&...&x\end{matrix}\right|$

根据[[线性方程组解#^MK19|初等行变换]]，将所有 $k,k\ne1$ 行减去第一行得 $|A|=|A_4|=(x+(n-1)y)\left|\begin{matrix}1&y&...&y\\0&x-y&...&y\\\vdots&\vdots&\ddots&\vdots\\0&0&...&x-y\end{matrix}\right|$

根据[[线性方程组解#^MK18|上三角矩阵]]得 $|A|=|A_4|=(x+(n-1)y)(x-y)^{n-1}$
...
根据[[线性方程组解#^MK19|初等行变换]]，将第 $n$ 行减去第 $n-1$ 行乘以 $a_n$ 得 $|A|=|A_1|=\left|\begin{matrix}1&1&...&1&1\\a_1&a_2&...&a_{n-1}&a_n\\...&...&...&...&...\\a_1^{n-2}(a_1-a_n)&a_2^{n-2}(a_2-a_n)&...&a_{n-1}^{n-2}(a_{n-1}-a_n)&0\end{matrix}\right|$
...
根据[[线性方程组解#^MK14|行乘系数推论]]对 $k,k\ne n$ 列提出系数 $|A|=|A_n|=\prod_{i=1}^{n-1}(a_i-a_n)\left|\begin{matrix}\frac{1}{a_1-a_n}&\frac{1}{a_2-a_n}&...&\frac{1}{a_{n-1}-a_n}&1\\1&1&...&1&0\\...&...&...&...&...\\a_1^{n-3}&a_2^{n-3}&...&a_{n-1}^{n-3}&0\\a_1^{n-2}&a_2^{n-2}&...&a_{n-1}^{n-2}&0\end{matrix}\right|$

又根据[[线性方程组解#^MK20|行列式拆分]] $\left|\begin{matrix}\frac{1}{a_1-a_n}&\frac{1}{a_2-a_n}&...&\frac{1}{a_{n-1}-a_n}&1\\1&1&...&1&0\\...&...&...&...&...\\a_1^{n-3}&a_2^{n-3}&...&a_{n-1}^{n-3}\\a_1^{n-2}&a_2^{n-2}&...&a_{n-1}^{n-2}&0\end{matrix}\right|=(-1)^{n+1}\left|\begin{matrix}1&1&...&1\\...&...&...&...\\a_1^{n-3}&a_2^{n-3}&...&a_{n-1}^{n-3}\\a_1^{n-2}&a_2^{n-2}&...&a_{n-1}^{n-2}\end{matrix}\right|$
