...
3. 存在加法[[抽象代数#^MK299|幺元]]： $\exists 0\in U\rightarrow\left((\forall a\in U)\rightarrow(a+0=0+a=a)\right)$ 记作零元

4. 存在加法[[抽象代数#^MK302|逆元]]： $\forall a\in U(\exists b\in U\rightarrow a+b=b+a=0)$ 其中 $0$ 是加法幺元
...
若 $U$ 是环，存在元素 $e\in U$ 与环中的乘法满足[[抽象代数#^MK299|幺元]]定义，则称 $e$ 为单位元

且根据[[抽象代数#^MK300|性质]]可得 $e$ 唯一

## 零元定理 ^MK9

任意[[多项式#^MK12|交换环]] $\mathcal{U}=\left<U,+,\cdot\right>$ 存在乘法[[抽象代数#^MK301|零元]]，且等于[[多项式#^MK10|环定义]]中加法幺元
...
若存在环 $U$ 其非空子集 $V$ 对于 $U$ 上的加法，乘法满足[[多项式#^MK10|环定义]]中的条件，则称 $V$ 是 $U$ 的子环
...
由于零元逆元均封闭， $V$ 满足[[多项式#^MK10|环定义]]中条件（3）,（4）
...
有环 $U,V$ 若存在[[离散数学概论#^MK200|双射函数]] $\sigma:U\rightarrow V$ 满足对于任意 $\{a,b\}\subset U$ ：
...
环 $U,V$ 同构且同构映射为 $\sigma:U\rightarrow V$ 若 $1\in U$ 为 $U$ 的[[多项式#^MK11|单位元]]，则 $V$ 有单位元且为 $\sigma(1)$
...
设有数域 $K$ ，环 $R$ 是[[多项式#^MK12|交换环]]且有[[多项式#^MK11|单位元]] $1$ ， $R$ 中有一个包含 $1$ 的子环 $I$

若存在[[多项式#^MK13|同构函数]] $\sigma:K\rightarrow I$ 则定义 $I[\tau]$ 是表达式 $\sum_{i=1}^\infty b_i\tau$ 其中 $b_i\in I$ ， $\tau\in R$ 是变元为 $\tau$ 的[[多项式#^MK1|一元多项式]]
...
1. 根据[[离散数学概论#^MK194|函数定义]]， $f(x)$ 到 $f(\tau)$ 的关系形成函数
...
由于 $K[x],I[\tau]$ 均为一元多项式，则根据[[多项式#^MK5|一元多项式相等]]可知元素相同的充要条件为系相同

由于 $\sigma$ 为[[离散数学概论#^MK200|双射函数]]，故必为单射 $\forall a\forall a'(\{a,a'\}\subset K\land a\ne a'\rightarrow \sigma(a)\ne\sigma(a'))$
...
定义[[线性方程组解#^MK24|数域]] $K$ ，若有表达式形式为 $f(x)=\sum_{i=0}^d a_ix^i$ ，其中 $a_i\in K$ ，其中 $x$ 是变元，可以有定义域
...
根据[[多项式#^MK6|乘法]]定义可得 $0(x)$ 乘以任何一元多项式均为 $0(x)$

根据[[多项式#^MK2|加法]]定义可得 $0(x)$ 加任意一元多项式 $f(x)$ 有 $f(x)=f(x)+0(x)=0(x)+f(x)$

### 一元多项式次数 ^MK17

若一元多项式 $f(x)=\sum_{i=1}^\infty a_ix^i$ 定义集合 $I=\left\{i\big|a_i\ne0\land i\in N\right\}$

则根据[[离散数学概论#^MK186|元]]集合 $I$ 的最大元为多项式 $f(x)$ 的次数，记作 $\deg(f(x))$

特殊的，当 $I$ 的最大元为零时，定义 $\deg(f(x))=-\infty$

#### 定理 ^MK18

若 $f(x),g(x)$ 均为[[多项式#^MK1|一元多项式]]，则有：
...
根据[[多项式#^MK3|零元]]可得 $f(x)g(x)=0(x)$ 且 $\deg(0(x))=-\infty$

假设 $f(x)=0(x)$ 则有 $\deg(f(x)g(x))=-\infty+\deg(g(x))=-\infty$

若 $\deg(f(x))\ne-\infty\land\deg(g(x))\ne-\infty$ 则假设 $\deg(f(x))=n,\deg(g(x))=m$

则根据[[多项式#^MK6|乘法]]可得 $f(x)g(x)=\sum_{s=1}^\infty(\sum_{i+j=s}a_ib_j)x^s$
...
根据[[多项式#^MK7|负元]]可得存在 $-f(x)h(x)$ 使得 $-f(x)h(x)+(-f(x)h(x))=0(x)$

则有 $f(x)g(x)+(-f(x)h(x))=f(x)h(x)+(-f(x)h(x))$

化简及应用[[多项式#^MK8|分配率]]得 $f(x)(g(x)-h(x))=0(x)$ 由于 $f(x)\ne0(x)$
...
根据[[线性空间#^MK1|线性空间定义]]，定义集合 $V$ 为所有[[多项式#^MK1|一元多项式]]组成的集合，则 $V$ 是线性空间
...
则根据[[多项式#^MK2|加法]]定义可得 $f(x)+g(x)=\sum_{i=1}^\infty(a_i+b_i)x^i=\sum_{i=1}^\infty(b_i+a_i)x^i=g(x)+f(x)$

###### 加法结合律

若 $f(x)=\sum_{i=0}^\infty a_ix^i,g(x)=\sum_{i=0}^\infty b_ix^i,h(x)=\sum_{i=0}^\infty c_ix^i$ 且 $\{f(x),g(x),h(x)\}\subset V$

则根据[[多项式#^MK2|加法]]定义可得 $(f(x)+g(x))+h(x)=\sum_{i=1}^\infty((a_i+b_i)+c_i)x^i=\sum_{i=1}^\infty(a_i+(b_i+c_i))x^i=f(x)+(g(x)+h(x))$
...
对于任意 $f(x)\in V$ 其负元为 $(-1)f(x)$ 根据[[多项式#^MK1|定义]]函数有形式 $f(x)=\sum_{i=1}^\infty a_ix^i$

则根据[[多项式#^MK4|数乘]]运算可得 $(-1)f(x)=\sum_{i=1}^\infty -a_ix^i$ 同时根据[[多项式#^MK3|加法]]可得 $f(x)+(-1)f(x)=\sum_{i=1}^n(a_i-a_i)x_i$

根据[[多项式#^MK3|零元]]定义可得 $f(x)+(-1)f(x)=0(x)$

###### 幺元

对于任意 $f(x)\in V$ 函数有形式 $f(x)=\sum_{i=1}^\infty a_ix^i$

根据[[多项式#^MK4|数乘]]运算可得 $1\times f(x)=\sum_{i=1}^\infty 1\times a_ix^i=\sum_{i=1}^\infty a_ix^i$

###### 数乘结合律

对于任意 $f(x)\in V$ 函数有形式 $f(x)=\sum_{i=1}^\infty a_ix^i$

对于任意 $\{l,k\}\subset K$ 根据[[多项式#^MK4|数乘]]运算可得 $(lk)f(x)=\sum_{i=1}^\infty (lk)a_ix^i=\sum_{i=1}^\infty l(ka_ix^i)=l(kf(x))$

###### 数乘前分配率

对于任意 $f(x)\in V$ 函数有形式 $f(x)=\sum_{i=1}^\infty a_ix^i$

对于任意 $\{l,k\}\subset K$ 根据[[多项式#^MK4|数乘]]运算可得 $(l+k)f(x)=\sum_{i=1}^\infty (l+k)a_ix^i=\sum_{i=1}^\infty la_ix^i+\sum_{i=1}^\infty ka_ix^i=lf(x)+kf(x)$

###### 数乘后分配率

对于任意 $\{f(x),g(x)\}\subset V$ 函数有形式 $f(x)=\sum_{i=1}^\infty a_ix^i,g(x)=\sum_{i=1}^\infty b_ix^i$

对于任意 $l\in K$ 根据[[多项式#^MK4|数乘]]以及[[多项式#^MK2|加法]]运算可得 $(l+k)f(x)=\sum_{i=1}^\infty (l+k)a_ix^i=\sum_{i=1}^\infty la_ix^i+\sum_{i=1}^\infty ka_ix^i=lf(x)+kf(x)$

##### 证明基

###### step 1

先证明 $\forall l\in N\rightarrow x^l\in V$ 根据[[多项式#^MK1|定义]]可得对于任意 $f(x)\in V$ 均有 $f(x)=\sum_{i=1}^\infty a_ix^i$ 的形式
...
根据[[线性空间#^MK22|子集线性无关]]，取任意 $\{x^{i_1},...,x^{i_n}\}\subset\{x^0,...,x^n,...\}$ 其中 $n\leq\infty$

根据集合定义 $a\ne b\rightarrow i_a\ne i_b$ 构造等式 $0(x)=\sum_{j=1}^nk_jx^{i_j}$ 其中 $k_j\in K$ 且 $0(x)$ 为[[多项式#^MK3|零元]]

根据零元定义以及[[多项式#^MK5|相等]]定义可得，当且仅当 $(\forall j\in N\land 0\leq j\leq n)\rightarrow k_j=0$ 成立时 $0(x)=\sum_{j=1}^nk_jx^{i_j}$
...
由于 $x^i\in\{x^0,...,x^n,...\}$ 则根据[[多项式#^MK4|数乘]]定义可得任意 $f(x)\in V$ 均可取 $k_i=a_i$ 使得 $f(x)=\sum_{i=0}^\infty k_ix^i$

即由 $\{x^0,...,x^n,...\}$ 线性表述

##### 证明维数

由于之前证明得 $V$ 的一组基为 $\{x^0,...,x^n,...\}$ ，则根据[[线性空间#^MK51|线性空间的维数]]可得 $\dim(V)=\infty$

### 一元多项式环 ^MK15

[[多项式#^MK1|一元多项式]]集合 $K[x]$ 是[[多项式#^MK10|环]]

#### 证明

定义 $f(x)=\sum_{i=0}^\infty a_ix^i,g(x)=\sum_{i=0}^\infty b_ix^i,h(x)=\sum_{i=0}^\infty c_ix^i$ 是 $K[x]$ 的任意一元多项式  根据[[多项式#^MK10|环定义]]证明

1. 根据[[多项式#^MK2|加法]]可得 $(f(x)+g(x))+h(x)=\sum_{i=1}^\infty((a_i+b_i)+c_i)x^i=\sum_{i=1}^\infty(a_i+(b_i+c_i))x^i=f(x)+(g(x)+h(x))$

2. 根据[[多项式#^MK2|加法]]可得 $f(x)+g(x)=\sum_{i=1}^\infty(a_i+b_i)x^i=\sum_{i=1}^\infty(b_i+a_i)x^i=g(x)+f(x)$

3. 根据[[多项式#^MK3|零元]]可得 $\forall f(x)\bigg(f(x)\in K[x]\rightarrow\exists 0(x)\big(f(x)+0(x)=0(x)+f(x)=f(x)\big)\bigg)$

4. 根据[[线性方程组解#^MK24|数域]]可得 $-1\in K$ 则结合乘法 $\forall \sum_{i=0}^\infty a_ix^i\in K[x]$ 均有 $(-1)\sum_{i=0}^\infty a_ix^i=\sum_{i=0}^\infty -a_ix^i\in K[x]$ 结合加法可得 $\sum_{i=0}^\infty a_ix^i+(-1)\sum_{i=0}^\infty a_ix^i=\sum_{i=0}^\infty 0x^i=0(x)$

5. 根据[[多项式#^MK8|乘法性质]]可得交换律

6. 根据[[多项式#^MK6|乘法性质]]可得与加法的分配率
...
根据 $K[x]$ [[多项式#^MK15|是环]]的证明：（1）任意元素存在逆元;（2）加/乘法封闭

故有 $f(x)h(x)$ 存在逆元为 $(-1)f(x)h(x)$

则根据[[多项式#^MK20|加法性质]]两边同加 $f(x)g(x)+(-f(x)h(x))=f(x)h(x)+(-f(x)h(x))$
...
由于 $f(x)\ne0(x)$ 根据[[多项式#^MK3|零元]]定义得必存在 $a_k\ne 0$

此时假 $g(x)+(-1)h(x)\ne0(x)$ 则存在 $b_u\ne0$

故 $f(x)(g(x)+(-1)h(x))$ 必有 $a_kb_ux^{u+k}$ 一项系数不为零

根据[[多项式#^MK3|零元]]定义 $f(x)(g(x)+(-1)h(x))\ne0(x)$ 与定义冲突，故必有 $g(x)+(-1)h(x)=0(x)$

###### 证明结论

同样根据[[多项式#^MK20|加法性质]] $g(x)+(-1)h(x)+h(x)=h(x)$
...
定义 $b\in K[x]$ 为零次[[多项式#^MK1|多项式]]且 $b\ne0$ ，则有 $\forall f(x)(f(x)\in K[x]\rightarrow b\nmid f(x))$

###### 证明

零次多项式定义为 $b=a_0x^0\land b\ne 0$ 根据[[线性方程组解#^MK24|数域]] $1\in K$ 则有 $\frac{1}{b}\in K$ 同理 $\frac{1}{b}$ 亦为零次一元多项式

根据[[多项式#^MK6|乘法]]可得 $f(x)=\left(\frac{1}{b}f(x)\right)b=f(x)$

##### 零元是任意多项式的被式 ^MK44

若 $0(x)\in K[x]$ 是[[多项式#^MK3|零元]]，则 $\forall f(x)\in K[x]$ 均有 $f(x)\mid 0(x)$

###### 证明

根据[[多项式#^MK15|一元多项式环]]可得 $0(x)$ 是 $K[x]$ 的零元，根据[[多项式#^MK9|定理]]可得零元是零因子，即 $0(x)f(x)=0(x)$

故得结论 $f(x)\mid 0(x)$

##### 零元的被式是零元 ^MK21

若 $0(x)\in K[x]$ 是[[多项式#^MK3|零元]]且 $0(x)$ 可以整除 $f(x)$ 即 $0(x)\mid f(x)$ 则 $f(x)=0(x)$

###### 证明

根据定义必须满足 $f(x)=0(x)h(x)$ 根据[[多项式#^MK9|定理]] $\forall h(x)\in K[x]\rightarrow 0(x)h(x)=0(x)$

故得定理

##### 因式的次数小于等于被试 ^MK22

若 $g(x)\mid f(x)$ 且 $\deg(f(x))\ne-\infty$ 则他们的[[多项式#^MK17|次数]]关系是 $\deg(g(x))\leq\deg(f(x))$

###### 证明

根据定义满足 $f(x)=g(x)h(x)$ 则根据[[多项式#^MK18|定理]]可得 $\deg(f(x))=\deg(g(x))+\deg(h(x))$
...
根据定义有 $f_i(x)=h_i(x)g(x)$ 则根据[[多项式#^MK8|乘法性质]]交换律，结合律有
...
根据[[多项式#^MK1|定义]]及条件 $\{p(x),f(x)\}\subset K'[x]$ ，则 $\forall i\in\mathbb{N}\forall s\in\mathbb{N}(b_i\in K'[x]\land \sum_{i+j=s}a_ib_j\in K'[x])$

故根据[[线性方程组解#^MK24|数域]]可得 $a_i\in K'[x]$ 则 $h(x)\in K'[x]$

则根据[[多项式#^MK5|相等]]定义可得在数域 $K'[x]$ 上有 $h(x)p(x)=f(x)$ 即满足整除定义

#### 整除关系

##### 定义

根据[[离散数学概论#^MK155|基础定义]]，对于任意 $\{f(x),g(x)\}\subset K[x]$ 要么 $g(x)\mid f(x)$ 要么 $g(x)\nmid f(x)$

则把满足 $g(x)\mid f(x)$ 的元素对组成[[离散数学概论#^MK151|有序组]]

进一步把所有满足条件元素对组成的有序组的集合定义为 $R\subset K[x]\times K[x]$

则此时根据[[离散数学概论#^MK155|基础定义]] $R$ 是集合 $K[x],K[x]$ 上的二元关系
...
- **证明**
  ...
  根据[[多项式#^MK19|定义]]可得存在 $u(x),v(x)$ 使得 $g(x)=u(x)f(x)\land h(x)=v(x)g(x)$

  则有 $h(x)=v(x)(u(x)f(x))$ 根据[[多项式#^MK8|结合律]] $h(x)=(v(x)u(x))f(x)$ 故满足 $f(x)\mid h(x)$
...
- **充分**
  ...
  若 $f(x)\mid g(x)\land g(x)\mid f(x)$ 根据[[多项式#^MK19|定义]]则有 $f(x)=u(x)g(x)\land g(x)=v(x)f(x)$

  若 $f(x)=0(x)$ 是零元，则根据[[多项式#^MK21|零元被式]]可得 $g(x)=0(x)$ 则满足 $f(x)=cg(x)=0(x)$

  若 $f(x)\ne0(x)$ 则 $f(x)=u(x)(v(x)f(x))$ 根据[[多项式#^MK8|乘法性质]]结合律、交换律可得 $1f(x)=(u(x)v(x))f(x)$

  根据[[多项式#^MK8|乘法性质]]消去律可得 $1=u(x)v(x)$ 根据[[多项式#^MK18|次数定理]]可得 $\deg(v(x))\leq\deg(1)=0$
...
- **必要**
  ...
  $f(x)=cg(x)$ 则根据[[线性方程组解#^MK24|数域]] $1\in K\rightarrow \frac{1}{c}\in K$ 则有 $g(x)=\frac{1}{c}f(x)$

  则根据[[多项式#^MK19|定义]]可得 $f(x)\mid g(x)\land g(x)\mid f(x)$
...
对于任意有限[[多项式#^MK17|次数]]的 $\{f(x),g(x)\}\subset K[x]$ 且 $\deg(f(x))\geq\deg(g(x))$ ，带余除法表达一定存在

###### 证明

由于次数有限，故元素根据[[多项式#^MK1|定义]]可表达为 $f(x)=\sum_{i=0}^n a_ix^i,g(x)=\sum_{i=0}^m b_ix^i$ 且 $n\geq m$

- **证明 $m=n$**

  假设 $n=m$ 且由于 $K$ 是[[线性方程组解#^MK24|数域]]且 $\{a_n,b_m\}\subset K$ 故有 $\frac{a_n}{b_m}\in K$

  则有 $f(x)-\frac{a_n}{b_m}g(x)=\sum_{i=0}^{n-1}\left(a_i-\frac{a_n}{b_m}b_i\right)x^i$

  根据[[多项式#^MK20|加法性质]]，两边同加 $\frac{a_n}{b_m}g(x)$ 则有 $f(x)=\frac{a_n}{b_m}g(x)+\sum_{i=0}^{n-1}\left(a_i-\frac{a_n}{b_m}b_i\right)x^i$

  此时 $\deg(r(x))<\deg(g(x))$ 满足带余除法表达[[多项式#^MK23|定义]]
...
- **当 $m+1$ 存在带余除法表达时 $m$ 亦存在**
  ...
  故有 $f(x)=h(x)\left(h'(x)g(x)\right)+r(x)$ 根据[[多项式#^MK8|结合律]]可得 $f(x)=(h(x)h'(x))g(x)+r(x)$

  此时若 $\deg(r(x))<\deg(g(x))$ 则满足带余除法表达

  若 $\deg(r(x))=\deg(g(x))$ 则根据第一步证明，存在带余除法表达 $r(x)=h''(x)g(x)+r'(x)$

  根据[[多项式#^MK2|加法]]和[[多项式#^MK6|乘法]]及其性质可得
  ...
  此时 $\deg(r'(x))<\deg(g(x))$ 故满足带余除法表达[[多项式#^MK23|定义]]
...
必有 $f(x)=r(x)\land f(x)=r'(x)$ 根据[[多项式#^MK20|传递性]]可得 $r(x)=r'(x)$

若 $g(x)\ne0(x)$ 则：

根据[[多项式#^MK20|加法性质]]及[[多项式#^MK8|乘法性质]]等式相减并整理
...
若 $r(x)-r'(x)\ne0(x)$ 则根据[[多项式#^MK19|定义]] $g(x)$ 整除 $r(x)-r'(x)$

根据[[多项式#^MK22|性质]]可得 $\deg(r(x)-r'(x))\geq g(x)$ 由于 $r(x)<g(x)\land r'(x)<g(x)$

根据[[多项式#^MK2|加法]]可得 $\deg(r(x)-r'(x))<g(x)$ 两者矛盾
...
若 $\{g(x),f(x)\}\subset K[x]$ 且 $g(x)\mid f(x)$ 即 $g(x)$ [[多项式#^MK19|整除]] $f(x)$
...
- **证明**
  ...
  根据[[多项式#^MK23|带余除法定义]] $f(x)=h(x)g(x)+0(x)$ 满足带余除法表达

  根据唯一性 $f(x)$ 对 $g(x)$ 的带余除法表达唯一，余式只能为 $0(x)$

##### 定理

若 $\{g(x),f(x)\}\subset K[x]$ 存在 $K\subset E$ 同为[[线性方程组解#^MK24|数域]]，则有 $\{g(x),f(x)\}\subset E[x]$
...
- **充分**
  ...
  假设在 $E[x]$ 上 $g(x)\mid f(x)$ 则根据[[多项式#^MK19|整除定义]]在 $E[x]$ 上存在 $h'(x)$ 使得 $f(x)=h'(x)g(x)=h'(x)g(x)+0(x)$

  根据[[多项式#^MK24|存在性]]可得在 $K[x]$ 上存在带余除法表达式 $f(x)=h(x)g(x)+r(x)$ 其中 $\{h(x),r(x)\}\subset E[x]$

  由于 $K\subset E$ 则有 $\{h(x),r(x)\}\subset E[x]$ 故 $f(x)=h(x)g(x)+r(x)$ 亦为 $E(x)$ 上的表达式。

  根据[[多项式#^MK25|唯一性]]可得 $r(x)=0(x)\land h'(x)=h(x)$ 故在 $K[x]$ 上有 $f(x)=h(x)g(x)$
...
若 $f(x)\in K[x]$ 存在 $g(x)\mid f(x)$ 即[[多项式#^MK19|整除]]，则称 $g(x)$ 是 $f(x)$ 的一个因式

### 公因式 ^MK42

若 $\{f_1(x),...,f_n(x)\}\subset K[x]$ 若 $h(x)\in K[x]$ 满足 $h(x)\mid f_1(x)\land...\land h(x)\mid f_n(x)$

即[[多项式#^MK19|整除]]，则称 $h(x)$ 是 $f_1(x),...,f_n(x)$ 的公因式

### 最大公因式 ^MK26

若 $\{f_1(x),...,f_n(x)\}\subset K[x]$ 存在 $d(x)$ 满足对于任意 $f_1(x),...,f_n(x)$ 的[[多项式#^MK42|公因式]] $h(x)$ 均有 $h(x)\mid d(x)$
...
由于[[多项式#^MK32|性质]]可知最大公因式相伴，故根据[[多项式#^MK36|定理]]任意最大公因式 $a(x)=kb(x)$

则定义 $(f_1(x),...,f_n(x))$ 为最高[[多项式#^MK17|次]]项系数为 $1$ 的[[多项式#^MK26|最大公因式]]
...
2. $p^n(x)\mid f(x)$ 即 $p(x)$ 的 $n$ 次[[多项式#^MK49|幂]]整除 $f(x)$
...
若 $\{f_1(x),...,f_n(x)\}\subset K[x]$ 且其[[多项式#^MK38|首一最大公因式]] $(f_1(x),...,f_n(x))=1$
...
根据[[多项式#^MK19|整除]]可得 $f(x)\mid f(x)\land f(x)\mid 0(x)$ 则 $f(x)$ 为公因式

假设公因式 $h(x)$ 则必有 $h(x)\mid f(x)$

故任意公因式均可整除 $f(x)$ 根据[[多项式#^MK26|定义]]可得 $f(x)$ 为最大公因式

且根据[[多项式#^MK21|零元被试]]，当 $f(x)\ne0(x)$ 时，不成立 $0(x)\mid f(x)$ 故 $0(x)$ 不为公因式
...
根据[[多项式#^MK19|整除]]可得 $\forall f(x)\in K[x]\rightarrow f(x)\mid 0(x)$ 此时 $\deg(f(x))$ 为自然数 $\deg(0(x))=-\infty$

但根据[[多项式#^MK21|性质1]]可得 $0(x)$ 与 $0(x)$ 的最大公因式为 $0(x)$

#### 引理 ^MK29

若 $\{f(x),g(x)\}\subset K[x]$ 假设其[[多项式#^MK23|带余除法]]表达式为 $f(x)=h(x)g(x)+r(x)$ 则有：
...
- **充分**
  ...
  由于 $f(x)=h(x)g(x)+r(x)$ 根据[[多项式#^MK20|加法相等]]两边同加 $h(x)g(x)$ 逆元得 $f(x)-h(x)g(x)=r(x)$

  由于 $c(x)$ 是 $f(x),g(x)$ 的公因式，故有 $c(x)\mid f(x)\land c(x)\mid g(x)$ 根据[[多项式#^MK28|被试和整除]]有 $c(x)\mid f(x)-h(x)g(x)$

  故有 $c(x)\mid r(x)$ 结合 $c(x)\mid g(x)$ 得 $c(x)$ 为 $g(x),r(x)$ 公因式

- **必要**

  由于 $f(x)=h(x)g(x)+r(x)$ 且 $c(x)$ 是 $r(x),g(x)$ 的公因式，故有 $c(x)\mid r(x)\land c(x)\mid g(x)$ 根据[[多项式#^MK28|被试和整除]]有 $c(x)\mid h(x)g(x)+r(x)$
...
- **充分**
  ...
  由于 $d(x)$ 为 $f(x),g(x)$ 的[[多项式#^MK26|最大公因式]]，则有 $c(x)\mid d(x)$
...
- **必要**
  ...
  由于 $d(x)$ 为 $r(x),g(x)$ 的[[多项式#^MK26|最大公因式]]，则有 $c(x)\mid d(x)$
...
对于任意有限[[多项式#^MK17|次]]一元多项式 $\{f(x),g(x)\}\subset K[x]$ ，根据[[多项式#^MK24|存在性]]必有[[多项式#^MK23|带余除法]]表达式 $f(x)=h_1(x)g(x)+r_1(x)$ 其中 $\{h_1(x),r_1(x)\}\subset K[x]$ 且 $\deg(r_1(x))<\deg(g(x))$
...
根据[[多项式#^MK30|零次多项式]]可得当 $\deg(r_n(x))=0$ 时必满足 $r_n(x)\mid r_{n-1}(x)$

根据[[多项式#^MK17|次数]]定义 $\deg(g(x))$ 为自然数，且有限。 $\deg(r_i(x))$ 定义域为 $N\cup-\infty$

由于每次带余除法余式的次数均下降，次数不为负，且当除式次数为零时余式为[[多项式#^MK3|零元]]。

故必然在有限次后有 $r_s(x)$ 满足 $\deg(r_s(x))\geq0\land r_{s-1}(x)=h_s(x)r_s(x)+0(x)$

此时 $r_{s+1}(x)=0(x)$ 则根据[[多项式#^MK21|性质1]]可得 $r_s(x),0(x)$ 最大公因式为 $r_s(x)$ 进一步根据[[多项式#^MK29|引理]]中（2）可得 $r_s(x),r_{s-1}(x)$ 最大公因式为 $r_s(x)$
...
对于任意 $\{f(x),g(x)\}\subset K[x]$ 若 $c(x)$ 为其[[多项式#^MK26|最大公因式]]，则必有 $c(x)=u(x)f(x)+v(x)g(x)$
...
且 $r_1(x)=f(x)-h_1(x)g(x),r_2(x)=g(x)-h_2(x)r_1(x)$ 进一步根据[[多项式#^MK2|加法]]和[[多项式#^MK6|乘法]]的性质可得
...
由于一元多项式 $K[x]$ [[多项式#^MK15|是环]]，且根据[[多项式#^MK10|定义]]加法，乘法封闭，故 $u_i(x)\in K[x]\land v_i(x)\in K[x]$

故经过有限次代换后 $c(x)$ 必可有表达式 $u(x)f(x)+v(x)g(x)$ 其中 $\{u(x),v(x)\}\in K[x]$

#### 最大公因式相伴 ^MK32

若 $a(x),b(x)$ 均为 $X\subset K[x]$ 的最大公因式，则 $a(x),b(x)$ [[多项式#^MK31|相伴]]，即 $a(x)=kb(x)$

##### 证明

根据[[多项式#^MK26|定义]]可得 $a(x)\mid b(x)\land b(x)\mid a(x)$ 符合相伴定义

#### 首一最大公因式不随数域扩大改变 ^MK37

若有[[线性方程组解#^MK24|数域]] $E,K$ 满足 $K\subset E$ 有 $\{f(x),g(x)\}\subset K[x]$

则 $f(x),g(x)$ 在 $K[x]$ 上的首一最大公因式等于 $f(x),g(x)$ 在 $E[x]$ 上的[[多项式#^MK38|首一最大公因式]]

##### 证明

分别在 $K[x],E[x]$ 上运用[[多项式#^MK33|辗转相除]]求最大公因式，根据带余除法表达式[[多项式#^MK25|唯一性]]可得最终获得相同结果 $r_s(x)$

则根据[[多项式#^MK32|最大公因式相伴]]可得 $K[x]$ 上任意最大公因式皆可表达为 $kr_s(x)$ ，同理 $E[x]$ 上任意最大公因式可表达为 $er_s(x)$
...
若 $\{f(x),g(x)\}\subset K[x]$ 且 $f(x),g(x)$ [[多项式#^MK34|互素]]则其充要条件为存在 $\{u(x),v(x)\}\subset K[x]$
...
故根据[[多项式#^MK35|推论2]]可得存在 $\{u(x),v(x)\}\subset K[x]$ 满足 $u(x)f(x)+v(x)g(x)=1$

###### 必要

若 $u(x)f(x)+v(x)g(x)=1$ 则定义 $c(x)$ 为 $f(x),g(x)$ 的最大公因式

根据[[多项式#^MK28|整除性质]]可得 $c(x)\mid u(x)f(x)+v(x)g(x)\rightarrow c(x)\mid 1$

同理根据[[多项式#^MK30|整除性质]]可得 $1\mid c(x)$ 故满足[[多项式#^MK31|相伴]]，则根据[[多项式#^MK36|定理]]有 $c(x)=k1=k$
...
若 $f(x),g(x)$ 在 $K[x]$ 上[[多项式#^MK34|互素]]，则其在 $K[x]$ 上[[多项式#^MK38|首一最大公因式]]为 $1$

根据[[多项式#^MK37|性质]]可得其在 $E[x]$ 上的首一最大公因式亦为 $1$

则根据定义 $f(x),g(x)$ 在 $E[x]$ 上[[多项式#^MK34|互素]]
...
根据[[多项式#^MK39|互素充要条件]]可得存在 $\{u(x),v(x)\}\subset K[x]$ 满足 $u(x)f(x)+v(x)g(x)=1$

则两边同乘 $h(x)$ 得 $u(x)f(x)h(x)+v(x)g(x)h(x)=h(x)$

根据[[多项式#^MK8|乘法交换律]]可得 $u(x)h(x)f(x)+v(x)g(x)h(x)=h(x)$

由于 $f(x)\mid g(x)h(x)\land f(x)\mid f(x)$ 则根据[[多项式#^MK28|被试和]]可得 $f(x)\mid u(x)h(x)f(x)+v(x)g(x)h(x)\rightarrow f(x)\mid h(x)$

#### 互素性质2 ^MK76

若 $\{f(x),g(x),h(x)\}\subset K[x]$ 满足 $f(x)\mid h(x)\land g(x)\mid h(x)$ 且 $f(x),g(x)$ [[多项式#^MK34|互素]]，则 $f(x)g(x)\mid h(x)$

##### 证明

由于[[多项式#^MK19|整除]]可得 $h(x)=a(x)f(x)\land h(x)=b(x)g(x)$

根据[[多项式#^MK39|互素充要条件]]可得 $u(x)f(x)+v(x)g(x)=1$
...
若 $\{f(x),g(x),h(x)\}\subset K[x]$ 满足 $f(x),h(x)$ [[多项式#^MK34|互素]] 且 $g(x),h(x)$ [[多项式#^MK34|互素]]，则有 $f(x)g(x),h(x)$ [[多项式#^MK34|互素]]

##### 证明

根据[[多项式#^MK39|互素充要条件]]可得
...
满足 $a(x)g(x)f(x)+b(x)h(x)=1$ 故根据[[多项式#^MK39|互素充要条件]]可得 $f(x)g(x),h(x)$ [[多项式#^MK34|互素]]

##### 推论

若 $f_i(x)\in K[x]\ i=1,2,...,n$ 与 $h(x)\in K[x]$ [[多项式#^MK34|互素]]，则有 $\prod_{i=1}^nf_i(x),h(x)$ [[多项式#^MK34|互素]]

## 可约

### 不可约元素 ^MK41

若 $f(x)\in K[x]$ 且 $f(x)$ 的[[多项式#^MK40|因式]]仅有零[[多项式#^MK17|次]]多项式和他本身的[[多项式#^MK31|相伴]]元素，则称 $f(x)$ 为不可约元素
...
2. 对于任意 $f(x)\in K[x]$ 其与 $p(x)$ 的[[多项式#^MK42|公因式]]只能为零[[多项式#^MK17|次]]多项式，或 $p(x)$ 的[[多项式#^MK31|相伴]]元素
...
则有 $c(x)\mid p(x)$ 故 $c(x)$ 为 $p(x)$ 的[[多项式#^MK40|因式]]，根据[[多项式#^MK41|定义]]不可约，故与条件冲突。

##### （2）到（3）

若 $p(x)\mid f(x)$ 则满足条件

若 $p(x)\nmid f(x)$ 根据（1）有 $f(x),g(x)$ 公因式为零次多项式，即 $(f(x),g(x))=1$ [[多项式#^MK34|互素]]

则根据[[多项式#^MK43|互素性质]]可得 $p(x)\mid g(x)$

##### （3）到（4）

由于 $p(x)=f(x)g(x)$ 则有 $p(x)\mid f(x)g(x)$

根据[[多项式#^MK18|定理]]可得 $\deg(p(x))=\deg(f(x)g(x))=\deg(f(x))+\deg(g(x))$

根据（3）可得必有一个可以被 $p(x)$ 整除，设为 $f(x)$

则有 $p(x)\mid f(x)$ 根据[[多项式#^MK22|整除性质]]可得 $\deg(p(x))\leq\deg(f(x))$
...
对于任意 $c(x)\in K[x]$ 且是 $p(x)$ 的[[多项式#^MK40|因式]]，则有 $c(x)\mid f(x)$ 根据[[多项式#^MK19|定义]]可得存在 $f(x)\in K[x]$ 满足 $p(x)=f(x)c(x)$

若 $c(x)=0(x)\lor f(x)=0(x)$ 则有 $p(x)=0(x)$ 则 $\deg(p(x))=-\infty<0$ 与其定义不符

故 $c(x)\ne0(x)\land f(x)\ne0(x)$ 即 $\deg(c(x))\geq0\land\deg(f(x))\geq0$

根据[[多项式#^MK18|定理]]可得 $\deg(p(x))=\deg(f(x))+\deg(c(x))$
...
设 $p(x)\in K[x]$ 且[[多项式#^MK41|不可约]]，若 $p(x)\mid \prod_{i=1}^n f_i(x),f_i(x)\in K[x]\land n<\infty$ 即[[多项式#^MK19|整除]] $n$ 个多项式乘积

则存在 $1\leq s\leq n\rightarrow p(x)\mid f_s(x)$

##### 证明

由于一元多项式是[[多项式#^MK15|环]]，根据[[多项式#^MK10|环定义]]（7）可得乘法封闭，故有 $\prod_{i=1}^n f_i(x)=f_1(x)\prod_{i=2}^n f_i(x)$
...
在 $K[x]$ 中一[[多项式#^MK17|次]]多项式[[多项式#^MK41|不可约]]

#### 推论3 ^MK47

设 $p(x)\in K[x]$ 且[[多项式#^MK41|不可约]]，若 $p(x)=\prod_{i=1}^n f_i(x),f_i(x)\in K[x]\land n<\infty$
...
根据定理中（4）及[[多项式#^MK18|定理]]可得必有 $\deg(p(x))=\deg(f_1(x))\lor \deg(p(x))=\deg\left(\prod_{i=2}^n f_i(x)\right)$

若为前者，则根据 $\deg(p(x))=\deg(f_1(x))+\deg\left(\prod_{i=2}^n f_i(x)\right)$ 可得 $\deg\left(\prod_{i=2}^n f_i(x)\right)=0$

故根据[[多项式#^MK17|次数]]定义有 $\prod_{i=2}^n f_i(x)=d\land d\in K$ 则有 $p(x)=df_1(x)$

若为后者，则根据 $\deg(p(x))=\deg(f_1(x))+\deg\left(\prod_{i=2}^n f_i(x)\right)$ 可得 $\deg(f_1(x))=0$

故根据[[多项式#^MK17|次数]]定义有 $f_1(x)=c_1\land c_1\in K$ 则有 $p(x)=c_1\prod_{i=2}^nf_i(x)=c_1f_2(x)\prod_{i=3}^nf_i(x)$
...
定义 $f(x)\in K[x]\land\deg(f(x))\geq 0$ 为有限[[多项式#^MK17|次]]多项式，则：

1. 存在有限个[[多项式#^MK41|不可约]]多项式 $p_1(x),...,p_n(x)\ \ n<\infty$ 满足 $f(x)=\prod_{i=1}^n p_i(x)$
...
2. $\forall i(1\leq i\leq m\rightarrow \exists j(1\leq j\leq n\land q_i(x)\sim p_j(x)))$ 其中 $\sim$ 为[[多项式#^MK31|相伴]]符号
...
根据[[多项式#^MK41|定义]]可得若 $f(x)\in K[x]$ 不可约，则 $f(x)=f(x)$ 满足条件

若 $f(x)$ 可约，由于 $\deg(f(x))\geq 0$ 及 $f(x)$ 不是不可约，根据[[多项式#^MK45|定理]]不满足（4）
...
并且每次分解次数严格下降一个正整数，故分解只能进行有限次。根据[[多项式#^MK45|定理]]中（4）可得所有可约元素均可分解。

故存在有限个[[多项式#^MK41|不可约]]多项式 $p_1(x),...,p_n(x)\ \ n<\infty$ 满足 $f(x)=\prod_{i=1}^n p_i(x)$
...
则根据[[多项式#^MK47|推论3]]可得必存在一个 $q_s(x)$ 满足 $p_i(x)=cq_s(x)$

故有 $p_1(x)\sim q_s(x)$

###### 当 $n-1$ 成立时 $n$ 成立

由于一元多项式是[[多项式#^MK15|环]]，根据[[多项式#^MK10|环定义]]（7）可得加乘法封闭

由于 $\deg(f(x))\geq0\rightarrow f(x)\ne0(x)$ 则 $p_i(x)\ne0(x)\land q_i(x)\ne0(x)$

此时 $f(x)=\prod_{i=1}^n p_i(x)=\prod_{i=1}^m q_i(x)=p_1(x)\prod_{i=2}^n p_i(x)=\prod_{i=1}^m q_i(x)$

由于 $p_1(x)$ 不可约，根据[[多项式#^MK46|推论1]]可得存在 $1\leq s\leq m\rightarrow p_1(x)\mid q_s(x)$

此时，不妨设 $s=1$ 则有 $p_1(x)\mid q_1(x)$ 又根据 $q_1(x)$ [[多项式#^MK41|不可约]]则有 $\deg(p_1(x))=0\lor p_1(x)\sim q_1(x)$

又由于 $p_1(x)$ 不可约，则必有 $p_1(x)\sim q_1(x)$ 故 $p_1(x)=cq_1(x)$

则根据 $cq_1(x)\prod_{i=2}^n p_i(x)=q_1(x)\prod_{i=2}^m q_i(x)$ 以及[[多项式#^MK8|乘法]]交换，结合，消去率有 $c\prod_{i=2}^n p_i(x)=\prod_{i=2}^m q_i(x)$
...
根据[[多项式#^MK48|上一步证明]]可知对于任意 $f(x)\in K[x]$ 均存在 $f(x)=\prod_{i=1}^n p_i(x)$ 且 $n<\infty$ 有限

则根据数学归纳法 $f(x)=\prod_{i=1}^n p_i(x)=\prod_{i=1}^m q_i(x)$ 必满足唯一性

### 标准分解式

若 $f(x)\in K[x]$ 若因式分解 $f(x)=c\prod_{i=1}^n p_i^{l_i}(x)$ 满足 $p_i(x)$ 为最高[[多项式#^MK17|次]]项系数为 $1$ 的[[多项式#^MK41|不可约]]多项式，且 $l_i>0$

则称该式为 $f(x)$ 的标准分解式

## 根 ^MK52

若 $f(x)\in K[x]$ 存在 $c\in K$ 满足 $(x-c)\mid f(x)$ 即[[多项式#^MK19|整除]]，则称 $c$ 是 $f(x)$ 的一个跟
...
3. 经过运算后得结果为[[多项式#^MK3|零元]]

#### 证明

根据定义存在 $h(x)\in K[x]$ 满足 $f(x)=(x-c)h(x)$

则当 $x=c$ 时有 $f(c)=(c-c)h(c)$ 根据[[多项式#^MK53|减法]]可得 $c-c=0[x]$ 即零多项式

根据[[多项式#^MK3|零元]]可得 $0(x)h(c)=0(x)$ 则有 $f(c)=0(x)$
...
根据[[多项式#^MK1|定义]]可得 $x-c=x+(-1)c\in K[x]$

根据带余除法[[多项式#^MK24|存在性]]必有 $f(x)=g_1(x)(x-c)+r_1(x)$ 其中 $\deg(r_1(x))<\deg(f(x))$
...
故有 $f(x)=\sum_{i=1}^{s-1}g_i(x)(x-c)$ 根据[[多项式#^MK8|乘法]]分配率可得 $f(x)=(x-c)\left(\sum_{i=1}^{s-1}g_i(x)\right)$

由于一元多项式是[[多项式#^MK15|环]]，根据[[多项式#^MK10|环定义]]（7）可得加乘法封闭

故有 $x-c\in K[x]\land \sum_{i=1}^{s-1}g_i(x)\in K[x]$ 则根据[[多项式#^MK19|定义]]可得 $x-c\mid f(x)$
...
根据[[多项式#^MK50|唯一因式分解]]可得必有 $f(x)=\prod_{i=1}^mp_i(x)$ 其中 $p_i(x)$ [[多项式#^MK41|不可约]]

根据[[多项式#^MK51|推论2]]可得一次多项式 $x+b$ 不可约，固若 $(x+b)\mid f(x)\rightarrow (x+b)\mid \prod_{i=1}^mp_i(x)$

则根据因式分解唯一性必有 $p_s(x)\sim (x+b)$ 即根据[[多项式#^MK31|相伴]] $p_s=c(x+b)$
...
则根据[[多项式#^MK18|次数定理]]可得 $\deg(f(x))=n+\sum_{i=n+1}^m\deg(p_i(x))$

由于次数均大于等于零，故得 $\max(n)=\deg(f(x))$ 证毕

#### 推论1

若 $f(x)\in K[x]$ 且 $\deg(f(x))\leq n$ 却有 $n+1$ 个[[多项式#^MK52|根]]，则 $f(x)=0(x)$
...
定义 $h(x)=f(x)-g(x)$ 根据[[多项式#^MK53|减法]]定义 $h(x)=f(x)+(-1)g(x)$

则根据[[多项式#^MK18|次数定理]]可得 $\deg(h(x))\leq\max(\deg(f(x)),\deg(g(x))\leq n$

又根据条件得 $h(c_i)=g(c_i)-f(c_i)=0(x)$ 则 $c_i$ 为 $h(x)$ 的根

由于有 $n+1$ 个根且 $\deg(h(x))\leq n$ 则 $h(x)=0(x)$

根据[[多项式#^MK54|减法性质]]可得 $f(x)=g(x)$

## 复数域上不可约多项式

### 代数基本定理 ^MK74

任何[[多项式#^MK17|次数]]大于 $0$ 的[[多项式#^MK1|一元多项式]] $f(x)$ 在复数域上有[[多项式#^MK52|根]]
...
假设 $f(x)=\sum_{i=0}^da_ix^i\in K[x]$ 为[[多项式#^MK1|多项式表达式]]，若定义 $\tau\in K$ 是定义域为数域 $K$ 的自变量

将 $\tau$ 替换 $x$ 引入数域 $K$ 上的乘法，则显然根据[[离散数学概论#^MK194|函数定义]] $f(\tau)=\sum_{i=0}^da_i\tau^i$ 为函数
...
根据[[多项式#^MK10|环]]的定义可证明 $<K_{pol},+,*>$ 组成环，其中零元为 $f(\tau)=0$
...
显然根据[[多项式#^MK68|定义]] $g(x),f(x)$ 均诱导出次数与系数相同的多项式函数 $f(\tau),g(\tau)$

则有 $f(\tau)-g(\tau)=0$ 故对于任意 $\tau\in K$ 均有 $f(\tau)=g(\tau)$

即根据[[离散数学概论#^MK194|函数]]可得 $\sigma:f(x)\to f(\tau)$ 是函数映射

###### 证明是同构映射

- **证明是双射**

  根据 $K_{pol}$ 的[[多项式#^MK68|定义]]可得对于任意 $f(\tau)\in K_{pol}$ 均由 $f(x)\in K$ 诱导，即满足 $\sigma(f(x))=f(\tau)$

  则根据[[离散数学概论#^MK198|满射]]可得 $\sigma$ 为满射
  ...
  由于不[[多项式#^MK5|相等]]，故不满足 $d_1=d_2\land a_i=b_i$

  根据 $K_{pol}$ 的[[多项式#^MK67|加法乘法]]可得 $f(\tau)-g(\tau)=\sum_{i=0}^{\max(d_1,d_2)}(a_i-b_i)\tau^i$ 故 $f(\tau)-g(\tau)\ne0$

  则不满足 $\forall\tau\rightarrow f(\tau)-g(\tau)=0$ 根据[[多项式#^MK65|相等定义]]可得 $f(\tau)\ne g(\tau)$

  则根据[[离散数学概论#^MK195|单射]]可得 $\sigma$ 为单射
...
- **证明保持加乘法**
  ...
  根据多项式[[多项式#^MK2|加法]]可得 $f(x)+g(x)=\sum_{i=0}^{\max(d_1,d_2)}(a_i+b_i)x^i=h(x)$

  根据 $K_{pol}$ [[多项式#^MK67|加法]]可得 $f(\tau)+g(\tau)=\sum_{i=0}^{\max(d_1,d_2)}(a_i+b_i)\tau^i=h(\tau)$

  根据[[多项式#^MK68|定义]]可得 $h(\tau)$ 为 $h(x)$ 诱导的函数，则有 $\sigma(f(x)+g(x))=\sigma(h(x))=h(\tau)=f(\tau)+g(\tau)$

  即保持加法。同理可证保持乘法

- **结论** ^MK72

  即 $\sigma$ 是[[多项式#^MK13|同构映射]]，则根据[[多项式#^MK71|推论]]若 $c$ 是 $f(x)$ 的根，则有 $f(x)=(x-c)h(x)$

  根据[[多项式#^MK70|同构性质]]有 $\sigma(f(x))=\sigma(x-c)\sigma(h(x))\Rightarrow f(\tau)=(x-c)h(\tau)=0$

  即数域 $K$ 上的多项式 $f(x)$ 有根 $c$ 等价于系数 $a_i\in K$ 的多项式函数 $f(\tau)$ 当 $\tau=c$ 时函数值为零

##### 证明定理

若 $K=\mathbb{C}$ 是复数域，若 $f(x)$ 无根，则根据[[多项式#^MK72|结论]] $f(\tau)$ 在 $\tau\in\mathbb{C}$ 上恒不等于零

则定义函数 $\Phi(\tau)=\frac{1}{f(\tau)}$ 则根据[[复变函数#^MK94|基本公式]]可得 $$\Phi'(\tau)=-\frac{f'(\tau)}{(f(\tau))^2}$$

且根据导数公式在复数域上多项式函数 $f'(\tau)$ 导数存，结合 $f(\tau)\ne0$ 可得在复数域上 $\Phi'(\tau)$ 存在

则根据[[复变函数#^MK65|解析]]可得函数 $\Phi(\tau)$ 解析。

根据[[复变函数#^MK91|性质]]以及[[复变函数#^MK57|函数极限]]可得 $\lim_{|\tau|\to\infty}\frac{1}{\tau^i}=0,i\in\mathbb{N}$
...
当 $|\tau|\leq M$ 时，根据[[复变函数#^MK80|可导必连续]]定义可得多项式函数连续

又由于 $|\tau|\leq M$ 有界闭区间，故根据[[Euclid空间上的极限与连续#^MK4|有界性定理]]可得有界，最终得 $\Phi(\tau)$ 有界且解析。

故根据[[复变函数#^MK95|刘维尔定理]]可得 $\Phi(\tau)$ 必为常值函数。即 $\Phi(\tau)=A$

则有 $f(\tau)=\frac{1}{A}$ 为零[[多项式#^MK17|次]]多项式函数。根据[[多项式#^MK68|定义]]为零次多项式诱导的函数。

即当且仅当 $f(x)=a_0\ne0$ 为零次多项式时，多项式无[[多项式#^MK52|根]]
...
根据代数基本定理 $f(x)$ 有根，则根据[[多项式#^MK73|Bezout定理]] $f(x)$ 有一次因式，则根据[[多项式#^MK41|可约]]
...
若 $f(x)=\sum_{i=0}^na_ix^i\in\mathbb{R}[x]$ 此时根据[[多项式#^MK72|多项式函数同构]]可得若 $c$ 是 $f(x)$ 的根，则 $f(c)=0$

根据[[线性方程组解#^MK37|推论2]]可得最大数域为 $\mathbb{C}$ 则 $c\in\mathbb{C}$ ，故 $c$ 存在[[复变函数#^MK93|共轭复数]]

取 $c$ 的共轭复数 $\overline{c}$ 带入亦有 $f(\overline{c})=0$ 即等价于 $\overline{c}$ 亦是多项式的根

#### 证明

对于函数 $\sum_{i=0}^na_ic^i=0$ 两边取共轭复数，则根据[[复变函数#^MK96|性质]]中加法乘法保持可得
...
由于 $0$ 的共轭复数为本身，故有 $\sum_{i=0}^na_i\overline{c}^i=0$ 即 $f(\overline{c})=0$ 则根据[[多项式#^MK72|多项式函数同构]]可得 $\overline{c}$ 亦为根。
...
定义 $p(x)\in\mathbb{R}[x]$ 为 $\deg(p(x))\geq1$ 的[[多项式#^MK41|不可约]]多项式，则根据[[多项式#^MK74|代数基本定理]]当 $x\in\mathbb{C}$ 时 $p(x)$ 存在根

定义为 $c$ ，若 $c$ 为实数，则根据[[多项式#^MK52|定义]]可得 $p(x)=(x-c)h(x)$ 则根据[[多项式#^MK41|不可约]]得 $p(x)\sim(x-c)$

即 $p(x)=a(x-c)$ 为一次多项式

若 $c$ 虚部不为零，则根据[[多项式#^MK75|命题]]可得 $\overline{c}$ 亦为根，固有 $(x-c)\mid p(x)\land (x-\overline{c})\mid p(x)$

由于 $x-c,x-\overline{c}$ 满足[[多项式#^MK34|互素]]，故根据[[多项式#^MK76|互素性质2]]可得 $(x-c)(x-\overline{c})\mid p(x)$

则根据[[多项式#^MK19|整除定义]]可得 $p(x)=(x-c)(x-\overline{c})\prod_{i=1}^nf_i(x)=(x-c)(x-\overline{c})h'(x)$

则有 $(x-c)(x-\overline{c})=x^2-(c+\overline{c})x+c\overline{c}$ 根据[[复变函数#^MK93|共轭复数]]定义，若 $c=a+ib$ 则有 $\overline{c}=c-ib$
...
根据[[多项式#^MK77|整除与数域无关]]可得在数域 $\mathbb{R}$ 上有 $p(x)=(x^2-(c+\overline{c})x+c\overline{c})h'(x)$

由于 $p(x)$ 与 $\mathbb{R}$ 上[[多项式#^MK41|不可约]]，则 $p(x)\sim x^2-(c+\overline{c})x+c\overline{c}$ 即 $p(x)=a(x^2-(c+\overline{c})x+c\overline{c})$
...
若 $f(x)\in\mathbb{Q}$ 即有理数义[[线性方程组解#^MK24|数域]]多项式，则必有本源多项式与之相伴。

##### 证明

由于 $a_i\in\mathbb{Q}$ 故有 $a_i=\frac{q_i}{p_i}$ 取 $\{p_i,...,p_n\}$ 的最小公倍数 $P$

则 $a_i=\frac{1}{P}\frac{Pq_i}{p_i}$ 故根据[[多项式#^MK4|数乘]]可得 $f(x)=\frac{1}{P}\sum_{i=0}^n\frac{Pq_i}{p_i}x^i$
...
故根据[[多项式#^MK31|相伴]]定义证得。

#### 推论2 ^MK82

若两个本源多项式 $\{f(x),g(x)\}\subset\mathbb{Q}[x]$ [[多项式#^MK31|相伴]]，则其充要条件为 $f(x)=\pm g(x)$
...
根据[[多项式#^MK4|数乘]]可得 $pf(x)=qg(x)$ 根据[[多项式#^MK5|相等]]可得 $pa_i=qb_i$ 由于本源多项式，故 $p,q,a_i,b_i$ 均为整数
...
根据[[多项式#^MK6|乘法]]有 $h(x)=f(x)g(x)=\sum_{s=0}^{m+n}\left(\sum_{i+j=s}a_ib_j\right)x^s=\sum_{s=0}^{m+n}c_sx^s$
...
定义 $f(x)\in\mathbb{Q}[x]$ 为[[多项式#^MK79|本源多项式]]且[[多项式#^MK41|可约]]的充要条件是 $f(x)$ 可表示为两个次数比他低的本源多项式乘积

##### 证明

###### 充分

已知 $f(x)$ 可约，则必有 $f(x)=g(x)h(x)$ 根据[[多项式#^MK80|推论1]]可得 $g(x)=Ag'(x),h(x)=Bh'(x)$

其中 $\{A,B\}\subset\mathbb{Q}$ 且 $g'(x),h'(x)$ 为本源多项式，则 $f(x)=ABg'(x)h'(x)$

根据[[多项式#^MK81|高斯引理]]可得 $h'(x)g'(x)$ 亦为本源多项式，根据[[多项式#^MK31|相伴定义]]可得 $f(x)\sim h'(x)g'(x)$

根据[[多项式#^MK82|推论2]]有 $AB=\pm 1$ 显然 $-h'(x)$ 亦为本源多项式，故充分性证毕。

###### 必要

已知 $f(x)=h(x)g(x)$ 其中 $h(x),g(x)$ 为本源多项式，根据[[多项式#^MK41|可约定义]]可约

#### 定理2 ^MK84

定义 $f(x)\in\mathbb{Q}[x]$ 是整系数多项式，则[[多项式#^MK41|可约]]的充要条件是可以分解成两个次数比他低的整系数多项式。

##### 证明

根据[[多项式#^MK79|定义]]可得若 $f(x)$ 不为本源多项式，则存在整数 $r\in\mathbb{N}$ 使得 $f(x)=rh(x)$

其中 $h(x)$ 为本源多项式，则 $f(x)$ 可约等价于 $h(x)$ 可约。

根据[[多项式#^MK83|定理1]]可得等价于 $h(x)=h_1(x)h_2(x)$ 两个本源多项式。
...
则 $f(x)$ 是有理数域上[[多项式#^MK41|不可约]]

#### 证明

假设 $f(x)$ 可约，则根据[[多项式#^MK84|定理2]]必有 $f(x)=h(x)g(x)$ 分解为两个整系数多项式

定义 $f(x)=\left(\sum_{i=0}^mb_ix^i\right)\left(\sum_{i=0}^lc_ix^i\right)$ 根据[[多项式#^MK6|乘法]]可得 $f(x)=\sum_{s=0}^n\left(\sum_{i+j=s}b_ic_j\right)x^s$
...
其中 $0\leq k\leq m$ 根据[[多项式#^MK18|定理]]可得 $l+m=n$ 由于 $l>0$ 故有 $m<n\rightarrow k<n$
...
有理[[线性方程组解#^MK24|数域]]上存在任意次不可约多项式
...
设 $f(x)$ 可约，则根据[[多项式#^MK84|定理2]]可得 $f(x)=h_1(x)h_2(x)\land \deg(f(x))>\deg(h_1(x))$
...
根据[[离散数学概论#^MK153|笛卡尔积]]以及[[离散数学概论#^MK155|基础定义]]定义关系 $R=\left\{\left<a,b\right>\in Z\times Z\big|a\mod m=b\mod m\right\}$

根据[[离散数学概论#^MK740|等价关系]]可知 $R$ 满足自反性，对称性以及传递性，故 $R$ 为等价关系

则根据[[数理逻辑#^MK179|定理1]] $R$ 的[[离散数学概论#^MK177|等价类]]划分 $\mathbb{Z}$
...
故定义集合 $U=\left\{\overline{x}\big|x\in\mathbb{Z}\right\}$ 由于 $m$ 的余数是小于 $m$ 的整数，故 $U$ 的[[离散数学概论#^MK3|基数]]是 $|U|=m$
...
根据[[抽象代数#^MK292|运算]]可知运算是[[离散数学概论#^MK194|函数]]，必须满足唯一性。
...
故 $<U,+,\times>$ 是环，进一步显然有乘法交换律，故是交换环。进一步有 $\forall a\left(\overline{a}\in\mathbb{Z}\rightarrow\overline{1}\times\overline{a}=\overline{a}\right)$ 即 $\overline{1}$ 是[[多项式#^MK10|环]]中的乘法幺元。
...
若 $\mathbb{Z}_m=<U,+,\cdot>$ 是一个 $m$ 的[[多项式#^MK85|剩余类环]]，若 $m$ 是素数，则 $Z_m$ 是[[线性方程组解#^MK24|域]]
...
根据[[多项式#^MK29|引理]]可得存在 $\{u,v\}\subset\mathbb{Z}$ 使得 $um+vx=1$ ，则根据[[多项式#^MK85|定义]]中的加法乘法可得
...
又根据[[多项式#^MK85|定义]]可得剩余类环 $\mathbb{Z}_m$ 是交换环，故根据[[线性方程组解#^MK24|域]]定义可得 $\mathbb{Z}_m$ 是域

## 定理2

若 $\mathbb{Z}_m=<U,+,\cdot>$ 是一个 $m$ 的[[多项式#^MK85|剩余类环]]，若 $m$ 是合数，则 $Z_m$ 不是[[线性方程组解#^MK24|域]]
...
故对于 $\overline{a}\in\mathbb{Z}_m\land\overline{a}\ne\overline{0}$ 不存在乘法[[抽象代数#^MK302|逆元]]，故不是[[线性方程组解#^MK24|域]]
