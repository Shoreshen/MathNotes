...
假设 $M=\{a_1,...,a_n\}$ 是[[~/文档/note/离散数学/lssx.org::MK82][有限集]]，则定义[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]] $\phi=f_+(\prod_{i=1}^{n}f_+(x_i,f_-(x_0)),c_1)$

易证 $\phi$ 存在[[~/文档/note/数理逻辑/sllj.org::MK396][等价]]的[[MK336][一元多项式]] $\psi$ ，对于任意 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]]满足 $v(x_i)=a_i$ 由于是[[~/文档/note/离散数学/lssx.org::MK82][有限集]]，则易证有 $\left<\mathcal{M},v\right>\vDash((f_+(x_j,x_0)\hat{=}c_0)$
...
即存在一个[[MK336][一元多项式]] $\phi$ 满足 $\mathcal{M}\vDash(\forall x_0(\lnot(\psi\hat{ =}c_0)))[a_1,...,a_n]$ 与[[MK411][公理]]矛盾，故假设不成立，即 $M$ 是[[~/文档/note/离散数学/lssx.org::MK83][无限集]]
