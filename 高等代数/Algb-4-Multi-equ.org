...
（3） 存在加法[[~/文档/note/抽象代数/cxds.org::MK299][幺元]]： $\exists 0\in U\rightarrow\left((\forall a\in U)\rightarrow(a+0=0+a=a)\right)$ 记作零元

（4） 存在加法[[~/文档/note/抽象代数/cxds.org::MK302][逆元]]： $\forall a\in U(\exists b\in U\rightarrow a+b=b+a=0)$ 其中 $0$ 是加法幺元 
...
若 $U$ 是环，存在元素 $e\in U$ 与环中的乘法满足[[~/文档/note/抽象代数/cxds.org::MK299][幺元]]定义，则称 $e$ 为单位元

且根据[[~/文档/note/抽象代数/cxds.org::MK300][性质]]可得 $e$ 唯一

** 零元定理<<MK9>>

任意[[MK12][交换环]] $\mathcal{U}=\left<U,+,\cdot\right>$ 存在乘法[[~/文档/note/抽象代数/cxds.org::MK301][零元]]，且等于[[MK10][环定义]]中加法幺元
...
（1） 根据[[~/文档/note/离散数学/Disc_Math.org::MK194][函数定义]]， $f(x)$ 到 $f(\tau)$ 的关系形成函数
...
由于 $\sigma$ 为[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]，故必为单射 $\forall a\forall a'(\{a,a'\}\subset K\land a\ne a'\rightarrow \sigma(a)\ne\sigma(a'))$
...
定义[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$ ，若有表达式形式为 $f(x)=\sum_{i=0}^d a_ix^i$ ，其中 $a_i\in K$ ，其中 $x$ 是变元，可以有定义域
...
则根据[[~/文档/note/离散数学/Disc_Math.org::MK186][元]]集合 $I$ 的最大元为多项式 $f(x)$ 的次数，记作 $\deg(f(x))$
...
根据[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK1][线性空间定义]]，定义集合 $V$ 为所有[[MK1][一元多项式]]组成的集合，则 $V$ 是线性空间
...
根据[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK22][子集线性无关]]，取任意 $\{x^{i_1},...,x^{i_n}\}\subset\{x^0,...,x^n,...\}$ 其中 $n\leq\infty$
...
由于之前证明得 $V$ 的一组基为 $\{x^0,...,x^n,...\}$ ，则根据[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK51][线性空间的维数]]可得 $\dim(V)=\infty$
...
（4） 根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]可得 $-1\in K$ 则结合乘法 $\forall \sum_{i=0}^\infty a_ix^i\in K[x]$ 均有 $(-1)\sum_{i=0}^\infty a_ix^i=\sum_{i=0}^\infty -a_ix^i\in K[x]$
...
零次多项式定义为 $b=a_0x^0\land b\ne 0$ 根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $1\in K$ 则有 $\frac{1}{b}\in K$ 同理 $\frac{1}{b}$ 亦为零次一元多项式
...
故根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]可得 $a_i\in K'[x]$ 则 $h(x)\in K'[x]$
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK155][基础定义]]，对于任意 $\{f(x),g(x)\}\subset K[x]$ 要么 $g(x)\mid f(x)$ 要么 $g(x)\nmid f(x)$

则把满足 $g(x)\mid f(x)$ 的元素对组成[[~/文档/note/离散数学/Disc_Math.org::MK151][有序组]]

进一步把所有满足条件元素对组成的有序组的集合定义为 $R\subset K[x]\times K[x]$

则此时根据[[~/文档/note/离散数学/Disc_Math.org::MK155][基础定义]] $R$ 是集合 $K[x],K[x]$ 上的二元关系

***** 性质

根据[[~/文档/note/离散数学/Disc_Math.org::MK167][定义]]
...
$f(x)=cg(x)$ 则根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $1\in K\rightarrow \frac{1}{c}\in K$ 则有 $g(x)=\frac{1}{c}f(x)$
...
假设 $n=m$ 且由于 $K$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]且 $\{a_n,b_m\}\subset K$ 故有 $\frac{a_n}{b_m}\in K$
...
若 $\{g(x),f(x)\}\subset K[x]$ 存在 $K\subset E$ 同为[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]，则有 $\{g(x),f(x)\}\subset E[x]$
...
若有[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $E,K$ 满足 $K\subset E$ 有 $\{f(x),g(x)\}\subset K[x]$
...
将 $\tau$ 替换 $x$ 引入数域 $K$ 上的乘法，则显然根据[[~/文档/note/离散数学/Disc_Math.org::MK194][函数定义]] $f(\tau)=\sum_{i=0}^da_i\tau^i$ 为函数
...
即根据[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]可得 $\sigma:f(x)\to f(\tau)$ 是函数映射
...
则根据[[~/文档/note/离散数学/Disc_Math.org::MK198][满射]]可得 $\sigma$ 为满射
...
则根据[[~/文档/note/离散数学/Disc_Math.org::MK195][单射]]可得 $\sigma$ 为单射
...
则定义函数 $\Phi(\tau)=\frac{1}{f(\tau)}$ 则根据[[~/文档/note/复变函数/Cplx_fc.org::MK94][基本公式]]可得 $$\Phi'(\tau)=-\frac{f'(\tau)}{(f(\tau))^2}$$

且根据导数公式在复数域上多项式函数 $f'(\tau)$ 导数存，结合 $f(\tau)\ne0$ 可得在复数域上 $\Phi'(\tau)$ 存在

则根据[[~/文档/note/复变函数/Cplx_fc.org::MK65][解析]]可得函数 $\Phi(\tau)$ 解析。

根据[[~/文档/note/复变函数/Cplx_fc.org::MK91][性质]]以及[[~/文档/note/复变函数/Cplx_fc.org::MK57][函数极限]]可得 $\lim_{|\tau|\to\infty}\frac{1}{\tau^i}=0,i\in\mathbb{N}$
...
当 $|\tau|\leq M$ 时，根据[[~/文档/note/复变函数/Cplx_fc.org::MK80][可导必连续]]定义可得多项式函数连续

又由于 $|\tau|\leq M$ 有界闭区间，故根据[[~/文档/note/数学分析/Chap11Note.org::MK4][有界性定理]]可得有界，最终得 $\Phi(\tau)$ 有界且解析。

故根据[[~/文档/note/复变函数/Cplx_fc.org::MK95][刘维尔定理]]可得 $\Phi(\tau)$ 必为常值函数。即 $\Phi(\tau)=A$
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK37][推论2]]可得最大数域为 $\mathbb{C}$ 则 $c\in\mathbb{C}$ ，故 $c$ 存在[[~/文档/note/复变函数/Cplx_fc.org::MK93][共轭复数]]

取 $c$ 的共轭复数 $\overline{c}$ 带入亦有 $f(\overline{c})=0$ 即等价于 $\overline{c}$ 亦是多项式的根

**** 证明

对于函数 $\sum_{i=0}^na_ic^i=0$ 两边取共轭复数，则根据[[~/文档/note/复变函数/Cplx_fc.org::MK96][性质]]中加法乘法保持可得
...
则有 $(x-c)(x-\overline{c})=x^2-(c+\overline{c})x+c\overline{c}$ 根据[[~/文档/note/复变函数/Cplx_fc.org::MK93][共轭复数]]定义，若 $c=a+ib$ 则有 $\overline{c}=c-ib$
...
若 $f(x)\in\mathbb{Q}$ 即有理数义[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]多项式，则必有本源多项式与之相伴。
...
有理[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]上存在任意次不可约多项式
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK153][笛卡尔积]]以及[[~/文档/note/离散数学/Disc_Math.org::MK155][基础定义]]定义关系 $R=\left\{\left<a,b\right>\in Z\times Z\big|a\mod m=b\mod m\right\}$

根据[[~/文档/note/离散数学/Disc_Math.org::MK740][等价关系]]可知 $R$ 满足自反性，对称性以及传递性，故 $R$ 为等价关系

则根据[[~/文档/note/数理逻辑/sllj.org::MK179][定理1]] $R$ 的[[~/文档/note/离散数学/Disc_Math.org::MK177][等价类]]划分 $\mathbb{Z}$
...
故定义集合 $U=\left\{\overline{x}\big|x\in\mathbb{Z}\right\}$ 由于 $m$ 的余数是小于 $m$ 的整数，故 $U$ 的[[~/文档/note/离散数学/Disc_Math.org::MK3][基数]]是 $|U|=m$
...
根据[[~/文档/note/抽象代数/cxds.org::MK292][运算]]可知运算是[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]，必须满足唯一性。
...
若 $\mathbb{Z}_m=<U,+,\cdot>$ 是一个 $m$ 的[[MK85][剩余类环]]，若 $m$ 是素数，则 $Z_m$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][域]]
...
根据[[MK29][引理]]可得存在 $\{u,v\}\subset\mathbb{Z}$ 使得 $um+vx=1$ ，则根据[[MK85][定义]]中的加法乘法可得 
...
又根据[[MK85][定义]]可得剩余类环 $\mathbb{Z}_m$ 是交换环，故根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][域]]定义可得 $\mathbb{Z}_m$ 是域

** 定理2

若 $\mathbb{Z}_m=<U,+,\cdot>$ 是一个 $m$ 的[[MK85][剩余类环]]，若 $m$ 是合数，则 $Z_m$ 不是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][域]]
...
故对于 $\overline{a}\in\mathbb{Z}_m\land\overline{a}\ne\overline{0}$ 不存在乘法[[~/文档/note/抽象代数/cxds.org::MK302][逆元]]，故不是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][域]]
