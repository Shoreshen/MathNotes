...
则定义函数 $\Phi(\tau)=\frac{1}{f(\tau)}$ 则根据[[~/文档/note/复变函数/Cplx_fc.org][基本公式]]可得 $$\Phi'(\tau)=-\frac{f'(\tau)}{(f(\tau))^2}$$
...
故根据[[~/文档/note/复变函数/Cplx_fc.org][刘维尔定理]]可得 $\Phi(\tau)$ 必为常值函数。即 $\Phi(\tau)=A$
...
对于函数 $\sum_{i=0}^na_ic^i=0$ 两边取共轭复数，则根据[[~/文档/note/复变函数/Cplx_fc.org][性质]]中加法乘法保持可得
...
根据[[~/文档/note/离散数学/Disc_Math.org][运算]]可知运算是[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]，必须满足唯一性。
...
故对于 $\overline{a}\in\mathbb{Z}_m\land\overline{a}\ne\overline{0}$ 不存在乘法[[~/文档/note/离散数学/Disc_Math.org::MK314][逆元]]，故不是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][域]]
