...
若 $V$ 是一个非空集合， $\mathcal{K}=\left<K,\mathfrak{g}\right>$ 为[[~/文档/note/抽象代数/cxds.org::MK338][域]]，若：

（1） $V$ 上定义了加法运算即 $V\times V\mapsto V$ ，记作 $\forall(\vec{a},\vec{b})\subset V,\exists \vec{c}\in V\Rightarrow \vec{a}+\vec{b}=\vec{c}$

（2） $V$ 上定义[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][域]] $K$ 与 $V$ 中元素的数乘运算即 $K\times V\mapsto V$ ，记作 $\forall k\in K,\forall \vec{a}\in V,\exists \vec{b}\in V\Rightarrow k\vec{a}=\vec{b}$
...
（5） $\forall\vec{a}\in V\Rightarrow 1 \vec{a}=\vec{a}$ 其中 $1$ 为[[~/文档/note/抽象代数/cxds.org::MK338][域]] $K$ 的乘法[[~/文档/note/抽象代数/cxds.org::MK299][幺元]]
...
即 $0\vec{a}=\vec{0},\vec{a}\in V$ 其中 $1$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$ 的乘法[[~/文档/note/抽象代数/cxds.org::MK299][幺元]] 
...
对于任意 $\vec{a}\in V$ 根据[[MK1][定义]]中（3）（8）有 $k\vec{a}=k(\vec{0}+\vec{a})=k\vec{0}+k\vec{a}$
...
*** <<MK120>> $\mathcal{V}_K=\left<K,K,\mathfrak{g}(f_+),\mathfrak{g}(f_\times)\right>$ 亦为[[MK1][线性空间]]，且 $\dim(\mathcal{V}_K)=1$

**** 是[[MK1][线性空间]]

定义 $\mathfrak{g}(f_+),\mathfrak{g}(f_\times),\mathfrak{g}(c_0),\mathfrak{g}(c_1)$ 分别用 $+,\cdot,0,1$ 表示，则根据[[~/文档/note/抽象代数/cxds.org::MK338][域定义]]显然对于任意 $\{a,b,c\}\subset K$ 均有
...
即 $\mathcal{V}_K$ 是[[MK1][线性空间]]。

**** $\dim(\mathcal{V}_K)=1$

对于任意 $B=\{b,a_1,...,a_n\}\subset K$ 均有 $A=\sum_{i=1}^na_i\in K$ 又根据[[~/文档/note/抽象代数/cxds.org::MK338][域定义]]有 $c\in K$ 满足 $c\cdot b=1$

又根据[[~/文档/note/抽象代数/cxds.org::MK350][性质8]]可得 $\mathfrak{g}(f_-)(c\cdot b)=\mathfrak{g}(f_-)(1)\cdot (c\cdot b)=(\mathfrak{g}(f_-)(1)\cdot c)\cdot b$ 且 $(\mathfrak{g}(f_-)(1)\cdot d)\in K$
...
显然当 $B=\{b\},b\ne0$ 时，根据[[~/文档/note/抽象代数/cxds.org::MK400][性质4]]可得任意 $a\in K$ 满足 $b\cdot a=0$ 必有 $a=0$ ，即 $B=\{b\}$ [[MK8][线性无关]]

去 $b=1$ 则任意 $a\in K$ 均有 $a\cdot 1=a$ 即 $\{1\}$ 是 $\mathcal{V}_K$ 的[[MK36][基]]，则根据[[MK51][维数]]可得 $\dim(\mathcal{V}_K)=1$
...
若 $\mathcal{V}=\left<V,K,+,\cdot\right>$ 是数域 $K$ 上的[[MK1][线性空间]]，若 $U\subset V$ 满足 $\mathcal{V}'=\left<V,K,+,\cdot\right>$ 亦为线性空间  
...
线性空间 $\mathcal{V}=\left<V,K,+,\cdot\right>$ 上[[MK46][生成子空间]] $\left<\text{span}(S),K,+,\cdot\right>$ 的[[MK36][基]]为 $S$ 的[[MK15][极大线性无关集]]
...
根据[[MK121][维数公式]] $\dim(V_1+V_2)=\dim(V_1)+\dim(V_2)-\dim(V_1\cap V_2)$ 
...
** 线性相关与线性无关<<MK8>>
 ...
 $\mathcal{V}=<V,K,+,\times>$ 是[[MK1][线性空间]]， $K$ 为[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]，有 $S\subset V$ 满足：

（1） 存在互不相等的 $\{\vec{\alpha}_1,...,\vec{\alpha}_n\}\subset S$ 满足存在一组不全为零的数 $\{k_1,...,k_n\}\subset K$

     使得 $\sum_{i=1}^nk_i\vec{\alpha}_i=\vec{0}$ 则称 $S$ 在 $\mathcal{V}$ 上线性相关

（2） 对于任意互不相等 $\{\vec{\alpha}_1,...,\vec{\alpha}_n\}\subset S, n\in\mathbb{N}$ 若满足 $\sum_{i=1}^nk_i\vec{\alpha}_i=\vec{0}$ 则有 $k_1=...=k_n=0$
...
根据定义 $m<n$ 即方程数目小于未知数数目，则根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK36][推论2]]得方程必有非零解。
...
若 $\mathcal{V}=\left<V,K,+,\times\right>$ 的一个基包含无限个向量，则 $V$ 的任意两组[[MK36][基]]均[[~/文档/note/离散数学/Disc_Math.org::MK27][等势]]

***** 证明

对于任意 $A,B$ 是 $\mathcal{V}$ 的两组[[MK36][基]]，对于任意 $a\in A$ 定义 $B'$ 为 $B$ 中可以[[MK12][线性表述]] $a$ 的最小集合，即当 $a=\sum_{i=1}^nk_kb_i$ 且 $k_i\in K,k_i\ne 0$ 时 $B'=\{b_1,...,b_n\}$

显然根据[[MK36][基定义]]可得 $B'$ 必为[[~/文档/note/离散数学/Disc_Math.org::MK42][有限集]]
...
****** 定义[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]] $\varphi:A\to\rho(B)$ 证明 $\bigcup_{a\in A}\varphi(a)=B$

则可定义[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]] $\varphi:A\to\rho(B)$ 满足 $\varphi(a)=\{b_1,...,b_n\}$
...
****** 定义 $\phi:A\to\bigcup_{n\in\mathbb{N}}B^n$ 证明是[[~/文档/note/离散数学/Disc_Math.org::MK195][单射]]

******* 定义 $\phi$ 

对于任意 $B'=\{b_1,...,b_n\}\subset B$ 是[[~/文档/note/离散数学/Disc_Math.org::MK42][有限集]]定义集合 $L_{B'} =\left\{\left<b_{i_1},...,b_{i_n}\right>\big|(\forall 1\leq j\leq n\to 1\leq i_j\leq n)\land(\forall j_1\ne j_2,1\leq j_1\leq n,1\leq j_2\leq n\to i_{j_1}\ne i_{j_2})\right\}$

易证 $|L_{B'}|=n!$ 显然 $|B'|=n$ 定义 $A'_{B'}=\left\{a\big|\varphi(a)=B'\right\}$ 根据上一步证明有 $|A'_{B'}|\leq n$ 根据[[~/文档/note/离散数学/Disc_Math.org::MK733][推论]]可得 $|A'_{B'}|\leq|L_{B'}|$

即对于任意 $B'$ 若 $A'_{B'}$ 非空，则存在[[~/文档/note/离散数学/Disc_Math.org::MK195][单射]] $\phi_{A'_{B'}}:A'_{B'}\to L_{B'}$ 。定义 $\phi=\bigcup_{a\in A}\varphi_{A'_{\varphi(a)}}$ 

******* $\phi$ 是[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]

假设 $\phi$ 不是[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]，则存在 $\left\{\left<a,\vec{b}_1\right>,\left<a,\vec{b}_2\right>\right\}\subset\phi$ 满足 $\vec{b}_1\ne \vec{b}_2$ ，由于 $\phi_{A'_{\varphi(a)}}$ 是[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]，则显然不存在 $\left\{\left<a,\vec{b}_1\right>,\left<a,\vec{b}_2\right>\right\}\subset\phi_{A'_{\varphi(a)}}$ 
...
即 $\phi$ 是[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]

******* $\phi$ 是[[~/文档/note/离散数学/Disc_Math.org::MK195][单射]]
...
若 $\varphi(a_1)=\varphi(a_2)$ 则由于 $\phi_{A'_{\varphi(a_1)}}(a_1)$ 是[[~/文档/note/离散数学/Disc_Math.org::MK195][单射]]，故可得 $\phi(a_1)=\phi_{A'_{\varphi(a_1)}}(a_1)\ne\phi_{A'_{\varphi(a_1)}}(a_2)=\phi(a_2)$

故 $\phi$ 是[[~/文档/note/离散数学/Disc_Math.org::MK195][单射]]

****** 证明 $\left|\bigcup_{n\in\mathbb{N}}B^n\right|=|B|$

根据[[~/文档/note/离散数学/Disc_Math.org::MK618][定理5]]以及数学归纳法易证对于任意 $n\in\mathbb{N}$ 均有 $|B^n|=|B|$ ，又根据[[~/文档/note/离散数学/Disc_Math.org::MK616][定理3]]可得 $\left|\bigcup_{n\in\mathbb{N}}B^n\right|=\max\left(\left\{|B^n|\big|n\in\mathbb{N}\right\}\right)$

综上所述可得 $\left|\bigcup_{n\in\mathbb{N}}B^n\right|=|B|$

****** 证明结论

显然根据上两步证明可得 $|A|\leq\left|\bigcup_{n\in\mathbb{N}}B^n\right|$ 以及 $\left|\bigcup_{n\in\mathbb{N}}B^n\right|=|B|$ 则根据[[~/文档/note/离散数学/Disc_Math.org::MK725][定理6]]可得 $|A|\leq|B|$

由于对称性同理可证 $|B|\leq|A|$ ，则根据[[~/文档/note/离散数学/Disc_Math.org::MK4][伯恩斯坦定理]]可得 $|A|=|B|$
...
若线性空间 $\mathcal{V}=\left<V,K,+,\cdot\right>,\mathcal{V}'=\left<V',K,\bar{+},\bar{\cdot}\right>$ 存在[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]] $\sigma:V\rightarrow V'$ 且满足：
...
定义线性空间 $\mathcal{V}=<V,K,+,\cdot>,\mathcal{V}'=<S,K,\bar{+},\bar{\cdot}>$ [[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]定义 $0\in K$ 则根据[[MK75][定义]]、[[MK5][零乘元素]] $\sigma(\vec{0}_v)=\sigma(0\vec{v})=0\sigma(\vec{v})=\vec{0}_s$
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]有 $\sigma\left(\sum_{i=1}^nk_iv_i\right)=\sigma(\vec{0}_v)$ 
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]有 $\sum_{i=1}^nk_iv_i=\vec{0}_v$
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]可的对于任意 $s\in S$ 必有唯一对应 $v\in V\rightarrow \sigma(v)=s$
...
定义有限维[[MK1][线性空间]] $\mathcal{V}=\left<V,K,+,\cdot\right>,\mathcal{V}'=\left<S,K,\bar{+},\bar{\cdot}\right>$ 其[[~/文档/note/抽象代数/cxds.org::MK338][域]]为 $\mathcal{K} =\left<K,\mathfrak{g}\right>$ 
...
定义 $\{v_1,..v_n\}\subset V,\{s_1,...,s_n\}\subset S$ 分别为线性空间[[MK36][基]],定义 $K$ 为[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]。
...
满足[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]
...
（1） $U\ne\varnothing$ ，即 $\exists u\in U$ ，由于同构映射为[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]，故必有 $\sigma(u)\in\sigma(U)\subset S$ 与之对应。故 $\sigma(U)\ne\varnothing$ 
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]]定义以及 $\sigma(U)$ 定义

（1） $\forall s\big(s\in\sigma(U)\rightarrow\exists u(u\in U\land \sigma(u)=s)\big)$

     由于 $\sigma:V\rightarrow S$ 为[[~/文档/note/离散数学/Disc_Math.org::MK195][单射函数]]，故 $a\ne b\rightarrow\sigma(a)\ne\sigma(b)$ 。则在 $U$ 上亦然
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK201][传递性]]可得 $\sigma_1\cdot\sigma_2$ ，即[[~/文档/note/离散数学/Disc_Math.org::MK202][函数合成]]，亦为双射函数，且 $\sigma_1\cdot\sigma_2:V\rightarrow U$
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK161][逆函数]]可得存在逆函数 $\sigma^{-1}:S\rightarrow V$

根据[[~/文档/note/离散数学/Disc_Math.org::MK205][性质2]]可得 $\sigma^{-1}$ 亦为双射函数
...
在数域 $K$ 上所有的线性空间，其同构关系构成[[~/文档/note/离散数学/Disc_Math.org::MK740][等价关系]]
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK740][等价关系]]定义，同构关系为等价关系。
...
定义列元素 $\vec{a}_i=\left[\begin{matrix}a_{1i}\\a_{2i}\\...\\a_{mi}\end{matrix}\right],1\leq i\leq n$ 则称 $\mathcal{V}_{col}=\left<\text{span}(\{\vec{a}_1,...,\vec{a}_n\}),K,+,\cdot\right>$ 为矩阵的[[MK46][列空间]]
...
矩阵 $A$ 可由初等行变换变为阶梯矩阵 $E$ 且根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK18][上三角矩阵行列式]]当且仅当矩阵 $E$ 为上三角矩阵式时 $|E|\ne0$
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK18][上三角矩阵行列式]]可得行列式不为零
...
故根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK18][上三角矩阵行列式]]子式的值为对角线乘积，故亦不为零。
...
故根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK14][行乘系数]] $E'$ 的 $r+1$ 行乘以 $0$ 后的行列式为 $0|E'|=0$
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK35][性质]]子矩阵的行列式仅会改变正负，不会改变零与非零
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK26][拉普拉斯理]]可得 $|A|=\sum_{\forall1\leq j\leq n}(-1)^{k+j}a_{kj}\overline{M}\left(\begin{matrix}k\\j\end{matrix}\right)=\sum_{\forall1\leq j\leq n}a_{kj}A_{kj}$
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK29][行列式与解的关系]]可得线性方程组唯一解的充要条件为行列式不为零
...
由于方程组有界，则系数矩阵与增广矩阵秩相同。根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK29][行列式与解的关系]]可得此时行列式为零
...
根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK30][推论]]可得方程组有非零解的充要条件是行列式为零。
...
根据[[MK120][性质]]可得 $\mathcal{V}=\left<K,K,+,\cdot\right>$ 是[[MK1][线性空间]]，则根据[[MK105][幂运算]]可得 $\mathcal{V}^n$ 是[[MK1][线性空间]]
...
定义 $A$ 为齐次线性方程组的系数矩阵，且未知数个数为 $n$ ，其[[MK113][解空间]]为 $\mathcal{W}=<W,K,+,\cdot>$ 

若 $rank(A)=r$ 则 $\dim(\mathcal{W})=n-r$ 且子空间 $\mathcal{W}$ 的一个基亦称为其基础解系

**** 证明

根据[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK34][推论1]]仅可能有无穷解和零解两种情况

***** 无穷解

线性方程组增广矩阵可化为[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK5][简化行阶梯型矩阵]]
...
故任意 $W$ 中的元素都可由 $\vec{\eta}_1,...,\vec{\eta}_{n-r}$ 线性表述，且其本身属于 $\mathcal{W}$ 并线性无关，故根据[[MK36][基的定义]]可得 $\vec{\eta}_1,...,\vec{\eta}_{n-r}$ 为 $W$ 的一组基
...
进一步根据[[MK62][解的情况]]可得 $rank(A)=n$ 故 $\dim(\mathcal{W})=n-rank(A)=n-n=0$ 亦满足
...
设 $\mathcal{V}=\left<V,\mathcal{K},+,\cdot\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK526][有序域]] $\mathcal{K} =\left<K,\mathfrak{g}\right>$ 上的[[MK1][线性空间]]， $f$ 是 $\mathcal{V}$ 上的[[~/文档/note/高等代数/Algb-6-d-liner.org::MK1][双线性函数]]具有[[~/文档/note/高等代数/Algb-6-d-liner.org::MK6][对称性]]以及[[~/文档/note/高等代数/Algb-6-d-liner.org::MK14][正定型]]

则对于任意 $\{a,b\}\subset V$ 称 $f(a,b)$ 是 $a,b$ 的内积，定义 $a\odot b=f(a,b)$

则结构 $\mathcal{W}=\left<V,K,+,\cdot,\odot\right>$ 是含有内积的[[MK1][线性空间]]，即内积空间
...
设 $\mathcal{V}=\left<V,K,+,\cdot,\odot\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK526][有序域]] $\mathcal{K} = \left<K,\mathfrak{g}\right>$ 上[[MK121][含有内积线性空间]]，定义其维数 $\dim(\mathcal{V})=\dim\left(\left<V,K,+,\cdot\right>\right)$

**** [[MK36][基]]<<MK137>>

设 $\mathcal{V}=\left<V,K,+,\cdot,\odot\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK526][有序域]] $\mathcal{K} = \left<K,\mathfrak{g}\right>$ 上[[MK121][含有内积线性空间]]，定义其[[MK36][基]]为 $\left<V,K,+,\cdot\right>$ 的[[MK36][基]]

** 性质

*** 性质1<<MK139>>

设 $\mathcal{V}=\left<V,K,+,\cdot,\odot\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK526][有序域]] $\mathcal{K} = \left<K,\mathfrak{g}\right>$ 的[[MK121][内积空间]]，对于任意 $B\subset V$ 满足 $\vec{0}\not\in B$

以及任意 $\{a,b\}\subset B$ 均有 $a\odot b=\mathfrak{g}(c_0)$ 则 $B$ [[MK8][线性无关]]

**** 证明

定义 $\times,<,0$ 分别为 $\mathfrak{g}(f_\times),\mathfrak{g}(p_<),\mathfrak{g}(c_0)$ 

任取互不相等的有限子集 $\{a_1,...,a_n\}\subset B$ 以及 $\{k_1,...,k_n\}\subset K$ 若有 $\sum_{i=1}^nk_i\cdot a_i=\vec{0}$ 其中加法为[[MK132][空间加法]]

则对于 $a_j$ 有 $a_j\odot \sum_{i=1}^nk_i\cdot a_j=a_i\odot\vec{0}$ 根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK7][性质2]]可得 $a_j\odot\vec{0}=0$

又根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK1][双线性函数]]可得 $a_i\odot \sum_{i=1}^nk_i\cdot a_i=\mathfrak{g}\left(\sum\right)_{i=1}^nk_i\times (a_j\odot a_i)$

根据条件可得 $\mathfrak{g}\left(\sum\right)_{i=1}^nk_i\times (a_j\odot a_i)=k_j\times(a_i\odot a_i)$ 根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK14][正定型]]以及 $\vec{0}\not\in B$ 可得 $0<a_i\odot a_i$

又根据[[~/文档/note/抽象代数/cxds.org::MK340][性质2]]以及[[~/文档/note/抽象代数/cxds.org::MK527][三歧性]]可得 $k_j=0$ 同理可证任意 $1\leq i\leq n$ 均有 $k_i=0$

即 $B$ 中任意有限子集[[MK8][线性无关]]，故 $B$ [[MK8][线性无关]]

** 定理

设 $\mathcal{V}=\left<V,K,+,\cdot,\odot\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK526][有序域]] $\mathcal{K} = \left<K,\mathfrak{g}\right>$ 的[[MK121][内积空间]]且 $\{a_1,...,a_n\}\subset V$ 是 $\mathcal{V}$ 的[[MK137][基]]

则存在 $\{b_1,...,b_n\}\subset V$ 亦为 $\mathcal{V}$ 的[[MK137][基]]且任意 $i\ne j$ 均有 $b_i\odot b_j=\mathfrak{g}(c_0)$
...
设 $\{a_1,...,a_n\}\subset V$ 是 $\mathcal{V}$ 的[[MK137][基]]，则定义 $\mathcal{V}_1=\left<\text{span}(\{a_1,...,a_{n-1}\}),K,+,\cdot,\odot\right>,\mathcal{V}_1=\left<\text{span}(\{a_n\}),K,+,\cdot,\odot\right>$ 

则根据[[MK89][性质3]]可得 $\mathcal{V}=\mathcal{V}_1\oplus\mathcal{V}_2$ 其中 $\oplus$ 是[[MK83][直和]]。又根据[[MK88][性质1]]可得 $\{a_1,...,a_{n-1}\},\{a_n\}$ 分别是 $\mathcal{V}_1,\mathcal{V}_2$ 的[[MK137][基]]

根据归纳假设 $\mathcal{V}_1$ 存在 $B=\{b_1,...,b_{n-1}\}$ 满足：

（1） $B$ 是 $\mathcal{V}_1$ 的[[MK137][基]]

（2） 任意 $i\ne j$ 均有 $b_i\odot b_j=\mathfrak{g}(c_0)$

根据[[MK102][性质3]]可得任意 $b_i\ne\vec{0}$ 则根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK14][正定型]]可得 $b_i\odot b_i\ne\mathfrak{g}(c_0)$ 则根据[[~/文档/note/抽象代数/cxds.org::MK526][有序域]]可得存在乘法逆元，记作 $\frac{1}{b_i\odot b_i}$

又根据[[MK69][性质1]]可得 $\{b_1,...,b_{n-1},a_n\}$ 是 $\mathcal{V}$ 的[[MK137][基]]，定义 $b_n=a_n-\sum_{i=1}^n\frac{a_n\odot b_i}{b_i\odot b_i}b_i$ 根据
...
假设 $\{b_1,...,b_n\}$ [[MK8][线性相关]]则存在不全为 $\mathfrak{g}(c_0)$ 的 $\{k_1,...,k_n\}\subset K$ 满足 $\sum_{i=1}^nk_ib_i=\vec{0}$

由于 $\{b_1,...,b_{n-1}\}$ 是 $\mathcal{V}_1$ 的[[MK137][基]]，故显然 $k_n\ne\mathfrak{g}(c_0)$ 则有 $\sum_{i=1}^{n-1}\left(k_i-\frac{a_n\odot b_i}{b_i\odot b_i}k_n\right)b_i+a_n=\vec{0}$

即 $\{b_1,...,b_{n-1},a_n\}$ [[MK8][线性相关]]，显然与其是 $\mathcal{V}$ 的[[MK137][基]]矛盾，故假设不成立，即 $\{b_1,...,b_n\}$ [[MK8][线性无关]]

则根据[[MK42][命题2]]可得 $\{b_1,...,b_n\}$ 是 $\mathcal{V}$ 的[[MK137][基]]且任意 $i\ne j$ 均有 $b_i\odot b_j=\mathfrak{g}(c_0)$

** 欧几里得空间<<MK134>>

有限[[MK121][维度]]的实数域上[[MK121][含有内积的线性空间]] $\mathcal{V}=\left<\mathbb{R}^n,\mathbb{R},+,\cdot,\odot\right>$ 即为欧几里得空间
...
其中 $I_n$ 为实数域上的[[~/文档/note/高等代数/Algb-3-Matrix.org::MK4][单位矩阵]]，则 $f(X,Y)=X\odot Y$ 是具有[[~/文档/note/高等代数/Algb-6-d-liner.org::MK6][对称性]]以及[[~/文档/note/高等代数/Algb-6-d-liner.org::MK14][正定型]]的[[~/文档/note/高等代数/Algb-6-d-liner.org::MK1][双线性函数]]

****** 证明

根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK15][性质3]]即可证

****** 性质<<MK131>>

任意 $\{X,Y\}\subset\mathbb{R}^n$ 均有：

（1） $(X\odot Y)^2\leq|X||Y|$ 当且仅当 $X,Y$ [[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK8][线性相关]]有 $(X\odot Y)^2=|X||Y|$
...
根据[[MK123][长度]] $(X\odot X)(Y\odot Y)=|X|^2|Y|^2=(|X||Y|)^2$ 则根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK18][柯西不等式]]有 $(X\odot Y)^2\leq (X\odot X)(Y\odot Y)=(|X||Y|)^2$

且当且仅当 $X,Y$ [[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK8][线性相关]]有 $(X\odot Y)^2=|X||Y|$

******** 证明（2）

********* 充分

若有 $Y=kX,k\in\mathbb{R}\land k<0$ 则根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK1][双线性]]易证 $X\odot Y=k(X\odot X)=k|X|^2$ 且 $|X||Y|=|X||kX|=\sqrt{k^2}|X|^2$
...
若有 $-|X||Y|= X\odot Y$ 则显然 $|X|^2|Y|^2=(X\odot Y)^2$ 结合（1）证明以及[[~/文档/note/高等代数/Algb-6-d-liner.org::MK18][柯西不等式]]可得 $X,Y$ [[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK8][线性相关]]
...
即 $\vec{0}=\left<0,...,0\right>\in\mathbb{R}^n$ 为[[MK1][加法零元]]，显然对于任意 $\vec{x}=\left<x_1,...,x_n\right>\in\mathbb{R}$

根据[[MK132][加法]]均有 $\vec{x}+\vec{0}=\left<x_1+0,...,x_n+0\right>=\vec{x}$

**** [[MK121][维数]]与[[MK137][基]]

***** 标准[[MK36][基]]<<MK136>>

[[MK134][欧几里得空间]] $\mathcal{V}=\left<\mathbb{R}^n,\mathbb{R},+,\cdot,\odot\right>$ 则有：


（1） $\dim(\mathcal{V})=n$


（2） 定义 $B=\left\{\vec{x}_i\bigg|\vec{x}_i=\left<x_1,...,x_n\right>\land x_j=\begin{cases}0&j\ne i\\1&j=i\end{cases}\land 1\leq i\leq n\land i\in\mathbb{N}\right\}$ 则 $B$ 是 $\mathcal{V}$ 的[[MK36][基]]

称其为[[MK134][欧几里得空间]]的标准[[MK36][基]]

****** 证明

定义 $\mathcal{V}'=\left<\mathbb{R}^n,\mathbb{R},+,\cdot\right>$ 对于任意 $\{k_1,...,k_n\}\subset\mathbb{R}$ 易证 $\sum_{i=1}^nk_i\vec{x}_i=\left<k_1,k_2,...,k_n\right>$ 则根据[[MK135][加法零元]]显然

当且仅当 $k_1=...=k_n=0$ 时有 $\sum_{i=1}^nk_i\vec{x}_i=\vec{0}$ 即可得 $B$ [[MK8][线性无关]]

对于任意 $\vec{y}=\left<y_1,...,y_n\right>\in\mathbb{R}$ 均有 $\vec{y}=\sum_{i=1}^ny_i\vec{x}_i$ 故任意元素可由 $B$ [[MK12][线性表述]]

即 $B$ 是 $\mathcal{V}'$ 的[[MK36][基]]，则根据[[MK121][维数]]与[[MK137][基]]可得 $B$ 是 $\mathcal{V}$ 的[[MK137][基]]且 $\dim(\mathcal{V})=n$

****** 性质

[[MK134][欧几里得空间]] $\mathcal{V}=\left<\mathbb{R}^n,\mathbb{R},+,\cdot,\odot\right>$ 的[[MK136][标准基]] $B=\left\{\vec{\epsilon}_1,...,\vec{\epsilon}_n\right\}$ 中任意两个不同的元素[[MK130][正交]]

******* 证明

根据[[MK124][内积]]定以易证 $i\ne j\to\vec{\epsilon}_i\odot\vec{\epsilon}_j=0$ 即[[MK130][正交]]

***** 正交[[MK36][基]]

[[MK134][欧几里得空间]] $\mathcal{V}=\left<\mathbb{R}^n,\mathbb{R},+,\cdot,\odot\right>$ 任意 $n$ 个两两[[MK130][正交]]的元素组成集合 $B$ 是其[[MK36][基]]，称作正交[[MK36][基]]

****** 证明

根据[[MK139][性质1]]可得 $n$ 个两两[[MK130][正交]]的元素[[MK8][线性无关]]，又根据[[MK136][标准基]]可得 $\dim(\mathcal{V})=n$

则根据[[MK42][命题2]]可得是 $B$ 是[[MK36][基]]

*** 扩展定义

**** 长度<<MK123>>

任意 $X\in\mathbb{R}^n$ 定义其长度为 $|X|=\sqrt{X\odot X}$ 其中 $X\odot X$ 为 $X$ 与自身的[[MK124][内积]]
...
根据[[~/文档/note/高等代数/Algb-6-d-liner.org::MK14][正定型]]可得 $X\odot X=f(X,X)\geq0$ 则显然 $\sqrt{X\odot X}\in\mathbb{R}$ 且 $|X|\geq 0$
...
根据 $\odot$ 为[[~/文档/note/高等代数/Algb-6-d-liner.org::MK6][对称性]]以及[[~/文档/note/高等代数/Algb-6-d-liner.org::MK14][正定型]]的[[~/文档/note/高等代数/Algb-6-d-liner.org::MK1][双线性函数]]，以及[[MK131][性质]]可得
...
结合[[MK128][非负性]]可得 $|X+Y|\leq|X|+|Y|$

又根据[[MK131][性质]]可得当且仅当 $Y=kX,k\in\mathbb{R}\land k\geq 0$ 时有 $|X||Y|= X\odot Y$ 结合上述公式可得 $|X+Y|=|X|+|Y|$

**** 夹角<<MK125>>

任意 $\{X,Y\}\subset\mathbb{R}^n$ 定义夹角 $<X,Y>=\arccos\left(\frac{X\odot Y}{|X||Y|}\right)$ 其中 $|X|,|Y|$ 分别为 $X,Y$ 的[[MK123][长度]]
...
根据[[MK131][性质]]易证 $-1\leq \frac{X\odot Y}{|X||Y|}\leq 1$ 则 $0\leq\arccos\left(\frac{X\odot Y}{|X||Y|}\right)\leq\pi$

**** 正交<<MK130>>

任意 $\{X,Y\}\subset\mathbb{R}^n$ 若有 $<X,Y>=0$ 则称 $X,Y$ 正交，记作 $X\perp Y$ ，其中 $<X,Y>$ 为 $X,Y$ 的[[MK125][夹角]]
...
根据[[MK124][内积]]的[[~/文档/note/高等代数/Algb-6-d-liner.org::MK6][对称性]]以及实数乘法交换律可得 $0=<X,Y>=\arccos\left(\frac{X\odot Y}{|X||Y|}\right)=\arccos\left(\frac{Y\odot X}{|Y||X|}\right)=<Y,X>$

***** 勾股定理

任意 $\{X,Y\}\subset\mathbb{R}^n$ 若[[MK130][正交]]，则有 $\left(|X+Y|\right)^2=|X|^2+|Y|^2$ 即勾股定理
...
根据[[MK126][举例]]、[[MK123][长度]]以及[[MK124][内积]] $d(X,Y)=|X+((-1)\cdot Y)|=\sqrt{(X+((-1)\cdot Y))\odot (X+((-1)\cdot Y))}=\sqrt{\sum_{i=1}^n\left(x_i-y_i\right)^2}$
...
根据[[MK127][对称性证明]]可得 $d(X,Y)=\sqrt{\sum_{i=1}^n\left(x_i-y_i\right)^2}$ 根据[[~/文档/note/抽象代数/cxds.org::MK531][性质5]]可得 $x_i-y_i\ne0\to (x_i-y_i)^2>0$

且根据[[~/文档/note/抽象代数/cxds.org::MK527][性质1]] $x_i-y_i=0\to(x_i-y_i)^2=0$ 又根据[[~/文档/note/抽象代数/cxds.org::MK527][性质1]]加法相容易证 $\sum_{i=1}^n\left(x_i-y_i\right)^2\geq0$
...
根据[[MK126][距离]]以及[[MK127][对称性]]可得 $d(X,Y)=|X-Y|,d(X,Z)=|X-Z|,d(Z,Y)=|Z-Y|$

根据[[MK132][加法]]易证 $X-Y=(X-Z)+(Z-Y)$ 定义 $a=X-Z,b=Z-Y$ 则有 $a+b=X-Y$

故 $d(X,Y)=|a+b|,d(X,Z)=|a|,d(Y,Z)=|b|$ 根据[[MK133][不等式]]有 $|a+b|\leq|a|+|b|$ 即 $d(X,Y)\leq d(Y,Z)+d(X,Z)$
...
根据[[MK131][性质]]可得 $(X\odot Y)^2=|X||Y|$ 则易证 $-|X||Y|\leq X\odot Y\leq |X||Y|$ 即 $-(X\odot Y)\leq |X||Y|$ 又根据[[MK127][之前证明]]可得
...
根据[[MK128][长度非负性]]易证 $|X|+|Y|\geq 0$ 结合[[MK129][距离非负性]]可得 $d(X,Y)\geq 0$ 结合 $(d(X,Y))^2\leq \left(|X|+|Y|\right)^2$ 易证 $d(X,Y)\leq |X|+|Y|$

******* 证明（2）

根据[[MK131][性质]]可得当且仅当 $Y=kX,k\in\mathbb{R}\land k\leq 0$ 时有 $-|X||Y|= X\odot Y$ 结合上一部有 $(d(X,Y))^2=\left(|X|+|Y|\right)^2$

同理结合[[MK128][长度非负性]]以及[[MK129][距离非负性]]易证 $d(X,Y)=|X|+|Y|$ 此时 $\frac{X\odot Y}{|X||Y|}=\frac{k|X|^2}{\sqrt{k^2}|X|^2}=-1$

则根据[[MK125][夹角]]可得 $<X,Y>=\arccos\left(\frac{X\odot Y}{|X||Y|}\right)=\arccos(-1)=\pi$
