...
（1） 存在乘法[[~/文档/note/抽象代数/cxds.org::MK299][幺元]]，定义为 $1$ （根据[[~/文档/note/高等代数/Algb-4-Multi-equ.org::MK9][定理]]存在乘法零元，等于加法幺元，记作 $0$ ）

（2） $1$ 不等于加法[[~/文档/note/抽象代数/cxds.org::MK299][幺元]]（定义为 $0$ ）

（3） $\forall a\left(a\in U\land a\ne0\rightarrow\exists y(y\in U\land a\cdot y=1)\right)$ 即任意不等于加法幺元的元素存在乘乘法[[~/文档/note/抽象代数/cxds.org::MK302][逆元]]
...
*** 推论2<<MK36>>：$n$ 元齐次线性方程组有非零解的充分条件为方程数目小于未知数数目
