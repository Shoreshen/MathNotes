...
且 $\sigma$ 对于改[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK36][基]]的[[MK28][映射矩阵]]为[[~/文档/note/高等代数/Algb-3-Matrix.org::MK86][Jordan块]] $J_{\dim(\mathcal{V})}(\mathfrak{g}(c_0))$
...
（7） $\sigma$ 的[[MK108][最小零化多项式]]可分解为一次多项式的乘积，即 $f(x)=\left(I_1[x](\prod)\right)_{i=1}^n(I_1[x](f_+)(x,k_i))$
...
根据[[~/文档/note/高等代数/Algb-3-Matrix.org::MK55][矩阵特征值]]及其求解可得存在 $X=[k_1,...,k_n]^T,k\in K$ 满足 $M_{\sigma(A)\to A}X=\lambda_0 X$
...
任意 $a\in V$ 定义[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK46][生成空间]] $\mathcal{V}_{\sigma^\alpha(a)}=\left<\text{span}\left(\left\{\sigma^n(a)\big|n\in\mathbb{N}\right\}\right),K,+,\cdot\right>$ 为 $\mathcal{V}$ 的循环子空间
...
（2） 行列数位 $n$ [[~/文档/note/抽象代数/cxds.org::MK338][域]] $\mathcal{K}=\left<K,\mathfrak{g}\right>$ 上的[[~/文档/note/高等代数/Algb-3-Matrix.org::MK76][矩阵单位域]]为 $\mathcal{M}_E=\left<M_{E},I_E\right>$
