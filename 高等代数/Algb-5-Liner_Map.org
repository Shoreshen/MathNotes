...
若 $f(x)\in M_1[x]$ 为其[[MK108][最小零化多项式]]满足 $f(x)=\prod_{i=1}^n(x+k_i\sigma_1)^{l_i}$ 其中：
...
由于 $f(x)=f_1(x)\cdot f_2(x)$ 则根据[[~/文档/note/抽象代数/cxds.org::MK442][性质1]]可得 $f(\sigma)=f_1(\sigma)\cdot f_2(\sigma)$ 

***** $\ker(f_1(\sigma))\dotplus\ker(f_2(\sigma))\subset \ker(f(\sigma))$

对于任意 $a\in \ker(f_1(\sigma))$ 可得 $f_1(\sigma)(a)=0\in V$ ，根据[[MK17][定义]]、[[MK2][零元映射]]有 $(f_2(\sigma)\cdot f_1(\sigma))(a)=f_2(\sigma)(f_1(\sigma)(a))=f_2(\sigma)(0)=0$

又根据条件（3）有 $\mathcal{M}_\sigma\vDash(\forall x_1(\forall x_2(f_\times(x_1,x_2)\hat{=}f_\times(x_2,x_1))))$ 可得 $f_2(\sigma)\cdot f_1(\sigma)=f_1(\sigma)\cdot f_2(\sigma)=f(\sigma)$
