...
（1） 存在一组[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK36][基]] $B$ 满足 $\sigma$ 对 $B$ 的[[MK28][映射矩阵]]为参数是 $\mathfrak{g}(c_0)$ 的[[~/文档/note/高等代数/Algb-3-Matrix.org::MK88][Jordan型]]，即 $M=\text{diag}\left(J_{n_1}(\mathfrak{g}(c_0)),...,J_{n_m}(\mathfrak{g}(c_0))\right)$
...
（3） 对于任意 $1\leq u\leq n$ 在 $\{n_1,...,n_m\}$ 中等于 $u$ 的元素个数 $z_u=\begin{cases}m-\sum_{j=2}^nz_j&u=0\\rank(\sigma^{u-1})+rank(\sigma^{u+1})-2rank(\sigma^u)&1<u<n\\rank(\sigma^{u-1})-rank(\sigma^u)&u=n\end{cases}$
...
可得 $z_u=\begin{cases}m-\sum_{j=2}^nz_j&u=0\\rank(\sigma^{u-1})+rank(\sigma^{u+1})-2rank(\sigma^u)&1<u<n\\rank(\sigma^{u-1})-rank(\sigma^u)&u=n\end{cases}$
...
******* $J_u(\mathfrak{g}(c_0))$ 的个数 $z_u=\begin{cases}m-\sum_{j=2}^nz_j&u=0\\N(M^{u-1})+N(M^{u+1})-2N(M^u)&1<u<n\\N(M^{u-1})-N(M^u)&u=n\end{cases}$
...
根据[[MK20][幂运算]]及其[[MK69][性质]]可得 $\sigma^m(a^*)=\sigma^{m-1}(\simga(a^*))$ 根据上一步可得 $\sigma(a^*)=\lambda_0a^*$ 即 $\sigma^m(a^*)=\sigma^{m-1}(\lambda_0a^*)$
...
根据[[MK66][定理]]可得其[[MK65][单位结构]] $\mathcal{M}_1=\left<M_1,I_{|M_1}\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK338][域]]，任意非零[[~/文档/note/抽象代数/cxds.org::MK338][域]] $\mathcal{M}_1$ 上的[[~/文档/note/抽象代数/cxds.org::MK436][多项式]] $f(x)\in M_1,f(x)\ne I_{|M_1}[x](c_0)$

若满足在[[MK65][一元结构]] $\mathcal{M}_\sigma=\left<M_\sigma,I\right>$ 上对 $\sigma$ 的[[~/文档/note/抽象代数/cxds.org::MK441][带入值]]为 $I(c_0)$ 即 $f(\sigma)=I(c_0)$ 则有

则称 $f(x)$ 为 $\sigma$ 的零化多项式。
...
（5） $\mathcal{V}=\mathcal{V}_{1}\oplus...\oplus\mathcal{V}_n$

（6） $\mathcal{V}_i$ 关于 $\sigma$ 的[[MK108][最小零化多项式]]为 $f_i(x)=(x+k_i\sigma_1)^{l_i}$

其中：

（a） $k_i\in K,l_i\in\mathbb{N}$
...
（f） $g_i(x)=(x+k_i\sigma_1)^{l_i}$ 且 $\mathcal{V}_i=\left<\ker\left(g_i(\sigma)\right),K,\pmb{+},\pmb{\cdot}\right>$

***** 证明

****** 证明（5）

根据[[MK71][推论1]]以及[[MK108][零化多项式定义]]直接可证
...
假设存在 $t_i<l_i$ 则根据[[MK109][定理3]]可得 $\prod(x+k_i\sigma_1)^{t_i}$ 亦为 $\sigma$ 的[[MK108][零化多项式]]，且根据[[~/文档/note/抽象代数/cxds.org::MK387][性质1]]可得[[~/文档/note/抽象代数/cxds.org::MK378][次]]数为 $\sum_{i=1}^nt_i$
...
存在 $N=\{n_1,...,n_k\}$ 满足 $\sigma$ 关于 $B$ 自身的[[MK28][映射矩阵]]（即 $\sigma(B)=BA$ ）为[[~/文档/note/高等代数/Algb-3-Matrix.org::MK72][分块对角矩阵]] $A=[a_{ij}]^N_{n\times n}$ 
...
设 $\mathcal{V}=\left<V,K,+,\cdot\right>$ 是[[~/文档/note/抽象代数/cxds.org::MK338][域]] $\mathcal{K}=\left<K,\mathfrak{g}\right>$ 上的[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK1][线性空间]]，对于任意 $\sigma\in\text{Hom}(\mathcal{V},\mathcal{V})$ [[MK4][线性变换]]
...
（3） $f(\sigma)=I(c_0)$ 即[[MK65][一元结构]] $\mathcal{M}_\sigma=\left<M_\sigma,I\right>$ 的[[MK61][加法零元]]

则定义[[MK22][核空间]] $\mathcal{V}_{f_i(\sigma)}=\left<\ker(f_i(\sigma)),K,\pmb{+},\pmb{\cdot})\right>$ 则有：
...
（3） $\sigma$ 关于上一步中 $B$ 自身的[[MK28][映射矩阵]]（即 $\sigma(B)=BA$ ）为[[~/文档/note/高等代数/Algb-3-Matrix.org::MK72][分块对角矩阵]]

***** 证明

****** 证明（1）

根据[[MK65][定义]]可得 $I(c_0)$ 为[[MK61][加法零元]]，即任意 $a\in V$ 均有 $I(c_0)(a)=\vec{0}$ 则根据[[MK37][定义]]可得 $\ker(I(c_0))=V$

则显然 $\mathcal{V}=\left<\ker(I(c_0)),K,+,\cdot\right>$ 。又根据条件（1）（2）以及[[MK71][推论1]]可得 $\mathcal{V}=\sum_{i=1}^k\mathcal{V}_{f_i(\sigma)}$

****** 证明（2）

根据的[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK74][性质]]直接可得

****** 证明（3）

根据[[MK72][性质3]]可得 $\mathcal{V}_{f_i(\sigma)}$ 是[[MK57][不变子空间]]，结合上两步结论以及[[MK73][定理1]]可得结论
