...
根据[[~/文档/note/数学分析/Chap14Note.org][推论]] $\lim_{\lambda\to0}\left\{\sum_{i=1}^{n-1}(u(\alpha_i,\beta_i),-v(\alpha_i,\beta_i))\cdot(\Delta a_i,\Delta b_i)\right\}$ 为微分形式的黎曼和
...
根据[[~/文档/note/数学分析/Chap14Note.org][连续函数可积]]可得 $\vec{F}(x,y)$ 可积
...
定义 $L$ 长度为 $l$ ，由于 $L$ 为有界闭集，故根据[[~/文档/note/数学分析/Chap11Note.org][康托定理]]可得 $u(x,y),v(x,y)$ 一致连续。
