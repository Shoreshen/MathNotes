...
（1） [[~/文档/note/数学分析/Chap14Note.org::MK52][可求长线段]]
...
构造 $\vec{x}_k=(a_k,b_k)$ 由于 $a_k\in\mathbb{R}\land b_k\in\mathbb{R}$ 故根据[[~/文档/note/数学分析/Chap11Note.org::BW][Bolzano-Weierstrass定理]]
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK194][函数定义]]，由于 $f(z)$ 为函数，故对于 $z=x+iy$ 必有一个 $f(z)=a+ib$ 形成映射
...
（1） $u(x,y),v(x,y)$ 在点 $(x_0,y_0)$ 根据[[~/文档/note/数学分析/Chap12Note.org::MK28][全微分]]定义，可微
...
即根据[[~/文档/note/数学分析/Chap12Note.org::MK28][全微分]]定义， $u(x,y)$ 在点 $x_0,y_0$ 可微

且根据[[~/文档/note/数学分析/Chap12Note.org::MK6][可微与偏导]]
...
定义 $z_0=x_0+iy_0$ 由于可微，则根据[[~/文档/note/数学分析/Chap12Note.org::MK6][可微与偏导]]可得
...
设 $D\subset\mathbb{C}$ 为复平面上的区域，若存在复平面上[[~/文档/note/数学分析/Chap14Note.org::MK52][可求长线段]] $L\subset D$

定义 $x_0,...,x_n$ 为线段上的点，将线段划分成 $n-1$ 份，其中 $x_0,x_n$ 分别为线段的起始和终止点

规定 $x_0\to x_n$ 为线段的正方向。若[[MK61][复函数]] $f(x)$ 在 $L$ 上有定义且有界（ $\forall z\in L\rightarrow|f(z)|<\infty$ ）

取 $\zeta_i$ 为线段 $x_i,x_{i+1}$ 上的一点，则有[[~/文档/note/数学分析/Chap7Note.org::MK23][黎曼和]] $$\sum_{i=1}^{n-1}f(\zeta_i)(x_{i+1}-x_i)$$
...
即转化为两个[[~/文档/note/数学分析/Chap14Note.org::MK57][第二类曲线积分]]，其中 $u(x,y)+iv(x,y)=f(x+iy)$ 根据[[MK61][定理]]所得
...
根据[[~/文档/note/数学分析/Chap14Note.org::MK59][推论]] $\lim_{\lambda\to0}\left\{\sum_{i=1}^{n-1}(u(\alpha_i,\beta_i),-v(\alpha_i,\beta_i))\cdot(\Delta a_i,\Delta b_i)\right\}$ 为微分形式的黎曼和
...
根据[[~/文档/note/数学分析/Chap7Note.org::MK5][线性]]可得：
...
根据[[~/文档/note/数学分析/Chap14Note.org::MK58][连续函数可积]]可得 $\vec{F}(x,y)$ 可积
...
由于[[MK67][可积]]故实部虚部均可积。根据[[~/文档/note/数学分析/Chap14Note.org::MK9][区间可加]]可得实部虚部均满足区间可加
...
根据[[~/文档/note/数学分析/Chap14Note.org::MK52][可求长线段]]定义 $\lim_{\lambda\to0}\sum_{i=1}^n|\Delta z_i|=l$ 又由于弧长大于等于弦长，故有 $\sum_{i=1}^n|\Delta z_i|\leq l$
...
定义 $L$ 长度为 $l$ ，由于 $L$ 为有界闭集，故根据[[~/文档/note/数学分析/Chap11Note.org::MK40][康托定理]]可得 $u(x,y),v(x,y)$ 一致连续。
...
定义 $L$ 的参数方程为 $z(t)=\alpha+r\left(\cos(t)+i\sin(t)\right)$ 根据[[~/文档/note/大学物理/Note/DiffEq.org::MK9][欧拉公式]]可得 $z(t)=\alpha+re^{it}$ 其中 $t\in[0,2\pi]$
...
若 $D\subset\mathbb{C}$ 是[[~/文档/note/数学分析/Chap14Note.org::MK40][平面单连通区域]]，函数 $f(z)$ 在 $z\in D$ 上[[MK65][解析]]
...
利用以上（1）（2）条件，根据[[~/文档/note/数学分析/Chap11Note.org::KTDL][康托闭区域套定理]]可得
...
若 $D\subset\mathbb{C}$ 是有界的[[~/文档/note/数学分析/Chap14Note.org::MK40][平面单连通区域]]，函数 $f(z)$ 在 $D$ 上[[MK65][解析]]
...
根据[[MK74][区间可加]]可得 $$\int_{L_1}\frac{f(z)}{z-z_0}dz+\int_{L_2}\frac{f(z)}{z-z_0}dz=\int_{L}\frac{f(z)}{z-z_0}dz-\int_{L^o}\frac{f(z)}{z-z_0}dz=0$$
...
由于 $L$ 为有界闭集且 $f(z)$ 解析，故 $f(z)$ 连续，根据[[~/文档/note/数学分析/Chap11Note.org::MK4][有界性定理]]可得有界
