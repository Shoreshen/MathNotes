...
其中 $n\in\mathbb{N}$ 是自然数， $\phi(p_1,...,p_n,x)$ 是[[MK441][自由变元]]为 $p_1,...,p_n,x$ 且不包含 $U,S$ 的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 对于任意 $\phi(p_1,...,p_n,x)\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 存在唯一 $S\in M$ 满足 $S=\left\{x\big|\mathcal{M}\vDash\phi(x)[P_1,...,P_n]\right\}$

***** 证明

对于任意 $\{P_1,...,P_n,U\}\subset M$ 以及 $\phi(p_1,...,p_n,x)\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 若存在 $S_1,S_2$ 同时满足

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]对于任意 $a\in M$ 有 $a\in S_1$ 等价于 $\mathcal{M}\vDash((x\in U)\land\phi(x))[P_1,...,P_n,a]$ 等价与 $a\in S_2$

即 $a\in S_1$ 的充要条件是 $a\in S_2$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\left<\mathcal{M},v\right>\vDash(\forall x_1(p_\in(x_1,x_2)\leftrightarrow p_\in(x_1,x_3)))$ 其中 $v(x_2)=S_1,v(x_3)=S_2$

则根据[[MK5][外延公理]]可得 $\left<\mathcal{M},v\right>\vDash(x_2\hat{=}x_3)$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $S_1=S_2$
...
对于任意 $\phi\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 满足 $\phi$ 中的[[MK441][自由变元]] $S\subset\{p_1,...,p_n,x_1,x_2\}$
...
对于任意[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]] $\phi$ 满足其[[MK441][自由变元]] $S\subset\{x_1,...,x_{n+1}\}$ 
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 以及任意[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]] $\phi\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 其[[MK441][自由变元]]为 $p_1,...,p_n,x$ 对于任意给定 $\left<P_1,...,P_n\right>\in M^n$
...
或者 $x=\Phi_{pair}(\mathcal{M},v(x_1),v(x_2)))$ 。记作 $v(x_3)=\Phi_{ord}(\mathcal{M},v(x_1),v(x_2))$

其中 $\Phi_{pair}$ 为[[MK712][配对]]
...
对于任意 $x\in M$ 有 $\left<x,v(x_3)\right>\in I(p_\in)$ 的充要条件是 $\left<x,v(x_1)\right>\in I(p_\in)$ 或者 $\left<x,v(x_2)\right>\in I(p_\in)$

记作作 $v(x_3)=\Phi_\cup(\mathcal{M},v(x_1))$
...
定义 $\phi_\rho(x_1,x_2)=(\forall y_1(p_\in(y_1,x_2)\leftrightarrow(\exists y_2(\phi_\subset(y_2,x_1)))))$ 其中 $\phi_\subset$ 为[[MK20][子集]]关系
...
对于任意 $x\in M$ 有 $\left<x,v(x_2)\right>\in I(p_\in)$ 的充要条件是存在 $y\in M$ 满足 $\Phi_\subset(y,v(x_1))$

记作 $\Phi_\rho(v(x_1),v(x_2))$
...
定义 $\phi_\cap(x_1,x_2)=(\exists x_5(\phi_\cup(x_1,x_5)\land(\forall x_3(p_\in(x_3,x_2)\leftrightarrow(p_\in(x_3,x_5)\land(\forall x_4(p_\in(x_4,x_1)\to p_\in(x_3,x_4))))))))$
...
在 $I(p_\in)$ 的角度上满足 $v(x_2)$ 是 $v(x_1)$ 中所有元素的交集
...
在 $I(p_\in)$ 的角度上满足 $v(x_3)$ 是 $v(x_1)$ 和 $v(x_2)$ 的差集
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 任取 $\{A,B\}\subset M$

****** 构造集合

（1） 根据[[MK713][配对存在]]可得存在 $C\in M$ 满足 $C=\Phi_{pir}(\mathcal{M},A,B)$

（2） 进一步根据[[MK710][并集存在]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证存在 $D\in M$ 满足 $D=\Phi_\cup(\mathcal{M},C)$

（3） 根据[[MK
 
即 $\left<x,D\right>\in I(p_\in)$ 的充要条件是 $\{\left<x,A\right>,\left<x,B\right>\}\cap I(p_\in)\ne\varnothing$
...
对于任意 $a\in M\land \left<a,A\right>\in I(p_\in)$ 以及任意 $b\in M\land \left<b,B\right>\in I(p_\in)$ ：

（1） 根据[[MK713][配对存在]]可得存在 $c_1\in M$ 满足任意 $y\in M$ 且 $\left<y,c\right>\in I(p_\in)$ 均有 $y=a$ 或 $y=b$

（2） 根据[[MK715][推论]]同理易证存在 $c_2\in M$ 满足 $y\in M$ 且 $\left<y,c\right>\in I(p_\in)$ 均有 $y=a$ 

（3） 根据[[MK713][配对存在]]可得存在 $c_3\in M$ 满足 $y\in M$ 且 $\left<y,c\right>\in I(p_\in)$ 均有 $y=c_1$ 或 $y=c_2$

显然根据[[MK714][定义]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得 $\mathcal{M}\vDash\phi_{ord}(x_1,x_2,x_3)[a,b,c_3]$
...
又根据[[MK713][配对存在]]可得对于任意 $a\in M\land \left<a,A\right>\in I(p_\in)$ 以及任意 $b\in M\land \left<b,B\right>\in I(p_\in)$ 均存在 $c_1\in M$

满足任意 $y\in M$ 且 $\left<y,c\right>\in I(p_\in)$ 均有 $y=a$ 或 $y=b$

根据[[MK715][推论]]同理易证对于任意 $a\in M\land a\in A$ 均有 $c_1\in M$ 满足 $y\in M$ 且 $\left<y,c\right>\in I(p_\in)$ 均有 $y=a$ 

又根据[[MK713][配对存在]]可得存在 $c_3\in M$ 满足 $y\in M$ 且 $\left<y,c\right>\in I(p_\in)$ 均有
...
且 $\{\left<x,A\right>,\left<x,B\right>\}\cap I(p_\in)\ne\varnothing$ 均存在 $y$ 满足 $\left<\right$ $\left<x,D\right>\in I(p_\in)$
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{r}(x_1)$ 的充要条件是

在 $I(p_\in)$ 的角度上满足任意 $a\in M$ 若有 $\left<a,v(x_1)\right>\in I(p_\in)$ 则存在 $\{b,c\}\in M$

使得 $\mathcal{M}\vDash\phi_{ord}(x_1,x_2,x_3)[b,c,a]$ 。其中 $\phi_\varnothing$ [[MK709][判断空集]]， $\phi_{ord}$ [[MK714][判断有序]]
