...
除[[~/文档/note/数理逻辑/sllj.org::MK105][固定符号]]外仅有一个[[~/文档/note/数理逻辑/sllj.org::MK110][谓词符号]] $p_\in$ 且根据[[~/文档/note/数理逻辑/sllj.org::MK11][符号假定]]有 $\pi_P(p_\in)=2$ 即 $\mathcal{A}_{zfc}=\{p_\in\}$
...
即对于任意结合 $A$ 其[[MK25][幂集]] $B$ 存在且亦为集合。其中 $\phi_\subset$ 为[[MK20][子集关系]]
...
即对于任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash\phi_{sep}$ 满足任意 $\{P_1,...,P_n,U\}\subset M$ 存在 $S\in M$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 假设 $\mathcal{M}\vDash(\exists x(\forall y (p_\in(y,x))))$ 则存在 $a\in M$ 满足任意 $y\in M$ 均有 $y\in a$

定义 $b=\left\{x\big|\left<x,a\right>\in I(p_\in)\land\left(\mathcal{M}\vDash(p_\in(x,x_1)\land(\lnot p_\in(x, x)))[a]\right)\right\}$ 根据[[MK9][分离模式]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得
...
又根据 $M$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\lnot(\exists x(\forall y (p_\in(y,x_1)))))$

****** 若 $\left<b,b\right>\in I(p_\in)$

根据 $b$ 的定义以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\left<b,b\right>\not\in I(p_\in)$ ，矛盾

****** 若 $\left<b,b\right>\not\in I(p_\in)$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]有 $\mathcal{M}\vDash(\lnot p_\in(x_1, x_1))[b]$ 又由于 $\mathcal{M}\vDash(\forall y (p_\in(y,x_1)))[a]$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\mathcal{M}\vDash p_\in(x_2, x_1)[a,b]$

即有 $\mathcal{M}\vDash(p_\in(x_2,x_1)\land(\lnot p_\in(x_2, x_2)))[a,b]$ 则根据 $b$ 定义以及[[MK9][分离模式]]有 $\left<b,b\right>\in I(p_\in)$ ，矛盾

**** 推论2

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 以及任意 $A\in M$ 定义 $B=\left\{b\big|\mathcal{M}\vDash(\forall x_3(p_\in(x_3,x_1)\to p_\in(x_2,x_3)))[A,b]\right\}$

对于任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 满足 $v(x_1)=A$ 有
...
若 $\left<\mathcal{M},v\right>\vDash\phi_\varnothing(x_1)$ 则根据[[MK709][定义]]可得任意 $c\in M$ 均有 $\left<c,A\right>\not\in I(p_\in)$

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证任意 $b\in M$ 均有 $\mathcal{M}\vDash(\forall x_3(p_\in(x_3,x_1)\to p_\in(x_2,x_3)))[A,b]$ 即 $B=M$ 根据[[MK708][推论1]]可得 $B\not\in M$

****** 证明（2）

若 $\left<\mathcal{M},v\right>\not\vDash\phi_\varnothing(x_1)$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]存在 $a\in M$ 满足 $\left<a,A\right>\in I(p_\in)$

定义 $\phi=(\forall x_4(p_\in(x_4,x_3)\leftrightarrow(p_\in(x_4,x_2)\land(\forall x_5(p_\in(x_5,x_1)\to p_\in(x_4,x_5))))))$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]以及[[MK9][分离模式]]易证 $\mathcal{M}\vDash(\exists x_3\phi)[A,a]$ ，即根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]存在 $c\in M$ 使得 $\mathcal{M}\vDash\phi[A,a,c]$

进一步根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $c=B$

**** 空集存在<<MK6>>

$T_s\vdash\left(\exists x_1\big(\forall x_2(\lnot p_\in(x_2,x_1))\big)\right)$ 即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 存在 $\varnothing\in M$ 满足任意 $b\in M$ 均有 $\left<b,\varnothing\right>\not\in I(p_\in)$

***** 证明

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK509][引理5]]易证存在 $a\in M$ 定义 $y=\left\{x\big|\mathcal{M}\vDash((x\in p_1)\land(\lnot(x\hat{=}x)))[a]\right\}$ 根据[[MK9][分离模式]]可得 $y\in M$

显然根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]以及[[~/文档/note/数理逻辑/sllj.org::MK197][恒等律]]有 $\mathcal{M}\vDash(\forall x(x\hat{=}x))$ 即对于任意 $b\in M$ 均有 $b=b$ 故可得对于任意 $b\in M$ 均有 $b\not\in y$

故可得 $\left<\mathcal{M},v\right>\vDash\big(\forall x_2(\lnot p_\in(x_2,x_1))\big)$ 其中 $v(x_1)=y$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]有 $\left<\mathcal{M},v\right>\vDash\left(\exists x_1\big(\forall x_2(\lnot p_\in(x_2,x_1))\big)\right)$

又由于 $\mathcal{M}$ 的任意性，根据[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash\left(\exists x_1\big(\forall x_2(\lnot p_\in(x_2,x_1))\big)\right)$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 根据[[MK6][空集存在]]可得存在 $\varnothing\in M$ 满足对于任意 $a\in M$ 均有 $a\not\in\varnothing$

若存在 $y\in M$ 亦满足上述条件，则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $\left<\mathcal{M},v\right>\vDash(\forall x_1(p_\in(x_1,x_2)\leftrightarrow p_\in(x_1,x_3)))$ 其中 $v(x_2)=\varnothing,v(x_3)=y$

则根据[[MK5][外延公理]]有 $\left<\mathcal{M},v\right>\vDash(x_2\hat{=}x_3)$ 即根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $y =\varnothing$

***** 空集是任意集合的子集

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 满足 $v(x_3)=\varnothing$ 均有 $\left<\mathcal{M},v\right>\vDash(\forall x_1(\forall x_2(p_\in(x_1,x_3)\to p_\in(x_1,x_2))))$

即根据[[MK20][子集]]可得任意 $a\in M$ 均有 $\varnothing\subset a$

****** 证明

由于 $v(x_3)=\varnothing$ 则根据[[MK6][空集]]可得任意 $a\in M$ 均有 $a\not\in\varnothing$ 即有 $\left<\mathcal{M},v\right>\vDash(\lnot p_\in(x_1,x_3))$

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]显然易证 $\left<\mathcal{M},v\right>\vDash(\forall x_1(\forall x_2(p_\in(x_1,x_3)\to p_\in(x_1,x_2))))$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 显然有 $\mathcal{M}\vDash\phi_{par}$ 则对于任意 $a\in M$ 取[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 满足 $v(x_1)=v(x_2)=a$

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]以及[[MK7][配对公理]]可得存在 $b\in M$ 使得任意 $c\in M$ 有 $c\in b$ 的充要条件为 $c=v(x_1)$ 或者 $c=v(x_2)$
...
其中 $\phi_\varnothing$ 为[[MK6][空集]]表达式。即存在一个集合 $A$ 包含[[MK6][空集]]，且任意元素 $x$ 若 $x\in A$ 则其后继 $x\cup\{x\}$ 也属于 $A$

定义所有满足 $((\varnothing\in A)\land ((x\in A)\rightarrow (x\cup\{x\}\in A)))$ 的集合为 $Inf$

*** 替换公理模式

对于任意 $\phi\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 满足 $\phi$ 中的[[~/文档/note/数理逻辑/sllj.org::MK140][自由变元]] $S\subset\{p_1,...,p_n,x_1,x_2\}$
...
其中 $\text{IsFunc}$ 是[[MK135][表达式确定函数]]，则对于任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash\phi_{sub}$ 均有对于任意给定 $\{P_1,...,P_n,a_1\}\subset M$ 满足对于任意 $a_2\in a_1$ 均有唯一 $b\in M$
...
即相当于若存在 $\phi$ 确定的[[MK194][函数]] $f:a_1\to M$ 满足 $f(a_1)=b$ 则所有 $f$ 关于 $a_1$ 的相组成的集合 $f_{im}=\left\{a\big|\exists b\left(\left<b,a_1\right>\in I(p_\in)\land f(b)=a\right)\right\}\in M$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 假设 $\mathcal{M}\vDash(\exists x_1p_\in(x_1,x_1))$ 则存在 $a\in M$ 且 $a\in a$

则根据[[MK7][配对公理]]设 $x=a,y=a$ 则存在 $b=\{a,a\}$ ，根据[[MK5][外延公理]]即得 $b=\{a\}$

显然 $b$ 非空且仅有一个元素 $a$ ，则 $a\cap b=a\cap\{a\}$ 由于 $a\in a$

故可得 $a=a\cap b$ 且 $\left<a,b\right>\in I(p_\in)$ ，根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证与[[MK706][正则公理]]矛盾。

故假设不成立，即 $\mathcal{M}\not\vDash(\exists x_1p_\in(x_1,x_1))$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $\mathcal{M}\vDash(\lnot(\exists x_1p_\in(x_1,x_1)))$

又根据 $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\lnot(\exists x_1p_\in(x_1,x_1)))$

**** 引理2<<MK606>>（证明需要修正）

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 不存在无限递降序列 $\\...\in A_n\in...\in A_2\in A_1\in M$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 对于任意 $\{x,y\}\subset M$ 有：
...
故可得 $x\cap S\ne\varnothing$ 同理可证 $y\cap S\ne\varnothing$ 故与[[MK706][正则公理]]矛盾，则假设不成立。即 $y\not\in x$
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 有任意 $A\in M$ 满足任意 $a\in A$ 不为[[MK6][空集]]，则存在 $B\in M$

满足对于任意 $a\in A$ 存在唯一 $c\in M$ 满足 $\{\left<c,a\right>,\left<c,B\right>\}\subset I(p_\in)$

相当存在 $B$ 使得 $f=\left\{\left<a,c\right>\big|\left(\left<a,A\right>\in I(p_\in)\right)\land\left(\left<c,a\right>\in I(p_\in)\right)\land\left(\left<c,B\right>\in I(p_\in)\right)\right\}$ 是[[MK194][函数]]

该 $f$ 亦被称为选择函数

** 相关定义

*** [[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]确定[[MK194][函数]]<<MK135>>

对于任意[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]] $\phi$ 满足其[[~/文档/note/数理逻辑/sllj.org::MK140][自由变元]] $S\subset\{x_1,...,x_{n+1}\}$ 
...
则对于任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 若满足 $\left<\mathcal{M},v\right>\vDash\text{IsFunc}(\phi,x_1,...,x_n)$
...
定义 $\phi_\varnothing(x_i)=(\forall x_1(\lnot p_\in(x_1,x_i)))$ 显然任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 满足 $\left<\mathcal{M},v\right>\vDash\phi_\varnothing(x_i)$

有任意 $b\in M$ 均有 $\left<b,v(x_i)\right>\not\in I(p_\in)$ 记作 $\Phi_\varnothing(v(x_i))$

*** 类<<MK707>>

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 以及任意[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]] $\phi\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 其[[~/文档/note/数理逻辑/sllj.org::MK140][自由变元]]为 $p_1,...,p_n,x$ 对于任意给定 $\left<P_1,...,P_n\right>\in M^n$

定义 $C=\left\{c\big|c\in M\land \mathcal{M}\vDash\phi[P_1,...,P_n,c]\right\}$ 则称 $C$ 为类

**** 性质1（真类）

存在 $C$ 是[[MK707][类]]满足不存在 $B\in M$ 满足对于任意 $b$ 满足 $\left<b,B\right>\in I(p_\in)$ 的充要条件为 $b\in C$ ，即 $C$ 不是集合

不是集合的[[MK707][类]]称为真类

***** 证明

定义 $R=\left\{c\big|c\in M\land\mathcal{M}\vDash(\lnot p_\in(x,x))[c]\right\}$ 显然根据[[MK707][定义]]可得 $R$ 是[[MK707][类]]，根据[[MK26][引理1]]可得 $\mathcal{M}\vDash(\lnot(\exists x_1p_\in(x_1,x_1)))$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得不存在 $b\in\mathcal{M}$ 满足 $\left<b,b\right>\in I(p_\in)$ 即可得任意 $c\in M$ 均有 $\mathcal{M}\vDash(\lnot p_\in(x,x))[c]$

故任意 $c\in M$ 均有 $c\in R$ 假设存在 $B\in M$ 满足任意 $b$ 满足 $\left<b,B\right>\in I(p_\in)$ 的充要条件为 $b\in R$

即可得任意 $c\in M$ 均有 $\left<c,B\right>\in I(p_\in)$ 即 $\mathcal{M}\vDash(\exists x(\forall y (p_\in(y,x))))$ 显然与[[MK708][推论1]]矛盾。
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_\subset(x_1,x_2)$ 的充要条件是
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 若有 $\left<\mathcal{M},v\right>\vDash(\phi_\subset(x_1,x_2)\land \phi_\subset(x_2,x_1))$

则根据[[MK20][定义]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证对于任意 $b$ 有 $\left<b,v(x_1)\right>\in I(p_\in)$ 的充要条件是 $\left<b,v(x_2)\right>\in I(p_\in)$

即根据[[MK5][外延公理]]可得 $\left<\mathcal{M},v\right>\vDash(x_1\hat{=}x_2)$ 故有 $\left<\mathcal{M},v\right>\vDash((\phi_\subset(x_1,x_2)\land \phi_\subset(x_2,x_1))\to(x_1\hat{=}x_2))$

又由于 $v$ 的任意性可得 $\mathcal{M}\vDash(\forall x_1(\forall x_2((\phi_\subset(x_1,x_2)\land \phi_\subset(x_2,x_1))\to(x_1\hat{=}x_2))))$

结合 $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\forall x_1(\forall x_2((\phi_\subset(x_1,x_2)\land \phi_\subset(x_2,x_1))\to(x_1\hat{=}x_2))))$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 若有 $\left<\mathcal{M},v\right>\vDash(\phi_\subset(x_1,x_2)\land \phi_\subset(x_2,x_3))$

则根据[[MK20][定义]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证对于任意 $b$ ：
...
结合 $v$ 的任意性， $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有易证 $T_s\vdash(\forall x_1(\forall x_2(\forall x_3((\phi_\subset(x_1,x_2)\land \phi_\subset(x_2,x_3))\to \phi_\subset(x_1,x_3)))))$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 若有 $\left<\mathcal{M},v\right>\vDash\phi_\varnothing(x_1)$ 则根据[[MK709][空集判断]]易证

对于任意 $a\in M$ 均有 $\left<a,v(x_1)\right>\not\in I(p_\in)$ ，则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\left<\mathcal{M},v\right>\vDash(\forall x_3(p_\in(x_3,x_1)\to p_\in (x_3,x_2)))$

则结合 $v$ 的任意性， $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有易证 $T_s\vdash(\forall x_1(\forall x_2(\phi_\varnothing(x_1)\to \phi_\subset(x_1,x_2))))$

*** 二元关系

定义 $\phi_r(x_1)=((\forall y_1(p_\in(y_1,x_1)\to(\exists y_2..y_3\phi_{ord}(y_2,y_3,y_1))))\land(\lnot\phi_\varnothing(x_1)))$

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{r}(x_1)$ 的充要条件是 $\Phi_\varnothing(v(x_1))$ 不成立

且任意 $x\in M$ 若有 $\left<x,v(x_1)\right>\in I(p_\in)$ 则存在 $\{a_1,a_2\}\subset M$ 满足 $x=\Phi_{ord}(\mathcal{M},a_1,a_2)$

其中 $\phi_\varnothing,\Phi_\varnothing$ [[MK709][判断空集]]， $\phi_{ord},\Phi_{ord}$ 为[[MK714][求序]]。记作 $\Phi_r(\mathcal{M},v(x_1))$

*** $n$ 元关系<<MK736>>

定义 $\phi_{r_n}(x_1)=((\forall y_1(p_\in(y_1,x_1)\to(\exists y_2..y_{n+1}\phi_{ord_n}(y_2,...,y_{n+1},y_1))))\land(\lnot\phi_\varnothing(x_1)))$

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{r_n}(x_1)$ 的充要条件是 $\Phi_\varnothing(v(x_1))$ 不成立

且任意 $x\in M$ 若有 $\left<x,v(x_1)\right>\in I(p_\in)$ 则存在 $\{a_1,...,a_n\}\subset M$ 满足 $x=\Phi_{ord_n}(\mathcal{M},a_1,...,a_n)$

其中 $\phi_\varnothing,\Phi_\varnothing$ [[MK709][判断空集]]， $\phi_{ord_n},\Phi_{ord_n}$ 为[[MK735][ $n$ 元求序]]。记作 $\Phi_{r_n}(\mathcal{M},v(x_1))$

*** 函数

定义 $\phi_f(x_1)=(\phi_r(x_1)\land(\forall y_1(\lnot(\exists y_2(p_\in(y_1,x_1)\land p_\in(y_2,x_1)\land(\exists y_3..y_5(\phi_{ord}(y_3,y_4,y_1)\land\phi_{ord}(y_3,y_5,y_2))))))))$
...
定义 $\phi_{pair}(x_1,x_2,x_3)=\left(\forall y_1(p_\in(y_1,x_3)\leftrightarrow((y_1\hat{=}x_1)\lor(y_1\hat{=}x_2)))\rigth)$

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{pair}(x_1,x_2)$ 的充要条件是
...
其中 $\phi_\cup$ 是[[MK30][并集]]。即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{pair_n}(x_1,...x_n,x_{n+1})$ 的充要条件是
...
根据[[MK713][配对存在]]，[[MK710][并集存在]]以及数学归纳法易证
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{ord}(x_1,x_2,x_3)$ 的充要条件是

对于任意 $x\in M$ 有 $\left<x,v(x_3)\right>\in I(p_\in)$ 的充要条件是 $x=\Phi_{pair}(\mathcal{M},v(x_1),v(x_1)))$

或者 $x=\Phi_{pair}(\mathcal{M},v(x_1),v(x_2)))$ 。其中 $\Phi_{pair}$ 为[[MK712][配对]]
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{ord_n}(x_1,...x_n,x_{n+1})$ 的充要条件是 $v(x_{n+1})=\Phi_{ord}(\mathcal{M},\Phi_{ord_{n-1}}(\mathcal{M},v(x_1),...,v(x_{n-1})),v(x_n))$
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_\cup(x_1,x_2)$ 的充要条件是
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 由[[MK8][并集公理]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\mathcal{M}\vDash(\forall x_1(\exists x_2(\phi_\cup(x_1,x_2))))$

结合 $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\forall x_1(\exists x_2(\phi_\cup(x_1,x_2))))$

***** $n$ 元并<<MK721>>

定义 $\phi_{\cup_n}(x_1,...,x_n,x_{n+1})=(\exists y_1(\phi_{pair_n}(x_1,...,x_n,y_1)\land\phi_\cup(y_1,x_{n+1})))$ 其中 $\phi_{pair_n}$ 是[[MK717][ $n$ 元配对]] $\phi_\cup$ 是[[MK30][并集]]

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash\phi_{\cup_n}(x_1,...,x_n,x_{n+1})$ 的充要条件是
...
根据[[MK718][ $n$ 元配对存在]]以及[[MK710][并集存在]]易证

***** 元素并<<MK723>>

定义 $\phi_{\cup_\in}(x_1,x_2,x_3)=(\forall y_1(p_\in(y_1,x_3)\leftrightarrow(\exists y_2(p_\in(y_2,x_1)\land\phi_\cup(y_2,x_2,y_1)))))$ 其中 $\phi_\cup$ 是[[MK30][并集]]

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash\phi_{\cup_{\in}}(x_1,x_2,x_3)$ 的充要条件是

对于任意 $x\in M$ 有 $\left<x,v(x_{3})\right>\in I(p_\in)$ 的充要条件是存在 $y\in M$ 满足 $\left<y,v(x_1)\right>$ 且 $x=\Phi_{\cup_2}(\mathcal{M},y,v(x_2))$

其中 $\Phi_{\cup_2}$ 为[[MK721][ $2$ 元并]]。即做 $v(x_3)=\Phi_{\cup_\in}(\mathcal{M},v(x_1),v(x_2))$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 任取 $\{A,B\}\subset M$ 根据以下步骤可得存在 $D\in M$ 使得 $\mathcal{M}\vDash\phi_{\cup_\in}(x_1,x_2,x_3)[A,B,D]$

即根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]有 $\mathcal{M}\vDash\vdash(\forall x_1..x_2(\exists x_3(\phi_{\cup_\in}(x_1,x_2,x_3))))$ 进一步根据 $\mathcal{M}$ 任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]可得 $T_s\vdash(\forall x_1..x_2(\exists x_3(\phi_{\cup_\in}(x_1,x_2,x_3))))$

******** 构造集合

（1） 根据[[MK30][并集]]可得存在 $C_1=\Phi_\cup(\mathcal{M},A)$ ，根据[[MK721][ $2$ 元并]]可得存在 $C_2=\Phi_{\cup_2}(\mathcal{M},C_1,B)$

（3） 根据[[MK25][幂集]]可得存在 $C_3=\Phi_\rho(\mathcal{M},C_2)$

（4） 定义 $\phi=(\exists y_1(p_\in(y_1,x_1)\land\phi_\cup(y_1,x_2,y_3)))$ 根据[[MK716][分离]]可得存在 $D=\Phi_{sep}(\mathcal{M},\phi[A,B],y_3,C_3)$

******** 证明 $D$ 满足 $D=\Phi_{\cup_\in}(\mathcal{M},A,B)$

********* 充分

由 $\phi$ 定义以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证对于任意 $x\in M$ 若有 $\left<x,D\right>\in I(p_\in)$ 则存在 $y\in M$ 满足 $\left<y,A\right>\in I(p_\in)$ 且 $x=\Phi_{\cup_2}(\mathcal{M},y,B)$

其中 $\Phi_{\cup_2}$ 为[[MK721][ $2$ 元并]]

********* 必要

对于任意 $\{A,B\}\subset M$ 有：

（1） 对于任意 $\left<a,A\right>\in I(p_\in)$ 根据[[MK30][并集]]可得任意 $x\in M$ 满足 $\left<x,a\right>\in I(p_\in)$ 均有 $\left<x,C_1\right>\in I(p_\in)$ ，

（2） 根据[[MK721][ $2$ 元并]]可证任意 $x\in M$ 满足 $\left<x,a\right>\in I(p_\in)$ 均有 $\left<x,C_2\right>\in I(p_\in)$ ，同理易证对于任意 $\left<b,B\right>\in I(p_\in)$ 均有 $\left<b,C_2\right>\in I(p_\in)$

（3） 根据[[MK721][ $2$ 元并]]易证存在 $d\in M$ 满足 $d=\Phi_{\cup_2}(\mathcal{M},a,B)$

（4） 根据（2）（3)以及[[MK20][子集]]易证 $\Phi_\subset(\mathcal{M},d,C_2)$ ，进一步根据[[MK25][幂集]]可得 $\left<d,C_3\right>\in I(p_\in)$ 

（5） 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\mathcal{M}\vDash\phi[\left<x_1,x_2,y_3\right>,\left<A,B,d\right>]$ ，则根据[[MK716][分离]]可得 $\left<d,D\right>\in I(p_\in)$

综上所述，对于任意 $\left<a,A\right>\in I(p_\in)$ 均存在 $d\in M$ 满足 $d=\Phi_{\cup_2}(\mathcal{M},a,B)$ 且 $\left<d,D\right>\in I(p_\in)$

**** 交集<<MK31>><<MK35>>

定义 $\phi_\cap(x_1,x_2)=(\exists y_1(\phi_\cup(x_1,y_1)\land\phi_{sep}((\forall y_3(p_\in(y_3,x_1)\to p_\in(x_3,y_3))),x_3,x_1,x_2)))$ 其中 $\phi_\cup$ 为[[MK30][并集]] $\phi_{sep}$ 为[[MK716][分离]]

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_\cap(x_1,x_2)$ 的充要条件是
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 根据[[MK710][并集存在型]]可得对于任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 均有 $\left<\mathcal{M},v\right>\vDash(\exists x_2(\phi_\cup(x_1,x_2)))$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得存在 $v'$ 满足 $v'(x_1)=v(x_1)$ 以及 $\left<\mathcal{M},v'\right>\vDash \phi_\cup(x_1,x_5)$ 

根据[[MK9][分离模式]]可得 $\left<\mathcal{M},v'\right>\vDash (\exists x_2(\forall x_3(p_\in(x_3,x_2)\leftrightarrow(p_\in(x_3,x_5)\land\phi))))$ 即根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得存在 $v''$ 满足 $v''(x_1)=v'(x_1),v''(x_5)=v'(x_5)$ 
...
根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得对于任意 $v^*$ 满足 $v^*(x_1)=v''(x_1),v^*(x_2)=v''(x_2)$ 均有 $\left<\mathcal{M},v^*\right>\vDash(\exists x_5(\phi_\cup(x_1,x_5)\land(\forall x_3(p_\in(x_3,x_2)\leftrightarrow(p_\in(x_3,x_5)\land\phi)))))$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得 $\left<\mathcal{M},v\right>\vDash(\exists x_2\phi_\cap(x_1,x_2))$ ，又根据 $v$ 的任意性有 $\mathcal{M}\vDash(\forall x_1(\exists x_2\phi_\cap(x_1,x_2)))$

结合 $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\forall x_1(\exists x_2(\phi_\cap(x_1,x_2))))$

***** $n$ 元交<<MK720>>

定义 $\phi_{\cap_n}(x_1,...,x_n,x_{n+1})=(\exists y_1(\phi_{pair_n}(x_1,...,x_n,y_1)\land\phi_\cap(y_1,x_{n+1})))$ 其中 $\phi_{pair_n}$ 是[[MK717][ $n$ 元配对]] $\phi_\cap$ 是[[MK31][交集]]

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash\phi_{\cap_n}(x_1,...,x_n,x_{n+1})$ 的充要条件是
...
根据[[MK718][ $n$ 元配对存在]]以及[[MK719][交集存在]]易证

***** 元素交<<MK724>>

定义 $\phi_{\cap_\in}(x_1,x_2,x_3)=(\forall y_1(p_\in(y_1,x_3)\leftrightarrow(\exists y_2(p_\in(y_2,x_1)\land\phi_\cap(y_2,x_2,y_1)))))$ 其中 $\phi_\cap$ 是[[MK31][交集]]

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash\phi_{\cap_{\in}}(x_1,x_2,x_3)$ 的充要条件是

对于任意 $x\in M$ 有 $\left<x,v(x_{3})\right>\in I(p_\in)$ 的充要条件是存在 $y\in M$ 满足 $\left<y,v(x_1)\right>$ 且 $x=\Phi_{\cap_2}(\mathcal{M},y,v(x_2))$

其中 $\Phi_{\cap_2}$ 为[[MK720][ $2$ 元交]]。即做 $v(x_3)=\Phi_{\cap_\in}(\mathcal{M},v(x_1),v(x_2))$
...
定义 $\phi_\rho(x_1,x_2)=(\forall y_1(p_\in(y_1,x_2)\leftrightarrow\phi_\subset(y_1,x_1)))$ 其中 $\phi_\subset$ 为[[MK20][子集]]关系

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_\rho(x_1,x_2)$ 的充要条件是

对于任意 $x\in M$ 有 $\left<x,v(x_2)\right>\in I(p_\in)$ 的充要条件是存在 $y\in M$ 满足 $\Phi_\subset(y,v(x_1))$ 。其中 $\Phi_\subset$ 为[[MK20][子集]]关系
...
定义 $\phi_{sep}(\phi,x_1,y_1,y_2)=(\forall x_1(p_\in(x_1,y_2)\leftrightarrow(p_\in(x_1,y_1)\land\phi)))$ 其中 $\phi$ 中含[[~/文档/note/数理逻辑/sllj.org::MK140][自由变元]] $p_1,...,p_n,x_1$

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_{sep}(\phi,y_1,y_2,x_1)$ 的充要条件是

对于任意 $a\in M$ 有 $\left<a,v(y_2)\right>\in I(p_\in)$ 的充要条件
是 $\left<a,v(y_1)\right>\in I(p_\in)$ 且对于[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v'$ 满足 $v'(u)=\begin{cases}a&u=x_1\\v(u)&u=p_i,1\leq i\leq n\end{cases}$ 有 $\left<\mathcal{M},v'\right>\vDash\phi$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 对于任意 $U\in M,\phi\in\mathcal{L}_{\mathcal{A}_{zfc}}$ 假设 $\phi$ 中含[[~/文档/note/数理逻辑/sllj.org::MK140][自由变元]] $p_1,...,p_n,x_1$ 对于给定 $\left<P_1,...,P_n\right>\in M^n$
...
均有 $\left<a,U\right>\in I(p_\in)$ 且对于[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v'$ 满足 $v'(u)=\begin{cases}a&u=x_1\\P_i&u=p_i,1\leq i\leq n\end{cases}$ 有 $\left<\mathcal{M},v'\right>\vDash\phi$

同理可得 $\left<a,S_2\right>\in I(p_\in)$ 。同理易证 $\left<a,S_2\right>\in I(p_\in)$ 可得 $\left<a,S_1\right>\in I(p_\in)$ 。

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\mathcal{M}\vDash(\forall y_1(p_\in(y_1,x_1)\leftrightarrow p_\in(y_1,x_2)))[S_1,S_2]$ 则根据[[MK5][外延公理]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $S_1=S_2$

**** 差<<MK32>>

定义 $\phi_-(x_1,x_2,x_3)=(\forall y_1(p_\in(y_1,x_3)\leftrightarrow(p_in(y_1,x_1)\land(\lnot(p_\in(y_1,x_2))))))$

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_-(x_1,x_2,x_3)$ 的充要条件是
...
即 $\phi$ 满足[[MK9][分离模式]] $T_s\vdash\phi$

**** 对称差

定义 $\phi_\Delta(x_1,x_2,x_3)=(\exists y_{1..2}(\phi_-(x_1,x_2,y_1)\land \phi_-(x_2,x_1,y_2)\land\phi_\cup(y_1,y_2,x_3)))$ 其中 $\phi_-$ 为[[MK32][差定义]] $\phi_\cup$ 为[[MK30][并集三元定义]]

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_\Delta(x_1,x_2,x_3)$ 的充要条件是
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 以及任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 根据[[MK711][差存在]]，[[MK710][并存在]]以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]易证
...
即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash \phi_\times(x_1,x_2,x_3)$ 的充要条件是
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 任取 $\{A,B\}\subset M$ 根据以下步骤可得存在 $D\in M$ 使得 $\mathcal{M}\vDash\phi_\times(x_1,x_2,x_3)[A,B,D]$

即根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]有 $\mathcal{M}\vDash(\forall x_1..x_2(\exists x_3\phi_\times(x_1,x_2,x_3)))$ 进一步根据 $\mathcal{M}$ 任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]可得 $T_s\vdash(\forall x_1..x_2(\exists x_3\phi_\times(x_1,x_2,x_3)))$

******* 构造集合 $D$

（1） 根据[[MK713][配对存在]]可得存在 $C_1\in M$ 满足 $C_1=\Phi_{pir}(\mathcal{M},A,B)$

（2） 进一步根据[[MK710][并集存在]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证存在 $C_2\in M$ 满足 $C_2=\Phi_\cup(\mathcal{M},C_1)$

（3） 根据[[MK25][幂集]]可得存在 $C_3\in M$ 满足 $C_3=\Phi_\rho(\mathcal{M},C_2)$

（4） 同理可得存在 $C_4\in M$ 满足 $C_4=\Phi_\rho(\mathcal{M},C_3)$

（5） 定义 $\phi=(\exists y_1..y_2(p_\in(y_1,p_1)\land p_\in(y_2,p_2)\land\phi_{ord}(y_1,y_2,x_1)))$ 则根据[[MK716][分离]]可得存在 $D\in M$ 满足 $D=\Phi_{sep}(\mathcal{M},\phi[A,B],x_1,C_4)$
...
则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证存在 $\{a,b\}\subset M$ 满足 $\{\left<a,A\right>,\left<b,B\right>\}\subset I(p_\in)$ 且 $d=\Phi_{ord}(\mathcal{M},a,b)$

******** 必要

对于任意 $\{a,b\}\subset M$ 满足 $\{\left<a,A\right>,\left<b,B\right>\}\subset I(p_\in)$ ：

（1） 根据[[MK712][配对]]以及[[MK30][并集]]定义易证 $\{\left<a,C_2\right>,\left<b,C_2\right>\}\subset I(p_\in)$

（2） 根据[[MK712][配对]]易证存在 $\{a',b'\}\subset M$ 使得 $a'=\Phi_{pair}(\mathcal{M},a,a),b'=\Phi_{pair}(\mathcal{M},a,b)$ ，显然根据[[MK20][子集]]易证 $\Phi_\subset(\mathcal{M},a',C_2),\Phi_\subset(\mathcal{M},b',C_2)$

（3） 根据上一步以及[[MK25][幂集]]易证 $\{\left<a',C_3\right>,\left<b',C_3\right>\}\subset I(p_\in)$

（4） 根据[[MK712][配对]]易证存在 $d'\in M$ 使得 $d'=\Phi_{pair}(\mathcal{M},a',b')=\Phi_{pair}(\mathcal{M},\Phi_{pair}(\mathcal{M},a,a),\Phi_{pair}(\mathcal{M},a,b))=\Phi_{ord}(\mathcal{M},a,b)$

（5） 显然根据[[MK20][子集]]易证 $\Phi_\subset(\mathcal{M},d',C_3)$ ，则根据[[MK25][幂集]]易证 $\left<d',C_4\right>\in I(p_\in)$

（6） 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证 $\mathcal{M}\vDash\phi[\left<p_1,p_2,x_1\right>,\left<A,B,d'\right>]$ ，则根据[[MK716][分离]]可得 $\left<d',D\right>\in I(p_\in)$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 任取 $\{A,B\}\subset M$ 若有 $A=B$ 则有 $\Phi_\cap(\mathcal{M},A)=\Phi_\cap(\mathcal{M},B),\Phi_\cup(\mathcal{M},A)=\Phi_\cup(\mathcal{M},B)$
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 任取 $\{A,B\}\subset M$ 有
...
其中 $\Phi_{\cup_2},\Phi_{\cap_2},\Phi_{\cup_\in},\Phi_{\cap_\in}$ 分别为[[MK721][ $2$ 元并]]，[[MK720][ $2$ 元交]]，[[MK723][元素并]]，[[MK724][元素交]]

***** 证明

仅证（1），其余同理可证，定义 $C_1=\Phi_{\cap_2}(\mathcal{M},A,\Phi_\cup(\mathcal{M},B)),C_2=\Phi_{\cup}(\mathcal{M},\Phi_{\cap_\in}(\mathcal{M},B,A))$

根据[[MK720][ $2$ 元交]]对于任意 $x\in M$ 有 $\left<x,C_1\right>\in I(p_\in)$ 的充要条件为 $\{\left<x,A\right>,\left<x,\Phi_\cup(\mathcal{M},B)\right>\}\subset I(p_\in)$

进一步根据[[MK30][并集]] $\left<x,\Phi_\cup(\mathcal{M},B)\right>\in I(p_\in)$ 的充要条件为存在 $y\in M$ 且 $\left<y,B\right>\in I(p_\in)$ 满足 $\left<x,y\right>\in I(p_\in)$
...
根据[[MK721][ $2$ 元并]]对于任意 $x\in M$ 有 $\left<x,C_2\right>\in I(p_\in)$ 的充要条件为存在 $z\in M$ 且 $\left<z,\Phi_{\cap_\in}(\mathcal{M},B,A)\right>\in I(p_\in)$ 满足 $\left<x,z\right>\in I(p_\in)$

进一步根据[[MK724][元素交]] $\left<z,\Phi_{\cap_\in}(\mathcal{M},B,A)\right>\in I(p_\in)$ 的充要条件为存在 $y\in M$ 满足 $\left<y,B\right>\in I(p_\in)$ 且 $z=\Phi_{\cap_2}(\mathcal{M},y,A)$

即 $\left<x,\Phi_{\cap_2}(\mathcal{M},y,A)\right>\in I(p_\in)$ 。

又根据[[MK720][ $2$ 元交]]可得 $\left<x,\Phi_{\cap_2}(\mathcal{M},y,A)\right>\in I(p_\in)$ 的充要条件是 $\{\left<x,y\right>,\left<x,A\right>\}\subset I(p_\in)$
...
即对于任意 $x\in M$ 有 $\left<x,C_1\right>\in I(p_\in)$ 的充要条件为 $\left<x,C_2\right>\in I(p_\in)$ 则根据[[MK5][外延公理]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 若有 $A\in M$ 的每一个[[MK2][元素]]都是集合 $B\in M$ 的元素，则称 $A$ 为 $B$ 的子集，记作 $A\subset B$ 。
...
当 $A=B$ 时，根据[[MK5][外延公理]]，[[MK380][等价等值式]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]有：
...
根据[[MK27][定义]]有[[MK200][双射函数]] $\sigma:C\to A$ 以及[[MK195][单射函数]] $\eta:A\to B$

则显然 $g(c)=\eta(\sigma(c)),c\in C$ 是 $C\to B$ 的[[MK195][单射函数]]，故有 $|C|\leq|B|$
...
根据定义存在[[MK195][单射]] $f_{A,B}:A\to B, f_{B,C}:B\to C$ 对于任意 $\{a_1,a_2\}\subset A\land a_1\ne a_2$ 可得 $f_{A,B}(a_2)\ne f_{A,B}(a_2)$

同理可得 $f_{B,C}(f_{A,B}(a_2))\ne f_{B,C}(f_{A,B}(a_2))$ 又根据[[MK202][复合]]可得 $f_{B,C}\cdot f_{A,B}$ 以为[[MK194][函数]]，故可得 $f_{B,C}\cdot f_{A,B}$ 是 $A$ 到 $C$ 的[[MK195][单射]]
...
假设 $|A|=|C|$ 则根据[[MK725][定理6]]有 $|B|\leq|A|$ ，有根据[[MK727][定理4]]有 $|A|\not<|B|$ 即与条件矛盾
...
定义 $\phi(A)$ 为 $A$ 是 $X$ 的[[MK622][滤子]]的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]，则定义 $D=\left\{E\big|E\in\rho(X)\land F\subset E\land \phi(E)\right\}$
...
若集合 $A$ 以[[MK15][归纳定义]]，若 $P(x)$ 是[[~/文档/note/数理逻辑/sllj.org::MK129][谓词公式]]其中 $x$ 是命题变元。要证明 $\forall x(x\in A\rightarrow P(x))$ 仅需证明：
...
假设集合 $A$ 有[[MK15][归纳定义]]，[[~/文档/note/数理逻辑/sllj.org::MK129][谓词公式]] $P$ 且两个条件均成立，即：
...
显然 $\forall c((c\in C)\rightarrow (x\in c))$ 是[[MK468][集合论语言]]的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]，又根据[[MK8][并集公理]]可得 $\bigcup C$ 存在
...
根据[[MK154][分配率]]，[[MK14][描述法定义]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $x$
...
根据[[MK37][可数性质]]、[[~/文档/note/数理逻辑/sllj.org::MK183][辖域变化]]、[[MK39][交换律]]以及[[MK29][运算定义]]
...
则根据[[MK2][成员定义]]，[[MK328][德摩根律]]，[[MK154][分配率]]，[[MK14][描述法定义]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $x$
...
根据[[MK2][成员定义]]，[[MK14][描述法定义]]，[[MK328][德摩根律]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $x$
...
根据[[MK37][可数性质]]、[[MK29][运算定义]]、[[~/文档/note/数理逻辑/sllj.org::MK48][蕴含等式]]、[[MK328][德摩根律]]、[[~/文档/note/数理逻辑/sllj.org::MK182][量词转换]]可得
...
[[MK44][可数]]集是[[MK29][势]]最小的[[MK43][无限集]]

****** 证明

根据[[MK614][定理1]]可得任意[[MK43][无限集]] $A$ 均有[[MK44][可数]]子集 $A'$ ，根据[[MK335][定理5]]有 $|\mathbb{N}|=|A'|\leq|A|$

根据[[MK728][定理7]]可得 $|\mathbb{N}|\leq|A|$ 又根据[[MK727][定理4]]可得 $|A|\not<|\mathbb{N}|$

综上所述，对于任意[[MK43][无限集]] $A$ 均有 $|A|\not<|\mathbb{N}|$ ，即 $\mathbb{N}$ 是[[MK27][势]]最小的[[MK43][无限集]]
...
即 $f$ 是 $(B\times\mathbb{N})\to A$ 的[[MK195][单射函数]]的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]。定义 $F=\left\{f\big|(\text{IsFunc}(f,B)\land B\subset A)\right\}$
...
即 $f$ 是 $A$ 到 $A\times A$ 的[[MK200][双射函数]]的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]，定义 $Y=\left\{f\big|A\subset X\land\text{IsFunc}(f,A)\right\}$
...
不妨设 $|A|\leq |B|$ 则易证存在[[MK195][单射]] $f:A\to B$ 进一步易证存在[[MK195][单射]] $g:A\times B\to B\times B$

即 $|A\times B|\leq |B\times B|$ 根据[[MK618][定理5]]可得 $|B\times B|=|B|$ 根据[[MK725][定理6]]可得 $|A\times B|\leq |B|$ 
...
若 $A$ 是[[MK44][可数集]]，则 $B=\left\{b\big|b\in\rho(A)\land |b|=n\right\}$ 亦是[[MK44][可数集]]，其中 $\rho(A)$ 是 $A$ 的[[MK25][幂集]]， $n\in\mathbb{N}$

****** 证明

根据[[MK618][定理5]]可得 $|A^n|=|A|$ 是[[MK44][可数集]]，其中 $A^n$ 是 $A$ 的 $n$ 次[[MK153][笛卡尔积]]

对于任意 $b=\{a_1,...,a_n\}\in B$ 根据[[MK151][有序组定义]]可定义 $g(b)=\left\{a\big|a\in A^n\land b\in a\right\}$ 

即 $A^n$ 中所有由 $\{a_1,...,a_n\}$ 组成的[[MK151][有序组]]，显然对于任意 $b\in B$ 有唯一 $c\subset A^n$ 满足 $g(b)=c$

故 $g$ 是[[MK194][函数]]，根据[[MK10][选择公理]]可得存在选择[[MK194][函数]] $f'$ 满足 $f'(g(b))\in g(b)$

最终定义 $f(b)=f'(g(b))$ 显然根据[[MK202][复合]]可得 $f$ 亦为[[MK194][函数]]

对于任意 $b_1\ne b_2$ 不妨设 $a^*\in b_1,a^*\not\in b_2$ 则根据[[MK151][有序组定义]]可得存在 $k_1\in f(b_1)$ 满足 $a^*\in k_1$

且不存在 $k_2\in f(b_2)$ 满足 $a^*\in k_2$ 则显然 $f(b_1)\ne f(b_2)$ 即 $f$ 是[[MK195][单射]]

即有 $|B|<|A^n|=|A|$ 显然 $B$ 是[[MK43][无限集]]，则根据[[MK45][定理1]]易证 $B$ 是[[MK44][可数集]]
...
代数数为整系数多项式的[[~/文档/note/高等代数/Algb-4-Multi-equ.org::MK52][根]]，若不是，则定义为超越数（例如 $\pi$ ）
...
定义 $g(x)=\sum_{i=0}^nb_ix^i$ 根据[[~/文档/note/高等代数/Algb-4-Multi-equ.org::MK5][相等]]可得若 $f(x)\ne g(x)$ 则至少有一组 $b_k\ne a_k$
...
进一步根据[[~/文档/note/高等代数/Algb-4-Multi-equ.org::MK87][根的个数]]可知 $f(x)$ 至多有 $n$ 个根，故 $\sigma$ 是[[MK198][满射]]
...
根据[[MK328][德摩根律]]、[[MK154][分配率]]以及[[MK39][交换律]]
...
故根据[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]以及[[MK14][描述法]]定义 $R\cdot S=\left\{<x,y>\big|\exists y(xRy\land ySz)\right\}$
...
根据[[MK162][定义]]，[[MK30][并运算]]，[[MK35][B不包含x]]，[[MK39][交换律]]，[[MK40][结合律]]，[[MK154][分配率]]，[[~/文档/note/数理逻辑/sllj.org::MK183][辖域变化]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]对于任意 $<x,y>$
...
根据[[MK162][定义]]以及[[MK161][逆]]运算，[[MK39][交换律]]，[[MK40][结合律]]对于任意 $<x,y>$
...
根据[[MK163][推论]]，[[MK162][关系定义]]，[[~/文档/note/数理逻辑/sllj.org::MK185][量词交换]]，[[~/文档/note/数理逻辑/sllj.org::MK183][辖域变化]]，[[MK39][交换律]]，[[MK40][结合律]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]对于任意 $<x,y>$
...
当 $R\cap R\sim\subset E_A$ 时，根据[[MK161][逆运算]]，[[MK31][交运算]]，[[MK20][子集定义]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]对于任意 $<x,y>$
...
同理可证若 $x\in[b]_R\rightarrow x\in[a]_R$ 根据[[MK380][等价等值式]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $x$ 有 $x\in[b]_R\leftrightarrow x\in[a]_R=1$
...
同理可证 $\forall y(y\in[b]_R\rightarrow y\in[a]_R)$ 故根据[[MK380][等价等值式]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $y$ 有 $y\in[b]_R\leftrightarrow y\in[a]_R=1$
...
若 $R$ 是 $A$ 上的[[MK176][等价关系]] $B=\left\{[a]_R\big|a\in A\right\}$ 则有 $|B|\leq|A|$

***** 证明

对于任意 $[a]_R\in B$ 根据[[MK10][选择公理]]可得存在选择函数 $f$ 满足 $f([a]_R)\in[a]_R$ 显然有 $f([a]_R)\in A$

显然对于任意 $[a]_R\ne[b]_R$ 不存在 $c\in[a]_R\land c\in[b]_R$ ，故可得 $f([a]_R)\ne f([b]_R)$

即 $f$ 是[[MK198][单射]]，根据[[MK27][等势]]可得 $|B|\leq|A|$
...
进一步根据[[MK380][等价传递]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]有 $\forall x(x\in A\leftrightarrow x\in\cup\pi)$
...
根据[[MK380][等价等值式]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $b$ 均有 $b\in B\leftrightarrow b\in[a]_R$
...
根据[[MK380][等价等值式]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $x$ 均有 $xRy\leftrightarrow xR'y$ 
...
则根据[[~/文档/note/实变函数/SBHS.org::MK265][推论3]]可得 $-\alpha\leq-a'$ 根据 $-A$ 定~/文档/note/离散数学/Disc_Math.org义可得 $\forall a\in A\rightarrow -\alpha\leq a$
...
任意[[MK384][良序关系]] $\left<X,\leq\right>$ 若对于命题 $P$ 满足：

（1） $P(x_0)$ 为真，其中 $x_0$ 是 $X$ 的[[MK186][最小元]]
...
对于任意[[MK384][良序关系]] $\left<X,\leq\right>$ 满足条件，假设存在 $a\in X$ 满足 $P(a)$ 为假

则定义集合 $S=\left\{a\big|a\in X\land(\lnot P(a))\right\}$ 显然 $S$ 非空，则根据[[MK384][良序]]可得存在 $s\in S$

满足对于任意 $s'\in S$ 均有 $s<s'$ 即 $S$ 上的[[MK186][最小元]]
...
根据[[MK40][结合律]]，[[MK30][并运算]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]，对于任意 $y$
...
若 $f:X\to Y$ 是[[MK198][满射]]，则存在 $g:Y\to X$ 是[[MK195][单射]]

****** 证明

定义 $f'(y)=\left\{x\big|x\in X\land f(x)=y\right\}$ 由于是[[MK198][满射]]，故对于任意 $y\in Y$ 均有 $f'(y)\ne\varnothing$

则根据[[MK10][选择公理]]可得存在选择函数 $h$ 满足 $h(f'(y))\in f'(y)$ ，则定义 $g:Y\to X$ 为 $g(y)=h(f'(y))$

对于任意 $y_1\ne y_2$ 假设 $g(y_1)=g(y_2)$ 则 $f(g(y_1))=y_1=y_2=f(g(y_2))$

即对于 $g(y_1)\in X$ 有 $\left<g(y_1),y_1\right>\in f,\left<g(y_1),y_2\right>\in f$ ，与 $f$ 是[[MK194][函数]]矛盾

故假设不成立，即对于任意 $y_1\ne y_2$ 均有 $g(y_1)\ne g(y_2)$ ，即 $g$ 是[[MK195][单射]]
...
若 $f_{A,B}:A\to B, f_{B,C}:B\to C$ 是两个[[MK195][单射]]，则 $f_{B,C}\cdot f_{A,B}$ 亦为[[MK195][单射]]

****** 证明

对于任意 $\{a_1,a_2\}\subset A\land a_1\ne a_2$ 可得 $f_{A,B}(a_2)\ne f_{A,B}(a_2)$ 同理可得 $f_{B,C}(f_{A,B}(a_2))\ne f_{B,C}(f_{A,B}(a_2))$

又根据[[MK202][复合]]可得 $f_{B,C}\cdot f_{A,B}$ 亦为[[MK194][函数]]，故可得 $f_{B,C}\cdot f_{A,B}$ 是 $A$ 到 $C$ 的[[MK195][单射]]

***** 多元函数符合<<MK737>>

若 $f_1,...,f_n:X\rightarrow Y,g:Y^n\rightarrow Z$ 则 $g\cdot (f_1,...,f_n)$ 满足对于任意 $\left<x_1,...,x_n\right>\in X^n$

均有 $g\cdot(f_1,...,f_n)=g(f_1(x_1),...,f_n(x_n))$ 亦为[[MK194][函数]]

****** 证明

由于 $f_i$ 均为[[MK194][函数]]，则对于任意 $x_i\in X$ 有且仅有一个 $f_i(x_i)\in Y$ 满足 $\left<x_i,f_i(x_i)\right>\in f_i$
...
根据[[MK203][逆函数定义]]，[[MK39][交换律]]以及[[~/文档/note/数理逻辑/sllj.org::MK40][替换原理]]可得对于任意 $<z,x>\in (f\cdot g)^{-1}$ 均有
