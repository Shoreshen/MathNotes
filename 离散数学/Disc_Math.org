...
除[[~/文档/note/数理逻辑/sllj.org::MK105][固定符号]]外仅有一个[[MK132][谓词符号]] $p_\in$ 且根据[[MK445][符号假定]]有 $\pi_P(p_\in)=2$ 即 $\mathcal{A}_{zfc}=\{p_\in\}$ 
...
$\phi_{exi}=\exists x(x\hat{=}x)$ 即存在一个集合

*** 幂集公理<<MK467>>

形式语言： $\phi_\rho=(\forall A(\exists B(\forall z(p_\in(z,B)\leftrightarrow z\subset A))))$

即对于任意结合 $A$ 其[[MK25][幂集]] $B$ 存在且亦为集合

*** 分离公理模式<<MK9>>

形式语言： $\phi_{sep}=(\forall p_1(...(\forall p_n(\forall U(\exists S(\forall x(p_\in(x,S)\leftrightarrow(p_\in(x,U)\land \phi(p_1,...,p_n,x)))))))...))$
...
为方便，记作 $S=\left\{x\big|\mathcal{M}\vDash\phi(x)[P_1,...,P_n]\right\}$
...
$T_s\vdash(\lnot(\exists x(\forall y (p_\in(y,x)))))$ 即不存在包含一切集合的集合
...
定义 $y=\left\{x\big|\mathcal{M}\vDash((x\in p_1)\land(x\not\in x))[a]\right\}$ 根据[[MK9][分离模式]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得 $y\in M$ 

此时若 $y\in y$ 则根据 $y$ 的定义有 $y\not\in y$ ，若 $y\not\in y$ 则根据 $y$ 的定义有 $y\in y$

故推出矛盾，即假设不成立，有 $\mathcal{M}\not\vDash(\exists x(\forall y (p_\in(y,x))))$ 即 $\mathcal{M}\vDash(\lnot(\exists x(\forall y (p_\in(y,x)))))$

又由于 $\mathcal{M}$ 的任意性，根据[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\lnot(\exists x(\forall y (p_\in(y,x)))))$ 
...
任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 则根据[[MK469][第一存在]]可得存在 $a\in M$ 定义 $y=\left\{x\big|\mathcal{M}\vDash((x\in p_1)\land(\lnot(x\hat{=}x)))[a]\right\}$ 根据[[MK9][分离模式]]可得 $y\in M$
...
$T_s\vdash(\forall x_1(\exists x_2 p_\in(x_1,x_2)))$
...
形式语言： $\phi_\cap=(\forall x_1(\exists x_2(\forall x_3(p_\in(x_3,x_2)\leftrightarrow(\exists x_4(p_\in(x_3,x_4)\land p_\in(x_4,x_2)))))))$
...
形式语言： $\phi_{sub}=(\forall p_1(...(\forall p_n(\forall x_1((\forall x_2(p_\in(x_2,x_1)\to\text{IsFunc}(\phi,\vec{p}_n,x_2)))\to(\exists x_3(\forall x_4(p_\in(x_4,x_3)\leftrightarrow(\exists x_5(p_\in(x_5,x_1)\land\phi(x_5,x_4)))))))))...))$
...
定义 $p_\subset(x_i,x_j)=(\forall x_{i'}(p_\in(x_{i'},x_i)\to p_\in(x_{i'},x_j)))$ 其中 $i'\ne i,i'\ne j$

即任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash p_\subset(x_1,x_2)$ 的充要条件是

在 $I(p_\in)$ 的角度上满足 $v(x_1)$ 是 $v(x_2)$ 的子集

**** 自反性<<MK21>>

$T_s\vdash(\forall x_1(\forall x_2((p_\subset(x_1,x_2)\land p_\subset(x_2,x_1))\to(x_1\hat{=}x_2))))$

***** 证明

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 若有 $\left<\mathcal{M},v\right>\vDash(p_\subset(x_1,x_2)\land p_\subset(x_2,x_1))$
...
即根据[[MK5][外延公理]]可得 $\left<\mathcal{M},v\right>\vDash(x_1\hat{=}x_2)$ 故有 $\left<\mathcal{M},v\right>\vDash((p_\subset(x_1,x_2)\land p_\subset(x_2,x_1))\to(x_1\hat{=}x_2))$

又由于 $v$ 的任意性可得 $\mathcal{M}\vDash(\forall x_1(\forall x_2((p_\subset(x_1,x_2)\land p_\subset(x_2,x_1))\to(x_1\hat{=}x_2))))$

结合 $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有 $T_s\vdash(\forall x_1(\forall x_2((p_\subset(x_1,x_2)\land p_\subset(x_2,x_1))\to(x_1\hat{=}x_2))))$

**** 传递性<<MK22>>

$T_s\vdash(\forall x_1(\forall x_2(\forall x_3((p_\subset(x_1,x_2)\land p_\subset(x_2,x_3))\to p_\subset(x_1,x_3)))))$

***** 证明

任意[[~/文档/note/数理逻辑/sllj.org::MK221][模型]] $\mathcal{M}=\left<M,I\right>\vDash T_s$ 及其[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 若有 $\left<\mathcal{M},v\right>\vDash(p_\subset(x_1,x_2)\land p_\subset(x_2,x_3))$
...
结合 $v$ 的任意性， $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有易证 $T_s\vdash(\forall x_1(\forall x_2(\forall x_3((p_\subset(x_1,x_2)\land p_\subset(x_2,x_3))\to p_\subset(x_1,x_3)))))$

**** 空集是子集<<MK23>>

$T_s\vdash(\forall x_1(\forall x_2(\phi_\varnothing(x_1)\to p_\subset(x_1,x_2))))$
...
则结合 $v$ 的任意性， $\mathcal{M}$ 的任意性以及[[~/文档/note/数理逻辑/sllj.org::MK257][第二完备]]有易证 $T_s\vdash(\forall x_1(\forall x_2(\phi_\varnothing(x_1)\to p_\subset(x_1,x_2))))$
