...
形式语言： $\phi_{ext}=(\forall x(\forall y((\forall z(p_\in(z,x)\leftrightarrow p_\in(z,y)))\to (x\hat{=}y))))$
...
形式语言： $\phi_\rho=(\forall A(\exists B(\forall z(p_\in(z,B)\leftrightarrow \phi_\subset(z, A)))))$

即对于任意结合 $A$ 其[[MK25][幂集]] $B$ 存在且亦为集合。其中 $\phi_\subset$ 为[[MK20][子集关系]]

*** 分离公理模式<<MK9>>

形式语言： $\phi_{sep}=(\forall \vec{p}_n(\forall U(\exists S(\forall x(p_\in(x,S)\leftrightarrow(p_\in(x,U)\land \phi(\vec{p}_n,x)))))))$
...
形式语言： $\phi_{par}=(\forall x_1(\forall x_2(\exists x_3((\forall x_4(p_\in(x_4,x_3)\leftrightarrow((x_4\hat{=}x_1)\lor(x_4\hat{=}x_2))))))))$
...
形式语言： $\phi_\cap=(\forall x_1(\exists x_2(\forall x_3(p_\in(x_3,x_2)\leftrightarrow(\exists x_4(p_\in(x_3,x_4)\land p_\in(x_4,x_1)))))))$

即对于任意集合 $A$ 存在 $A$ [[MK2][元素]]的[[MK2][元素]]的集合 $B$

*** 无穷公理<<MK462>>

形式语言： $\phi_{\inf}=(\exists x_1(\forall x_2(\phi_\varnothing(x_2)\to p_\in(x_2,x_1))\land(\forall x_3(p_\in(x_3,x_1)\to(\exists x_4(p_\in(x_4,x_1)\land(\forall x_5(p_\in(x_5,x_4)\leftrightarrow((x_5\hat{=}x_3)\lor p_\in(x_5,x_3))))))))))$
...
形式语言： $\phi_{sub}=(\forall \vec{p}_n\forall x_1((\forall x_2(p_\in(x_2,x_1)\to\text{IsFunc}(\phi,\vec{p}_n,x_2)))\to(\exists x_3(\forall x_4(p_\in(x_4,x_3)\leftrightarrow(\exists x_5(p_\in(x_5,x_1)\land\phi(\vec{p}_n,x_5,x_4))))))))$
...
故可得 $a=\{a\}\cup (a-a)$ 故可得 $a\cap\{a\}=a$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证与[[MK706][正则公理]]矛盾。
...
定义 $\phi_\subset(x_i,x_j)=(\forall x_{i'}(p_\in(x_{i'},x_i)\to p_\in(x_{i'},x_j)))$ 其中 $i'\ne i,i'\ne j$
...
在 $I(p_\in)$ 的角度上满足 $v(x_1)$ 是 $v(x_2)$ 的子集
...
在 $I(p_\in)$ 的角度上满足 $v(x_3)=\{v(x_1),v(x_2)\}$
...
在 $I(p_\in)$ 的角度上满足 $v(x_3)=\{\{v(x_1)\},\{v(x_1),v(x_2)\}\}=\left<v(x_1),v(x_2)\right>$
...
在 $I(p_\in)$ 的角度上满足 $v(x_2)$ 是 $v(x_1)$ 中所有元素的并集
