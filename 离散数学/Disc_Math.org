...
代数数为整系数多项式的[[~/OneDrive/高等代数/Algb-4-Multi-equ.org::MK52][根]]，若不是，则定义为超越数（例如 $\pi$ ）
...
定义 $g(x)=\sum_{i=0}^nb_ix^i$ 根据[[~/OneDrive/高等代数/Algb-4-Multi-equ.org::MK5][相等]]可得若 $f(x)\ne g(x)$ 则至少有一组 $b_k\ne a_k$
...
进一步根据[[~/OneDrive/高等代数/Algb-4-Multi-equ.org][根的个数]]可知 $f(x)$ 至多有 $n$ 个根，故 $\sigma$ 是[[MK198][满射]]
