...
定义 $x\in R$ 则有 $0\leq \left(xf(t)+g(t)\right)^2$ 故根据[[~/文档/note/数学分析/Chap7Note.org::MK8][保序]]有 $0=\int_a^b0dt\leq\int_a^b\left(xf(t)+g(t)\right)^2dt$ 
...
故根据[[~/文档/note/数学分析/Chap11Note.org::MK38][Heine-Borel定理]]得 $D$ 为紧集

又根据[[~/文档/note/数学分析/Chap11Note.org::MK40][康托定理]]得 $f(\vec{x},\vec{y})$ 在区域 $D$ 上一致连续，定义积分区域 $S$ 的面积为 $V$

则根据[[~/文档/note/数学分析/Chap11Note.org::MK32][补充多元函数一致连续定义]]
...
根据中[[~/文档/note/数学分析/Chap13Note.org::MK3][线性]]，[[~/文档/note/数学分析/Chap13Note.org::MK5][当 $f(\vec{x})=1$ 时]]，[[~/文档/note/数学分析/Chap13Note.org::MK4][保序性]]以及[[~/文档/note/数学分析/Chap13Note.org::MK49][绝对可积性]]得
...
由于[[MK1][含参变量常义积分定义]]，被积函数 $f(\vec{x},\vec{y})$ 在区域 $D$ 上连续，故根据[[~/文档/note/数学分析/Chap13Note.org::MK11][推论]]任意维度下子重积分均可积

故根据[[~/文档/note/数学分析/Chap13Note.org::MK8][不规则区域上重积分]]重积分可化为累次积分。
...
定义 $\vec{y}=(\vec{y}',y_k)$ ，故根据导数定义，[[~/文档/note/数学分析/Chap13Note.org::MK3][线性]]以及[[MK2][连续性定义推论]]
...
故根据[[~/文档/note/数学分析/Chap7Note.org::MK28][牛顿莱布尼茨公式]] $I(u,v,w)=\int_u^vf(x,w)dx=G(u,w)-G(v,w)$ 
...
由于 $I(u,v,w)$ 的偏导数均连续，故根据[[~/文档/note/数学分析/Chap12Note.org::MK5][可微条件]]函数在定义域上可微
...
故根据[[~/文档/note/数学分析/Chap12Note.org::MK7][链式法则]] $\frac{\partial I(u,v,w)}{\partial y}=\frac{\partial I(u,v,w)}{\partial u}\frac{\partial u}{\partial y}+\frac{\partial I(u,v,w)}{\partial v}\frac{\partial v}{\partial y}+\frac{\partial I(u,v,w)}{\partial w}\frac{\partial w}{\partial y}$
...
根据[[~/文档/note/数学分析/Chap8Note.org::MK7][必要]]可得在以上条件下 $\forall A>A_0,\forall y\in[c,d]\Rightarrow\left|\int_a^Af(x,y)dx-K(y)\right|<2\epsilon$

则有 $\forall\epsilon>0,\exists A_0>a,\forall A>A_0,\forall y\in[c,d]\Rightarrow \left|\int_a^Af(x,y)dx-K(y)\right|<\epsilon$ 根据[[~/文档/note/数学分析/Chap8Note.org::MK5][定义]]可得 $K(y)=\int_a^\infty f(x,y)dx=I(y)$
...
由于 $\int_a^\infty F(x)dx$ 收敛，根据[[~/文档/note/数学分析/Chap8Note.org::MK6][柯西收敛原理：]]可得 $\forall\epsilon>0,\exists A_0,\forall A''>A'>A_0\Rightarrow\left|\int_{A'}^{A''}F(x)dx\right|<\epsilon$

由于 $0\leq|f(x,y)|\leq F(x)$ 故有 $\forall\epsilon>0,\exists A_0,\forall A''>A'>A_0\Rightarrow\int_{A'}^{A''}F(x)dx<\epsilon$

根据[[~/文档/note/数学分析/Chap7Note.org::MK8][保序]]及[[~/文档/note/数学分析/Chap7Note.org::MK27][绝对可积]]可得 $\left|\int_{A'}^{A''}f(x,y)dx\right|\leq\int_{A'}^{A''}|f(x,y)|dx\leq\int_{A'}^{A''}F(x)dx$
...
根据[[~/文档/note/数学分析/Chap8Note.org::MK1][第二积分中值定理：]]及条件（2）可得 $\forall y\in[c,d]$ 均有：
...
根据[[~/文档/note/数学分析/Chap8Note.org::MK1][第二积分中值定理：]]及条件（1）可得 $\forall y\in[c,d]$ 均有：
...
根据[[~/文档/note/数学分析/Chap10Note.org::MK1][定义]]，证毕。
...
根据[[~/文档/note/数学分析/Chap10Note.org::MK12][连续性：]]可得 $S(y)$ 连续，即 $I(y)$ 连续
...
根据[[~/文档/note/数学分析/Chap10Note.org::MK12][连续性：]]可得 $\int_c^d S(y)dy = \sum_{i=1}^\infty \int_c^d dy\int_{a_i}^{a_{i+1}}f(x,y)dx$
...
定义 $I_y(y) = \int_a^\infty f_y(x,y)dx$ 根据条件（2），（3），[[MK8][可积性]]及[[~/文档/note/数学分析/Chap7Note.org::MK28][牛顿莱布尼茨公式]] 
...
根据[[~/文档/note/数学分析/Chap8Note.org::MK7][必要]]可得在以上条件下 $\forall \eta<\eta_0,\forall y\in[c,d]\Rightarrow\left|\int_a^{b-\eta} f(x,y)dx-K(y)\right|<2\epsilon$

则有 $\forall\epsilon>0,\exists \eta_0>0,\forall \eta<\eta_0,\forall y\in[c,d]\Rightarrow \left|\int_a^{b-\eta}f(x,y)dx-K(y)\right|<\epsilon$ 根据[[~/文档/note/数学分析/Chap8Note.org::MK8][定义：]]可得 $K(y)=\int_a^b f(x,y)dx=I(y)$
...
由于 $\int_a^b F(x)dx$ 收敛，根据[[~/文档/note/数学分析/Chap8Note.org::MK6][柯西收敛原理：]]可得 $\forall\epsilon>0,\exists \eta_0,\forall \eta''<\eta'<\eta_0\Rightarrow\left|\int_{b-\eta'}^{b-\eta''}F(x)dx\right|<\epsilon$

由于 $0\leq|f(x,y)|\leq F(x)$ 故有 $\forall\epsilon>0,\exists \eta_0,\forall\eta''<\eta'<\eta_0\Rightarrow\int_{b-\eta'}^{b-\eta''}F(x)dx<\epsilon$

根据[[~/文档/note/数学分析/Chap7Note.org::MK8][保序]]及[[~/文档/note/数学分析/Chap7Note.org::MK27][绝对可积]]可得 $\left|\int_{b-\eta'}^{b-\eta''}f(x,y)dx\right|\leq\int_{b-\eta'}^{b-\eta''}|f(x,y)|dx\leq\int_{b-\eta'}^{b-\eta''}F(x)dx$
...
则根据[[~/文档/note/数学分析/Chap8Note.org::MK12][比较判别法]]当且仅当 $p>0$ 时 $\int_0^{\frac{1}{2}}x^{p-1}(1-x)^{q-1}dx$ 收敛
...
故根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]分割积分区域 $\Gamma(s)=\int_0^1 x^{s-1}e^{-x}dx+\int_1^\infty x^{s-1}e^{-x}dx$
...
故根据[[~/文档/note/数学分析/Chap8Note.org::MK12][比较判别法]]当 $s>0$ 时 $\int_0^1 x^{s-1}e^{-x}dx$ 存在
...
由于当 $x\in[1,s+2]$ 时 $x^{s-1}e^{-x}$ 连续，故根据[[~/文档/note/数学分析/Chap7Note.org::MK3][推论1]]若 $s>1$ 则 $\int_1^{s+2} x^{s-1}e^{-x}dx$ 必可积
...
则根据[[~/文档/note/数学分析/Chap8Note.org::MK11][Dirichlet]] $\int_s^\infty \frac{1}{x}\frac{x^s}{e^x}dx$ 对于任意 $s$ 收敛

根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]] $\int_1^\infty \frac{1}{x}\frac{x^s}{e^x}dx$ 对于任意 $s$ 收敛
...
定义 $[a,b]$ 其中 $a>0,b<\infty$ 同样根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]分割积分区域
...
故根据[[~/文档/note/大学物理/Note/DiffEq.org::MK22][洛必达法则]]有 $\lim_{x\to0^+}\frac{f(x)}{g(x)}=\lim_{x\to0^+}\frac{f'(x)}{g'(x)}$ 即 $\lim_{x\to0^+}\frac{(\ln(x))^n}{x^{-0.5a}}=\lim_{x\to0^+}\frac{x^{0.5a}}{-0.5a}=0$ 
...
则根据[[~/文档/note/大学物理/Note/DiffEq.org::MK22][洛必达法则]]有 $\lim_{x\to0^+}\frac{f(x)}{g(x)}=\lim_{x\to0^+}\frac{f'(x)}{g'(x)}=0$
...
则根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]分割积分 $\int_0^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx=\int_0^\delta x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx+\int_\delta^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$
由于 $x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 在 $[\delta,1]$ 上连续，

故根据根据[[~/文档/note/数学分析/Chap7Note.org::MK3][推论1]] $\int_\delta^1x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 可积
...
则根据[[~/文档/note/数学分析/Chap8Note.org::MK11][Dirichlet]] $\int_0^\delta x^{0.5a-1}\left|\frac{(\ln(x))^n}{x^{-0.5a}}\right|dx$ 收敛
...
故根据[[~/文档/note/数学分析/Chap8Note.org::MK11][Dirichlet]]得 $\int_1^\infty x^{b-1}e^{-x}(\ln(x))^ndx$ 收敛
...
故根据[[~/文档/note/数学分析/Chap13Note.org::MK43][区域无限反常重积分计算]]以及[[~/文档/note/数学分析/Chap13Note.org::MK50][无界反常重积分计算]]
...
则根据[[~/文档/note/数学分析/Chap13Note.org::MK52][重积分变量代还]]有 $\int_{D'} s^{2p-1}t^{2q-1}e^{-s^2-t^2}dv=\int_{T(D')} r^{2(p+1)-1}e^{-r^2}(\cos\theta)^{2p-1}(\sin\theta)^{2q-1}dv$
...
根据[[~/文档/note/数学分析/Chap13Note.org::MK28][引理1]]和[[~/文档/note/数学分析/Chap13Note.org::MK53][引理1]]
...
根据条件（3）及[[~/文档/note/数学分析/Chap8Note.org::MK10][非负反常积分判别]]得 $\int_a^b u(x)dx$ 也收敛

故 $\forall\epsilon>0$

根据条件（3），（4）及[[~/文档/note/数学分析/Chap8Note.org::MK8][定义：]]有 $\exists\delta,\forall 0<\eta<\delta\Rightarrow\int_{b-\eta}^b\phi(x)dx<\frac{\epsilon}{3}$
...
根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]] $\int_a^c u(x)dx+\int_c^b u(x)dx=\int_a^b u(x)dx,\int_a^c u_n(x)dx+\int_c^b u_n(x)dx=\int_a^b u_n(x)dx$
...
根据[[~/文档/note/数学分析/Chap9Note.org::MK11][正弦函数无穷乘积展开]]得 $\sin(x)=x\prod_{i=1}^\infty\left(1-\frac{x^2}{i^2\pi^2}\right)$
...
根据[[~/文档/note/数学分析/Chap9Note.org::MK9][P级数：]]得 $\sum_{i=1}^\infty \frac{2b}{i^2\pi^2}$ 收敛，且 $l=\lim_{n\to\infty}\frac{\frac{2b}{i^2\pi^2-b^2}}{\frac{2b}{i^2\pi^2}}=\frac{1}{1-\frac{b^2}{i^2\pi^2}}=1$ 故 $0<l<\infty$ 

又由于 $b<\pi\Rightarrow\frac{2b}{i^2\pi^2-b^2}>0$ 根据[[~/文档/note/数学分析/Chap9Note.org::MK7][比较判别法：]]得 $\sum_{i=1}^\infty\frac{2b}{i^2\pi^2-b^2}$ 亦收敛

又根据[[~/文档/note/数学分析/Chap10Note.org::MK6][Weierstrass判别]]得 $u_n'(x)$ 在 $x\in(0,b]$ 上一致收敛
...
故当 $x\in(0,b]$ 时根据[[~/文档/note/数学分析/Chap10Note.org::MK14][可导性：]]可得 $u_n'(x)=(\ln(\sin(x))-\ln(x))'$
...
根据[[~/文档/note/数学分析/Chap10Note.org::MK10][经典例题]]得 $(1+t)^{-1}=\sum_{i=0}^\infty\left(\binom {-1} n t^i\right)$
...
（5） $\forall\eta>0$ 定义 $a_n(t)=t^{s+i-1},b_n(t)=(-1)^n$ 则当 $t\in[\eta,1-\eta]$ 时有
      ...
      a) $0<a_n(t)<(1-\eta)^{s+n-1}$ 且 $\lim_{n\to\infty}(1-\eta)^{s+n-1}=0$ ，
         ...
         则根据[[~/文档/note/数学分析/Chap10Note.org::MK6][Weierstrass判别]]及夹逼定理得 $a_n(t)$ 一致收敛到 $0$

      b) $\left|\sum_{i=0}^n b_i(t)\right|\leq 1<1.5$

      故根据[[~/文档/note/数学分析/Chap10Note.org::MK11][Dirichlet:]]得函数项级数 $\left\{\sum_{i=0}^n(-1)^it^{s+i-1}\right\}$ 在 $[\eta,1-\eta],1>\eta>0$ 上一致收敛 
...
根据[[~/文档/note/数学分析/Chap9Note.org::MK10][Stirling公式：]]得 $\lim_{n\to\infty}\frac{n!}{\sqrt{2\pi}n^{n+\frac{1}{2}}e^{-n}}=1$ 故根据极限保序性及四则运算有：
...
根据[[~/文档/note/数学分析/Chap10Note.org::MK15][幂级数展开：]]在 $x_0=0$ 点有展开
...
则根据[[~/文档/note/数学分析/Chap9Note.org::MK6][达朗贝尔]]以及[[~/文档/note/数学分析/Chap9Note.org::MK8][收敛性质]]
