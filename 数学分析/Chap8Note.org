...
根据[[~/文档/note/大学物理/Note/DiffEq.org][介值定理]]，必存在 $\eta\in(a,b)\Rightarrow f(\eta)=\frac{\int_a^b f(x)g(x) dx}{\int_a^b g(x) dx}\Rightarrow f(\eta)\int_a^b g(x) dx=\int_a^b f(x)g(x) dx$
