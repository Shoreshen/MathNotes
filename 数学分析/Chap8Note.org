...
根据[[~/文档/note/大学物理/Note/DiffEq.org::MK3][有界性定理]]可得 $m\leq f(x)\leq M$
...
根据[[~/文档/note/大学物理/Note/DiffEq.org::MK21][介值定理]]，必存在 $\eta\in(a,b)\Rightarrow f(\eta)=\frac{\int_a^b f(x)g(x) dx}{\int_a^b g(x) dx}\Rightarrow f(\eta)\int_a^b g(x) dx=\int_a^b f(x)g(x) dx$
...
则根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]，取 $A',A''>\delta$
...
取 $\delta+1<A$ 则根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]有 $\int_a^A f(x) dx=\int_a^{\delta+1} f(x) dx+\int_{\delta+1}^A f(x) dx$
...
根据[[~/文档/note/数学分析/Chap11Note.org::MK29][Bolzano-Weierstrass定理]]必有收敛子列 $\int_a^{x_i} f(x) dx,x_i\in\{x_n|n>N'\}$ 且因为 $x_i\in\{x_n|n>N'\}\Rightarrow \lim_{i\to\infty}x_i=\infty$
...
根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]] $\int_a^A f(x)dx+\int_A^{x_I} f(x)dx=\int_a^{x_I} f(x)dx\Rightarrow \int_a^A f(x)dx=\int_a^{x_I} f(x)dx-\int_A^{x_I} f(x)dx$ 有 
...
则根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]，取 $\eta_1,\eta2\in (0,\delta)$
...
取 $b-\frac{\delta}{2}<A<b$ 则根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]有 $\int_a^A f(x) dx=\int_a^{b-\frac{\delta}{2}} f(x) dx+\int_{b-\frac{\delta}{2}}^A f(x) dx$
...
根据[[~/文档/note/数学分析/Chap11Note.org::MK29][Bolzano-Weierstrass定理]]必有收敛子列 $\int_a^{x_i} f(x) dx,x_i\in\{x_n|n>N'\}$ 且因为 $x_i\in\{x_n|n>N'\}\Rightarrow \lim_{i\to\infty}x_i=b^-$
