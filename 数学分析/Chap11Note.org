...
亦可根据[[~/文档/note/实变函数/SBHS.org::MK31][完备性等价定理]]从[[MK9][确界存在定理]]证得
...
若 $V$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$ 上的根据[[~/文档/note/高等代数/Algb-2-Liner_Space.org::MK1][线性空间]]，定义 $R$ 为实数，则根据[[~/文档/note/离散数学/Disc_Math.org::MK194][函数]]有函数 $\|\cdot\|:V\rightarrow R$ 满足：
...
根据[[~/文档/note/实变函数/SBHS.org::MK45][完备集]]，进一步根据闭集定义以及[[MK8][聚点]]定义，
...
对于任意 $a\in A$ 定义 $\delta(a)=\frac{1}{2}d(\{a\},B)$ 其中 $d(a,B)$ 是[[~/文档/note/实变函数/SBHS.org::MK89][子集距离]]
...
则根据[[~/文档/note/实变函数/SBHS.org::MK38][度量空间]]三角性可得 $d(a',x)+d(x,b')\geq d(a',b')$
...
根据[[MK10][开集定义]]表达式 $\forall \vec{x}\in S\rightarrow \vec{x}\in S_i$ 为真是 $S$ 为开集的等价条件，由于对于空集 $\vec{x}\in S'$ 永假，
