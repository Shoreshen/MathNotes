...
根据[[~/文档/note/数学分析/Chap7Note.org::MK26][乘积可积]]可得 $f(x)^2,g(x)^2,f(x)g(x)$ 均在 $[a,b]$ 上可积

根据[[~/文档/note/数学分析/Chap7Note.org::MK25][定积分定义]]，对于任意可积函数在任意划分与取点下，只要满足 $\lambda\to0$ 黎曼和均收敛于定积分，故有：
...
则根据[[~/文档/note/数学分析/Chap7Note.org::MK24][三角函数正交性:]]及[[~/文档/note/数学分析/Chap10Note.org::MK13][可积性：]]有
...
根据积分[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]] $\int_0^\infty\frac{\sin(x)}{x}dx=\int_0^{\frac{\pi}{2}}\frac{\sin(x)}{x}dx+\int_{\frac{\pi}{2}}^\infty\frac{\sin(x)}{x}dx$

分别为区间无限及无界反常积分由于 $\int_a^b\sin(x)dx$ 有界，对于任意 $0\leq a<b<\infty$ 且 $\lim_{x\to\infty}\frac{1}{x}=0$ 且 $\frac{1}{x}$ 单调

故根据[[~/文档/note/数学分析/Chap8Note.org::MK11][Dirichlet]] $\int_{\frac{\pi}{2}}^\infty\frac{\sin(x)}{x}dx$ 收敛

又因为 $\frac{\sin(x)}{x}<1$ 且有 $\lim_{a\to0}\int_a^\frac{\pi}{2}1dx=\frac{\pi}{2}$ 收敛，故根据[[~/文档/note/数学分析/Chap8Note.org::MK10][非负反常积分判别]] $\int_0^{\frac{\pi}{2}}\frac{\sin(x)}{x}dx$ 亦收敛
...
且有 $\sin(x)=-\sin(x+\pi)$ 故 $\frac{\sin(x)}{x}+\frac{\sin(x+\pi)}{x+\pi}>0$ 根据[[~/文档/note/数学分析/Chap7Note.org::MK8][保序]]可得 $\int_{2i\pi}^{(2i+1)\pi}\left(\frac{\sin(x)}{x}+\frac{\sin(x+\pi)}{x+\pi}\right)dx\geq0$
...
由于可积，则根据[[~/文档/note/数学分析/Chap7Note.org::MK1][定积分可积充要条件2:]]有 $\forall\epsilon>0,\exists\delta,\forall\lambda<\delta,\forall P\Rightarrow\left|\sum_{i=1}^\infty\omega_i\Delta x_i\right|<\frac{\epsilon}{2}$

且根据[[~/文档/note/数学分析/Chap7Note.org::MK26][乘积可积]]得 $\phi(x)\sin(px)$ 亦可积
...
则根据[[~/文档/note/数学分析/Chap7Note.org::MK5][线性]]，[[~/文档/note/数学分析/Chap7Note.org::MK27][绝对可积]]，[[~/文档/note/数学分析/Chap7Note.org::MK26][乘积可积]]以及[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]可得
...
定义 $b$ 为奇点，则根据条件有 $\int_a^b|\phi(x)|dx$ 存在，根据[[~/文档/note/数学分析/Chap8Note.org::MK8][定义：]] $\forall\epsilon>0,\exists\delta>0,\forall\eta<\delta\Rightarrow\int_{b-\eta}^b|\phi(x)|dx<\frac{\epsilon}{2}$
...
故根据[[~/文档/note/数学分析/Chap7Note.org::MK27][绝对可积]]以及根据[[~/文档/note/数学分析/Chap8Note.org::MK9][反常积分绝对收敛:]]得
...
故根据[[~/文档/note/数学分析/Chap7Note.org::MK3][推论1]]得出 $g(u)$ 在 $[0,\pi]$ 上可积

故根据[[~/文档/note/数学分析/Chap7Note.org::MK26][乘积可积]]有 $g(u)\phi(u)$ 在 $u\in[0,\delta]$ 上亦可积或绝对可积 则有
...
由于 $\phi(u)$ 单调，则 $\phi(u)-\phi(0^+)$ 亦单调，根据[[~/文档/note/数学分析/Chap7Note.org::MK21][推论2]]得出可积

由于 $\frac{1}{u}$ 在 $[\eta,\delta]$ 上亦可积，则根据[[~/文档/note/数学分析/Chap7Note.org::MK26][乘积可积]] $\frac{\phi(u)-\phi(0^+)}{u}$ 可积
...
故根据[[~/文档/note/数学分析/Chap7Note.org::MK30][推论]]有在 $(0,\eta]$ 上的反常积分等于在 $[0,\eta]$ 上的常义积分
...
由于 $\phi(u)-\phi(0^+)$ 单调 $\frac{\sin(pu)}{u}$ 在 $[0,\eta]$ 上连续，则根据[[~/文档/note/数学分析/Chap7Note.org::MK3][推论1]]得必可积

故根据[[~/文档/note/数学分析/Chap8Note.org::MK1][第二积分中值定理：]]得
...
则根据[[~/文档/note/数学分析/Chap7Note.org::MK5][线性]]，三角函数和差化积
...
故根据[[~/文档/note/数学分析/Chap7Note.org::MK30][推论]]有在 $(0,\pi]$ 上的反常积分等于在 $[0,\pi]$ 上的常义积分
...
定义 $\sigma(x)$ 为 $S_m(x)$ 的收敛值，则有 $\lim_{m\to\infty}\{S_m(x)-\sigma(x)\}=0$ 根据[[~/文档/note/数学分析/Chap7Note.org::MK5][线性]]有
...
则有当 $u\in[0,\delta]$ 时 $\phi(u),\phi'(u)$ 单调，故根据[[~/文档/note/数学分析/Chap7Note.org::MK7][区间可加]]和极限四则运算拆分积分
...
同理 $\frac{1}{\sin\left(\frac{u}{2}\right)}$ 可积，故根据[[~/文档/note/数学分析/Chap7Note.org::MK26][乘积可积]] $\frac{\phi(u,x)}{\sin\left(\frac{u}{2}\right)}$ 在 $u\in[a,\pi],0<a<\pi$ 可积或绝对可积
...
由于 $\left|\frac{f(x+u)-f(x^{+})}{u}\right|>0$ 故根据[[~/文档/note/数学分析/Chap8Note.org::MK10][非负反常积分判别]] $\left|\frac{f(x+u)-f(x^{ +})}{u}\right|$ 亦可积
...
故 $F(x)$ 为周期函数，周期为 $2\pi$ 。且则根据[[~/文档/note/数学分析/Chap7Note.org::L1][定理1:]]可得 $F(x)$ 连续
...
由于 $F(c)=0$ 故 $\frac{A_0}{2}=-\sum_{i=1}^\infty\left(-\frac{b_i}{i}\cos(ic)+\frac{a_i}{i}\sin(ic)\right)$ 故根据[[~/文档/note/数学分析/Chap7Note.org::MK5][线性]]有
