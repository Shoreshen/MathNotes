...
根据[[~/文档/note/大学物理/Note/DiffEq.org::MK1][可导必连续]]得出 $\Phi(u)$ 在某个闭区间 $u\in (a,b)$ 上连续，则存在区域 $[c,d]\subset [a,b]$ 使得函数在 $u\in(c,d)$ 上可导， $u\in [c,d]$ 上连续

综合上述条件根据[[~/文档/note/大学物理/Note/DiffEq.org::MK5][拉格朗日中值定理]]有 $\Phi(u)-\Phi(u_0)=\Phi(\xi)(u-u_0),\xi\in (u_0,u)$ 
...
根据[[~/文档/note/大学物理/Note/DiffEq.org::MK5][拉格朗日中值定理]]在 $\sqrt{(x-x_0)^2+(y-y_0)^2}<\delta$ 时有 $[f(x,y)-f(x_0,y)]-[f(x,y_0)-f(x_0,y_0)]=\frac{\partial [f(x,y)-f(x_0,y)]}{\partial y}\big|_{y=\xi},\xi\in (y,y_0)$
...
若区域 $D\subset R^n$ 为连通集（根据[[~/文档/note/数学分析/Chap11Note.org::MK32][补充连通定义]]）其中任意两点 $\vec{x},\vec{y}$ 相连的直线属于该区域 $\left\{\vec{x}+t(\vec{y}-\vec{x})\big|t\in [0,1]\right\}\in D$ 则 $D$ 为凸区域
...
故 $\frac{d(\Phi(t_0))}{dt}=\sum_{i=1}^n \left\{\frac{\partial f(\vec{x}+t_0(\vec{y}-\vec{x}))}{\partial x_i}(y_i-x_i)\right\},t_0\in[0,1]$ 故 $\Phi(t)$ 在 $(0,1)$ 上可导 $[0,1]$ 上连续，则应用一元函数[[~/文档/note/大学物理/Note/DiffEq.org::MK5][拉格朗日中值定理]]可得
...
由于 $\Phi(t)$ 在 $t\in[0,1]$ 有定义，且在 $t\in[0,1]$ 连续 $t\in(0,1)$ 可导，故根据[[~/文档/note/数学分析/Chap10Note.org::MK16][补充2：Taylor公式]]有
...
同理根据[[~/文档/note/数学分析/Chap10Note.org::MK16][补充2：Taylor公式]]的拉格朗日余项有
...
定义 $\Phi(x_i)=f(...,x_{(i-1)_0},x_i,x_{(i+1)_0}...)$ 则有 $x_{i_0}$ 为 $\Phi(x_i)$ 的极值点，则根据[[~/文档/note/大学物理/Note/DiffEq.org::MK4][费马引理]]可得 $\frac{d\Phi(x_i)}{dx_i}=0$
