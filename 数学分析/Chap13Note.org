...
由于 $D_{n-1}$ 有界闭集，故根据[[~/文档/note/数学分析/Chap11Note.org][Heine-Borel定理]] $D_{n-1}$ 为紧集
...
由于有界闭集，则根据[[~/文档/note/数学分析/Chap11Note.org][Heine-Borel定理]]得 $D$ 是紧集，又由于 $f(x,y)$ 连续，根据[[~/文档/note/数学分析/Chap11Note.org][康托定理]]得函数一直连续，
...
定义 $S=\left\{\vec{f}(t)\big|t\in[0,1]\right\}$ 由于 $[0,1]$ 有界闭集，根据[[~/文档/note/数学分析/Chap11Note.org][Heine-Borel定理]]得出是紧集。
...
（4） 由于连续映射，且 $A^k$ 为有界集闭集，
      ...
      根据[[~/文档/note/数学分析/Chap11Note.org][Heine-Borel定理]]及[[~/文档/note/数学分析/Chap11Note.org::MK3][紧集的连续映射（连续函数）也是紧集]]得出 $[T_k(\vec{x}_u)-T_k(\vec{x}_l)]$ 有界
...
根据条件（4）有 $D$ 为有界闭集，根据[[~/文档/note/数学分析/Chap11Note.org][Heine-Borel定理]] $D$ 紧集

根据[[~/文档/note/数学分析/Chap11Note.org::MK3][紧集的连续映射（连续函数）也是紧集]]得 $T(D)$ 紧集，同理 $T(D)$ 有界闭集 

故 $f(\vec{t})$ 为有界闭集上的连续函数，根据[[~/文档/note/数学分析/Chap11Note.org][最大值定理]]存在 $m'\leq f(\vec{t})\leq M',\vec{t}\in T(D)$ 同理存在 $m''\leq f\left(\vec{T}(\vec{x})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial\vec{x}}\right)\right|\leq M''$
...
根据[[~/文档/note/数学分析/Chap11Note.org][Heine-Borel定理]] $D$ 为紧集，
...
（3） $f_n(x),f(x)$ 在 $[a,b]$ 上均可积，故根据[[~/文档/note/数学分析/Chap7Note.org::MK5][线性]] $\int_a^bf_n(x)-f(x)dx$ 也可积，
     ...
     又根据[[~/文档/note/数学分析/Chap7Note.org][绝对可积]] $\int_a^b|f_n(x)-f(x)|dx$ 也可积 
