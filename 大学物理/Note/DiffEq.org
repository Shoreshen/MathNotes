...
因为 $x_n$ 有界故根据[[~/文档/note/Series.org][Bolzano-Weiserstrass定理]]，必存在收敛子列 $x_{n_k}\Rightarrow\lim_{k\to\infty} x_{n_k}=\xi,\xi\in\{x_{n_k}|k\in N^+\}\subset[a,b]$ 
...
故根据[[~/文档/note/Chap12Note.org][一元隐函数存在定理]]存在 $x=g^{-1}(y)$ 且 $\frac{dx}{dy}=-\frac{\frac{\partial F(x,y)}{\partial y}}{\frac{\partial F(x,y)}{\partial y}}=-\frac{-1}{g'(x)}=\frac{1}{g'(x)}$
...
虚部， $\sin(x),\cos(x)$ 同理。故判别，当 $x\in(-\infty,+\infty)$ 时，展开式绝对收敛。根据[[~/文档/note/Series.org][推论1]]可得：绝对收敛 $\Rightarrow$ 级数收敛
...
根据[[~/文档/note/Chap7Note.org][牛顿莱布尼茨公式]]及[[~/文档/note/Chap8Note.org::MK2][边界无穷积分]]，若 $e^{-st}f(t)$ 连续且 $F(x)=\int e^{-st}f(t) dx$ ，则有
t
\begin{aligned}
...
定义 $s<0$ 且 $\lim_{s\to\infty}\left\{e^{-st}f(t)\right\}=0$ 则根据根据[[~/文档/note/Chap7Note.org][牛顿莱布尼茨公式]]，[[~/文档/note/Chap8Note.org::MK2][边界无穷积分]]以及分部积分法有
...
因为连续，则根据[[~/文档/note/Chap10Note.org][Weierstrass第一逼近定理]]存在多项式 $P_n(x)$ ，使得 $\forall \epsilon>0,\ \exists N,\ \forall n>N,\forall x\in[0,1]\Rightarrow |f(x)-P_n(x)|<\epsilon$
