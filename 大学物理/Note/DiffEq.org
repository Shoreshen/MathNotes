...
因为 $\int_0^\infty e^{-st}f(t)dt=0,\forall s\in R$ 则根据[[~/文档/note/数学分析/Chap8Note.org::MK4][柯西收敛]]有 $\forall \epsilon>0,&\exist \delta\geq a,\forall A',A''>\delta,\Rightarrow \left|\int_{A'}^{A''} e^{-st}f(t)dt\right|<\epsilon$
...
根据[[~/文档/note/Chap11Note.org::MK20][实数系闭区间套定理]] $\exists \xi \in \cap_{n=1}^\infty [a_n,b_n]$ 则 $f(\xi)$ 无界

又根据[[MK2][函数连续定义]]有 $\lim_{x\to\xi}f(x)=f(\xi)$ 根据[[~/文档/note/数学分析/Chap11Note.org::MK36][局部有界性]]可得则 $\exists \delta\Rightarrow-\infty<L<f(x)<U<+\infty, x \in O(\xi,\delta)$
...
因为 $x_n$ 有界故根据[[~/文档/note/数学分析/Chap11Note.org::MK29][Bolzano-Weiserstrass定理]]，必存在收敛子列 $x_{n_k}\Rightarrow\lim_{k\to\infty} x_{n_k}=\xi,\xi\in\{x_{n_k}|k\in N^+\}\subset[a,b]$ 
...
故根据[[~/文档/note/数学分析/Chap12Note.org::MK34][一元隐函数存在定理]]存在 $x=g^{-1}(y)$ 且 $\frac{dx}{dy}=-\frac{\frac{\partial F(x,y)}{\partial y}}{\frac{\partial F(x,y)}{\partial y}}=-\frac{-1}{g'(x)}=\frac{1}{g'(x)}$
...
根据[[~/文档/note/数学分析/Chap10Note.org::MK10][经典例题]]中步骤，需证明展开式收敛且余项趋于零

**** 证明展开式收敛

利用[[~/文档/note/数学分析/Chap9Note.org::MK6][达朗贝尔]]判别法，对实部进行判别

\begin{aligned}
\lim_{n\to\infty}\left|\frac{\frac{x^{2n+2}}{(2n+2)!}}{\frac{x^{2n}}{(2n)!}}\right|=\lim_{n\to\infty}\left|\frac{x^2}{(2n+1)(2n+2)}\right|
\end{aligned}

虚部， $\sin(x),\cos(x)$ 同理。故判别，当 $x\in(-\infty,+\infty)$ 时，展开式绝对收敛。根据[[~/文档/note/数学分析/Chap9Note.org::MK14][推论1]]可得：绝对收敛 $\Rightarrow$ 级数收敛
...
根据[[~/文档/note/Chap7Note.org::MK28][牛顿莱布尼茨公式]]及[[~/文档/note/Chap8Note.org::MK2][边界无穷积分]]，若 $e^{-st}f(t)$ 连续且 $F(x)=\int e^{-st}f(t) dx$ ，则有
t
\begin{aligned}
...
根据[[~/文档/note/Chap7Note.org::MK5][线性]]
...
定义 $s<0$ 且 $\lim_{s\to\infty}\left\{e^{-st}f(t)\right\}=0$ 则根据根据[[~/文档/note/Chap7Note.org::MK28][牛顿莱布尼茨公式]]，[[~/文档/note/Chap8Note.org::MK2][边界无穷积分]]以及分部积分法有
...
因为连续，则根据[[~/文档/note/数学分析/Chap10Note.org::MK17][Weierstrass第一逼近定理]]存在多项式 $P_n(x)$ ，使得 $\forall \epsilon>0,\ \exists N,\ \forall n>N,\forall x\in[0,1]\Rightarrow |f(x)-P_n(x)|<\epsilon$

假设 $P_n(x)=p_0+p_1x^1+p_2x^2+...+p_nx^n$ 根据[[~/文档/note/Chap7Note.org::MK5][线性]] $\int_0^1 P_n(x)f(x) dx=p_0\int_0^1f(x) dx+p_1\int_0^1x^1f(x) dx+p_2\int_0^1x^2f(x) dx+...+p_n\int_0^1x^nf(x) dx=0$

$\lim_{n\to\infty}P_n(x)f(x)=\sum_{i=0}^\infty p_ix^if(x)$ 是函数项级数。

因为 $f(x)$ 闭区间连续根据[[MK3][有界性定理]]， $|f(x)|<K,x\in[0,1]$ ，且 $P_n(x)$ 一致收敛,则根据[[~/文档/note/数学分析/Chap10Note.org::MK7][Abel:]]定理，级数 $\sum_{i=0}^\infty p_ix^if(x)$ 一致收敛。

则根据[[~/文档/note/数学分析/Chap10Note.org::MK8][推论：]] $\int_0^1\lim_{n\to\infty}\{P_n(x)\}f(x)dx=\lim_{n\to\infty}\int_0^1P_n(x)f(x)dx=0$
...
根据[[~/文档/note/Chap7Note.org::MK7][区间可加]] $\int_0^1 |f(x)| dx=\int_0^{x'-\delta'}|f(x)|dx +\int_{x'-\delta'}^{x' +\delta'}|f(x)|dx +\int_{x' +\delta}^1 |f(x)|dx$

由于 $|f(x)|>0,\forall x\in O(x',\delta')$ ，故连续，则根据[[~/文档/note/Chap7Note.org::MK4][推论]]得出 $\int_{x'-\delta'}^{x' +\delta'}|f(x)|dx$ 可积，并且 $\int_{x'-\delta'}^{x' +\delta'}|f(x)|dx>0$
