...
根据[[~/文档/note/大学物理/Note/Note1.org::MK1][动量定律]]以及[[MK1][线性恢复力]]可得 $F=ma\Rightarrow m\frac{d^2x}{dt^2}=-kx$

整理上式得 $\frac{d^2x}{dt^2}+\frac{k}{m}x=0$ 定义 $\omega^2=\frac{k}{m}$ 则有 $\frac{d^2x}{dt^2}+\omega^2x=0$ 此为 *简谐运动的动力学方程*

上式为二阶常系数齐次微分方程，根据[[~/文档/note/大学物理/Note/DiffEq.org::MK13][二阶常系数齐次微分方程]]
